	...
	_each(('cut,copy,paste').split(','), function(i, name) {
		self.clickToolbar(name, function() {
			...
			try {
				...
				jzdc_alert(self.lang(name + 'Error'));
			}
		});
	});
				...
				yesBtn : {
					...
					click : function(e) {
						...
						if (url == 'http://' || K.invalidUrl(url)) {
							jzdc_alert(self.lang('invalidUrl'));
							urlBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(width)) {
							jzdc_alert(self.lang('invalidWidth'));
							widthBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(height)) {
							jzdc_alert(self.lang('invalidHeight'));
							heightBox[0].focus();
							return;
						}
				...
				var uploadbutton = K.uploadbutton({
					...
					afterUpload : function(data) {
						...
						if (data.error === 0) {
							...
							jzdc_alert(self.lang('uploadSuccess'));
						} else {
							jzdc_alert(data.message);
						}
					},
				...
				click : function(e) {
					...
					if (showLocal && showRemote && tabs && tabs.selectedIndex === 1 || !showRemote) {
						if (uploadbutton.fileBox.val() == '') {
							jzdc_alert(self.lang('pleaseSelectFile'));
							return;
						}
						dialog.showLoading(self.lang('uploadLoading'));
						uploadbutton.submit();
...
KindEditor.plugin('map', function(K) {
	...
	self.clickToolbar(name, function() {
		...
		var iframeHtml = ['<!doctype html><html><head>',
			...
			'			});',
			'		} else {',
			'			jzdc_alert("Invalid address: " + address);',
			'		}',
			'	});',
			'}',
			'</script>',
...
SWFUpload.Console.writeLine = function (message) {
	...
	try {
		...
		jzdc_alert("Exception: " + ex.name + " Message: " + ex.message);
	}
};
				...
				yesBtn : {
					...
					click : function(e) {
						...
						if (rows == 0 || !/^\d+$/.test(rows)) {
							jzdc_alert(self.lang('invalidRows'));
							rowsBox[0].focus();
							return;
						}
						...
						if (!/^\d*$/.test(padding)) {
							jzdc_alert(self.lang('invalidPadding'));
							paddingBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(spacing)) {
							jzdc_alert(self.lang('invalidSpacing'));
							spacingBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(border)) {
							jzdc_alert(self.lang('invalidBorder'));
							borderBox[0].focus();
							return;
						}
						//modify table
