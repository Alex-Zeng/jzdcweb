...
//翻页（PHP代码/函数）	
function LansionDigitPage($sum,$current_page,$page_size,$touch_id='',$page_template='当前：第 <span class=set_page></span> 页，共{max_page}页,{sum}条数据'){
	//$sum=1000;
	...
	$list="<div class='dataTables_paginate paging_simple_numbers'><ul class='pagination' id='LansionDigitPage' user_color='page'>{$page_list}</ul></div>";
	$touch_event='';
	if($touch_id!=''){
		$touch_event="		
		if(touchAble){
			//$('".$touch_id."').attr('ontouchstart','set_touch_start(event)');
			//$('".$touch_id."').attr('ontouchmove','exe_touch_move(event,\"jzdc_page_go\")');
		}
";	
	}
...
$(document).ready(function(){
	$('#LansionDigitPage #page_".$current_page."').parent().addClass('active');
	...
	$('#page_size').change(function(){
		...
		$(document).keydown(function(event){
			if(event.keyCode==37 && event.target.tagName!='INPUT' && event.target.tagName!='TEXTAREA'){jzdc_page_go('left');}			  	
			if(event.keyCode==39 && event.target.tagName!='INPUT' && event.target.tagName!='TEXTAREA'){jzdc_page_go('right');}			  	
		});		
...
//获取用户组选项option Lansion专用（PHP代码/函数）	）
...
//获取jzdc_table.css路径 Lansion专用（PHP代码/函数）
function get_jzdc_table_css_dir($path){
	$t=str_replace(DIRECTORY_SEPARATOR,"/",$path);
	//echo $t;
	$t=explode("templates/",$t);
	$t=explode("/",$t[1]);
	...
	for($i=0;$i<count($t)-1;$i++){$dir.=$t[$i]."/";}
	$dir="./templates/".$dir;
	if(file_exists($dir.'jzdc_table.css')){
		return $dir;	
	}else{
...
//获取jzdc_table.js 路径 Lansion专用（PHP代码/函数）
function get_jzdc_table_js_dir($path){
	$t=str_replace(DIRECTORY_SEPARATOR,"/",$path);
	//echo $t;
	$t=explode("templates/",$t);
	$t=explode("/",$t[1]);
	...
	for($i=0;$i<count($t)-1;$i++){$dir.=$t[$i]."/";}
	$dir="./templates/".$dir;
	if(file_exists($dir.'jzdc_table.js')){
		return $dir;	
	}else{
		$dir='./templates/index/'.$t[count($t)-3].'/'.$t[count($t)-2];
		if(file_exists($dir.'/jzdc_table.js')){
...
//获取用户所在用户组名称 Lansion专用（PHP代码/函数）
...
//格式化模板调用参数 返回数组 Lansion专用（PHP代码/函数）
...
//格式化字段为checkbox Lansion专用（PHP代码/函数）
...
function safe_str($str,$replace_script=true,$allow_html=true){
	...
	if(!is_array($str)){
		...
		$str=str_replace("--",'- - ',$str);
		$str=str_replace("\*",'\-*',$str);
		$str=str_replace("\\",'jzdc_backslash',$str);
		if($replace_script){
			$str = preg_replace("/<script/iUs", "<jzdc_script", $str); 
			$str = preg_replace("/script>/iUs", "jzdc_script>", $str); 
		}
...
//获取地区选项option Lansion专用（PHP代码/函数）
...
function send_sms($config,$language,$pdo,$id=0){
	...
	if($r['addressee']!='' && $r['content']!=''){
		...
		if($send_result){
			 ...
			 $content='http://'.$config['web']['domain'].'/index.php?jzdc=index.config#sms';
			 ...
			 email($config,$language,$pdo,'jzdc',$config['sms']['fail_email'],$title,$content);
...
//获取邮件发件箱SMTP信息 Lansion专用（PHP代码/函数）
...
//调用发送邮件 Lansion专用（PHP代码/函数）
function send_email($config,$pdo,$id=0, $attach){
	$id=intval($id);
	if($id==0){
	...
	if($r['addressee']!='' && $r['content']!=''){
			...
			require_once($path); 
			$result=sendmail($config['web']['name'],$r['addressee'],$r['title'],$r['content'],get_mail_info($pdo,$r['addressee']), $attach);
			@ob_clean(); 
			@ob_end_flush(); 
...
//发送邮件 Lansion专用（PHP代码/函数）
function email($config,$language,$pdo,$sender,$addressee,$title,$content,$attach){
	...
	if($pdo->exec($sql)){
		return  send_email($config,$pdo,$pdo->lastInsertId(), $attach);
	}else{
		return false;
	}
...
//获取内容发布时间与当前的时差 Lansion专用（PHP代码/函数）
...
function operator_money($config,$language,$pdo,$username,$money,$reason,$program,$negative=false){
	...
	if($after_money<0 && $negative==false){
		$money=str_replace('-','',$money);
		$_POST['operator_money_err_info']='<a href=./index.php?jzdc=index.recharge&money='.$money.'&return_url='.urlencode(str_replace('&','|||',@$_SERVER['HTTP_REFERER'])).' id=please_recharge>'.$language['please'].$language['recharge'].'</a>';	
		return false;
	}
	$sql="update ".$pdo->index_pre."user set `money`=`money`+$money where `username`='$username'";
	//file_put_contents('./test.txt',$sql);
	if($pdo->exec($sql)){
		$sql="insert into ".$pdo->index_pre."money_log (`time`,`username`,`money`,`reason`,`operator`,`program`,`before_money`,`after_money`,`ip`) values ('".time()."','$username','$money','$reason','".@$_SESSION['jzdc']['username']."','$program','".$r['money']."','$after_money','".get_ip()."')";
...
//更新用户充值状态 Lansion专用（PHP代码/函数）
function update_recharge($pdo,$config,$language,$id,$money=''){
	$id=floatval($id);
	$sql="select `state`,`money` from ".$pdo->index_pre."recharge where `in_id`='$id'";
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['state']==4){return true;}
	if($money!='' && $money<$r['money']){return true;}
...
function call_recharge_return_function($config,$language,$pdo,$name,$for_id,$in_id){
	...
	//file_put_contents('u.txt',$path);
	if(!is_file($path)){return false;}
	file_put_contents('u.txt',$fun[0].'_'.$fun[1]);
...
//获取jzdc所在目录 Lansion专用（PHP代码/函数）
function get_jzdc_path(){
	$path=get_url();
...
//检测Lansion子程序使用权限 Lansion专用（PHP代码/函数）
...
//检测Lansion子程序安装包的完整性 Lansion专用（PHP代码/函数）
...
//检测Lansion子程序升级包的完整性 Lansion专用（PHP代码/函数）
...
//跳转到404页面 Lansion专用（PHP代码/函数）
function not_find(){
	header('location:index.php?jzdc=index.not_found');exit;
	echo('404');	
}

//更新导航条静态数据 Lansion专用（PHP代码/函数） stop
...
//记录用户的访问设备 Lansion专用（PHP代码/函数）
function set_device(){
	$device_array=array('pc','phone');
	//if(!isset($_COOKIE['jzdc_device'])){$_COOKIE['jzdc_device']=$device_array[0];return true;}
	
	if(isset($_GET['jzdc_device']) && in_array(@$_GET['jzdc_device'],$device_array)){	
		$_COOKIE['jzdc_device']=$_GET['jzdc_device'];
		//echo $_COOKIE['jzdc_device'].'=jzdc_device';
		return true;	
	}
	if(!in_array(@$_COOKIE['jzdc_device'],$device_array)){
		$_COOKIE['jzdc_device']=$device_array[0];
		return true;
	}
	if(isset($_COOKIE['jzdc_device'])){return true;}
	
	$pc=array('Windows','Macintosh');
	$phone=array('Android','iPhone','iPod','BlackBerry','Windows Phone');
	foreach($pc as $v){
		if(strpos($_SERVER['HTTP_USER_AGENT'],$v)!==false){$_COOKIE['jzdc_device']='pc';break;}	
	}
	foreach($phone as $v){
		if(strpos($_SERVER['HTTP_USER_AGENT'],$v)!==false){$_COOKIE['jzdc_device']='phone';break;}	
	}
...
//删除jzdc子程序的相关数据表 Lansion专用（PHP代码/函数）
...
//导出mysql数据表 Lansion专用（PHP代码/函数）
function output_table($pdo,$save_dir,$program='',$flag=1){
	set_time_limit(0);
    $mysql='jzdc_sql_start';
	if($program!=''){
    ...
    foreach ($statments as $value) {
	  ...
	  if($mysql!=''){
		  $file_count++;
		  file_put_contents($save_path.$file_count.'.sql',$mysql.'jzdc_sql_end');
		  //echo $save_path.$file_count.'.sql'.'<br/>';
		  $mysql='';
		  $files[]=$save_path.$file_count.'.sql';
	  }elseif($file_count>0){
		  //echo $save_path.$file_count.'.sql<hr>';
		  file_put_contents($save_path.$file_count.'.sql',file_get_contents($save_path.$file_count.'.sql').'jzdc_sql_end');
	  }
	  $_POST['output_table']=$files;
     return  true;
}

//获取本批数据备份文件名 Lansion专用（PHP代码/函数）
   ...
   if (file_exists($path)) {
	   ...
	   if(is_numeric($i) && $auto){
		  ...
		  // var_dump( $series);
		   ...
		   if(strpos(file_get_contents($series[0]),'jzdc_sql_start')===false){$_POST['err_info']='lack_series_file';return false;}
		   if(strpos(file_get_contents($series[count($series)-1]),'jzdc_sql_end')===false){$_POST['err_info']='lack_series_file';return false;}
		   ...
		   $sql_stream=trim($sql_stream,'jzdc_sql_start');
		   $sql_stream=trim($sql_stream,'jzdc_sql_end');
		   $sql_array=explode(";m;o;n;\n\r",$sql_stream);
		   array_filter($sql_array);
		  if(count($sql_array)==0){$_POST['err_info']='the_sql_is_not_for_Lansion';return false;}
...
//启用用户的个人设置 Lansion专用（PHP代码/函数）
function send_user_set_cookie($pdo){
	$sql="select * from ".$pdo->index_pre."user_set_item";
	$r=$pdo->query($sql,2);
	foreach($r as $v){
		$sql2="select `item_value` from ".$pdo->index_pre."user_set where `user_id`='".@$_SESSION['jzdc']['id']."' and `item_variable`='".$v['variable']."'";
		$r2=$pdo->query($sql2,2)->fetch(2);
		if($r2['item_value']==''){
...
//浏览器直接查看数组 （PHP代码/函数）		
function jzdc_var_dump($array){
	echo '<pre>';
	var_dump($array);
	echo '</pre>';	
}				

//更新同步程序的page.php文件 Lansion专用（PHP代码/函数）
...
function oauth_login($pdo,$language,$config){
	...
	if($v['state']!=1){
        ...
        setcookie("jzdc_group_id",$v['group_id'],0,'/');
		setcookie("jzdc_id",$v['userid'],0,'/');
		setcookie("jzdc_nickname",$v['nickname'],0,'/');
		setcookie("jzdc_icon",$_SESSION['jzdc']['icon'],0,'/');
		...
		if($v['c']<$config['other']['user_login_log']){
			$sql="insert into ".$pdo->index_pre."user_login (`userid`,`ip`,`time`) values ('".$_SESSION['jzdc']['id']."','$ip','$time')";
		}else{
...
function get_ip_position($ip,$ak=''){
	if($ak==''){
		return file_get_contents('http://www.lansion.cn/get_position.php?ip='.$ip);	
	}else{
...
function operation_credits($pdo,$config,$language,$username,$number,$reason,$type='other'){
	if($number<0){
		...
		$r=$pdo->query($sql,2)->fetch(2);
		if(($r['credits']+$number)<0){return false;}
	}
...
function set_weixin_info($wid,$pdo){
	...
	$_POST['jzdc_weixin'][$wid]=array();
	$_POST['jzdc_weixin'][$wid]['AppId']=$r['AppId'];
	$_POST['jzdc_weixin'][$wid]['token']=$token['access_token'];
	$_POST['jzdc_weixin'][$wid]['ticket']=$ticket['ticket'];
	$_POST['jzdc_weixin'][$wid]['expires_in']=time()-60;
	$r=json_encode($_POST['jzdc_weixin'][$wid]);
	$r=safe_str($r);
	$sql="update ".$pdo->sys_pre."weixin_account set `weixin_token`='".$r."' where `wid`='".$wid."' limit 1";
	$pdo->exec($sql);
	//echo $sql;
	$_POST['jzdc_weixin'][$wid]['new']=date('Y-m-d H:i',time());
	return true;	
}

function get_weixin_info($wid,$pdo){
	if(isset($_POST['jzdc_weixin'][$wid]['expires_in'])){if(time()<$_POST['jzdc_weixin'][$wid]['expires_in']+3600){return true;}}
	...
	foreach($r as $k=>$v){
		if($v==''){continue;}
		$_POST['jzdc_weixin'][$wid][$k]=$v;	
	}
	//var_dump(date('Y-m-d H:i',$_POST['jzdc_weixin'][$wid]['expires_in']));
	
	if(time()<$_POST['jzdc_weixin'][$wid]['expires_in']+3600){return true;}else{return  set_weixin_info($wid,$pdo); }
		
}

function get_weixin_js_config($wid,$pdo){
	get_weixin_info($wid,$pdo);
	if(!isset($_POST['jzdc_weixin'][$wid]['ticket'])){return false;}
	$_POST['weixin_js_config']['appId'] =$_POST['jzdc_weixin'][$wid]['AppId'];
	$_POST['weixin_js_config']['timestamp'] = time();
	$_POST['weixin_js_config']['nonceStr'] = get_random_str(6);
	$_POST['weixin_js_config']['signature']=sha1(sprintf("jsapi_ticket=%s&noncestr=%s&timestamp=%s&url=%s",$_POST['jzdc_weixin'][$wid]['ticket'], $_POST['weixin_js_config']['nonceStr'], $_POST['weixin_js_config']['timestamp'],'http://'.get_url()));
...
function notice_app($pdo,$wid,$data){
	get_weixin_info($wid,$pdo);
	if(!isset($_POST['jzdc_weixin'][$wid]['token'])){return false;}
	$r= https_post('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$_POST['jzdc_weixin'][$wid]['token'],$data);
...
function get_weixin_template_id($pdo,$wid,$id){
	...
	$r= https_post('https://api.weixin.qq.com/cgi-bin/template/api_add_template?access_token='.$_POST['jzdc_weixin'][$wid]['token'],$data);
	var_dump($r);
	$r=json_decode($r,1);
...
function other_push_email($pdo,$config,$language,$email,$data){
	...
	if(email($config,$language,$pdo,'jzdc',$email,$data['title'],$data['content'])){
		return true;
	}else{
		return false;
	}
...
function authcode_push_sms($pdo,$config,$language,$phone,$code){
	...
	if(sms($config,$language,$pdo,'jzdc',$phone,$code)){
...
function authcode_push_email($pdo,$config,$language,$email,$code){
	...
	if(email($config,$language,$pdo,'jzdc',$email,$title,$content)){
...
function push_credits($pdo,$config,$language,$username,$type,$money,$balance){
	...
	$notice['url']='http://'.$config['web']['domain'].'/index.php?jzdc=index.credits_log';	
	$notice['topcolor']='#FF0000';	
	$notice['data']=array();	
	$notice['data']['first']=array();	
	$notice['data']['first']['value']=str_replace('{webname}',$config['web']['name'],$language['verification_code_title']);	
...
function push_money_add_info($pdo,$config,$language,$username,$reason,$money,$balance){
	...
	$notice['url']='http://'.$config['web']['domain'].'/index.php?jzdc=index.money_log';	
	$notice['topcolor']='#FF0000';	
	$notice['data']=array();	
	$notice['data']['first']=array();	
	$notice['data']['first']['value']=str_replace('{webname}',$config['web']['name'],$language['verification_code_title']).' '.$username;	
...
function push_new_order_info($pdo,$config,$language,$username,$title,$shop_name,$goods_name,$money,$state,$id){
	...
	$notice['url']='http://'.$config['web']['domain'].'/index.php?jzdc=mall.order_admin&search='.$id;	
	$notice['topcolor']='#FF0000';	
	$notice['data']=array();	
	$notice['data']['first']=array();	
...
function push_cancel_order_info($pdo,$config,$language,$username,$title,$id,$money,$page,$goods_name){
	...
	$notice['url']='http://'.$config['web']['domain'].'/index.php?jzdc='.$page.'&search='.$id;	
	$notice['topcolor']='#FF0000';	
	$notice['data']=array();	
	$notice['data']['first']=array();	
	$notice['data']['first']['value']=$title;	
...
function push_order_agree_refund_info($pdo,$config,$language,$username,$title,$goods_name,$money,$id){
	...
	$notice['url']='http://'.$config['web']['domain'].'/index.php?jzdc=mall.my_order&search='.$id;	
	$notice['topcolor']='#FF0000';	
	$notice['data']=array();	
	$notice['data']['first']=array();	
	$notice['data']['first']['value']=$title;	
...
function push_login_info($pdo,$config,$language,$username){
	...
	$notice['url']='http://'.$config['web']['domain'].'/index.php?jzdc=index.edit_user&field=password';	
	$notice['topcolor']='#FF0000';	
	$notice['data']=array();	
	$notice['data']['first']=array();	
...
function color_to_css($color){
	$css="<style>
		...
		[user_color='container'] [jzdc-module]{ border-color:".$color['module']['border']."; background:".$color['module']['background']."; color:".$color['module']['text'].";}
		[user_color='container'] [jzdc-module] a{ color:".$color['module']['text'].";}
		[user_color='container'] [jzdc-module] input,[user_color='container'] [jzdc-module] select,[user_color='container'] [jzdc-module] textarea{background:".$color['module']['background']."; color:".$color['module']['text'].";}
...
"text":
{
	...
	$url = "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=".$_POST['jzdc_weixin'][$wid]['token'];
	$r=https_post($url,$v);
	$r=json_decode($r,1);
...
function safe_order_by($v){
  ...
  function t_browser($v){
	...
	function wexin_red_pack($pdo,$username,$sender,$wishing,$money,$program){
		//wexin_red_pack($pdo,$_SESSION['jzdc']['username'],$sender,$wishing,$money,'prize')
		...
		$sql="insert into ".$pdo->index_pre."wxpay (`time`,`username`,`openid`,`money`,`reason`,`operator`,`program`,`ip`,`send_state`,`receive_state`,`type`,`wxid`) values ('".time()."','".$username."','".$user['openid']."','".$money."','".$sender.','.$wishing."','".@$_SESSION['jzdc']['username']."','".$program."','".get_ip()."','0','0','0','".$wxid."')";
	...
	function wexin_transfers($pdo,$username,$sender,$money,$program){
		//$r=wexin_transfers($pdo,$_SESSION['jzdc']['username'],self::$config['web']['name'].' '.self::$language['withdraw'],1,'index');
		//if(!$r){echo($_POST['err_code_des']);}else{}
		$sql="select `openid` from ".$pdo->index_pre."user where `username`='".$username."' limit 0,1";
		$user=$pdo->query($sql,2)->fetch(2);
		if($user['openid']==''){$_POST['err_code_des']='openid is null';return false;}
		...
		$sql="insert into ".$pdo->index_pre."wxpay (`time`,`username`,`openid`,`money`,`reason`,`operator`,`program`,`ip`,`send_state`,`receive_state`,`type`,`wxid`) values ('".time()."','".$username."','".$user['openid']."','".$money."','".$sender."','".@$_SESSION['jzdc']['username']."','".$program."','".get_ip()."','0','0','1','".$wxid."')";
		if(!$pdo->exec($sql)){$_POST['err_code_des']='insert redpack err';return false;}
		$log_id=$pdo->lastInsertId();
		$config=require('./payment/weixin/config.php');
		if($config['state']!='opening'){$_POST['err_code_des']='weixin pay not is opening';return false;}
...
function set_reg_introducer($pdo,$config,$language){
	if(!isset($_SESSION['jzdc']['id'])){return false;}
	if(!isset($_SESSION['jzdc']['username'])){return false;}
	if(@$_SESSION['jzdc']['introducer']!=''){return false;}
	if(isset($_COOKIE['store_id']) && $_COOKIE['store_id']!=''){
		...
		$r=$pdo->query($sql,2)->fetch(2);
		if($r['username']!=$_SESSION['jzdc']['username']){$introducer=$r['username'];}
	}
	...
	if(isset($introducer)){
		...
		$r=$pdo->query($sql,2)->fetch(2);
		$sql="select `reg_time`,`username`,`introducer` from ".$pdo->index_pre."user where `id`=".$_SESSION['jzdc']['id'];
		$r2=$pdo->query($sql,2)->fetch(2);
		if($r2['introducer']!=''){return false;}
		if($r2['reg_time']>$r['reg_time']){
			$sql="update ".$pdo->index_pre."user set `introducer`='".$introducer."' where `id`=".$_SESSION['jzdc']['id'];
			$pdo->exec($sql);
...
function wx_diy_qr_crate_user($pdo,$language,$openid,$wid,$introducer=''){
	...
	$config=require('./config.php');
	$sql="insert into ".$pdo->index_pre."user (`icon`,`nickname`,`username`,`password`,`reg_time`,`reg_ip`,`group`,`state`,`manager`,`gender`,`reg_oauth`,`openid`) values ('".$userinfo['headimgurl']."','".$userinfo['nickname']."','".$userinfo['nickname']."','".md5(time().rand())."','".time()."','".get_ip()."','".$config['reg_set']['default_group_id']."',1,0,'".$userinfo['sex']."',1,'".$userinfo['openid']."')";
	if($pdo->exec($sql)){
	...
	if($pdo->exec($sql)){
		...
		//$agency->update_store_qr_path($pdo,$s_id);
				if($config['web']['wid']!=''){
					...
					$r= https_post('https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token='.$_POST['jzdc_weixin'][$config['web']['wid']]['token'],$data);
					$r=json_decode($r,1);
					if(isset($r['url'])){
		...
		$r=$pdo->query($sql,2);
		foreach($r as $v){
			$sql="insert into ".$pdo->sys_pre."store_goods (`goods_id`,`shop_id`,`store_id`,`username`,`time`) values ('".$v['goods_id']."','".$v['shop_id']."','".$module['id']."','".$_SESSION['jzdc']['username']."','".$time."')";
			$pdo->exec($sql);
		}
	}
}
...
function update_user_last($pdo){
	if(!isset($_SESSION['jzdc']['username'])){return false;}
	$sql="update ".$pdo->index_pre."user set `last`=".time()." where `username`='".$_SESSION['jzdc']['username']."' limit 1";
	$pdo->exec($sql);
}
	...
	if($r['id']!=''){
		...
		if($pdo->exec($sql)){
			if(!is_online($pdo,$receiver)){
				$url='http://'.$config['web']['domain'].'/index.php?jzdc=im.talk&with='.$sender;
...
function jzdc_number_difference($a,$b,$difference){
	$v=$a-$b;
	$v=abs($v);
