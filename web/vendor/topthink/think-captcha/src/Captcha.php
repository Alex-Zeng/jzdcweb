...
class Captcha
{
    ...
    public function entryDb($id = '')
    {
        ...
        $model = new \app\common\model\Captcha();
        $row = $model->where(['key'=>$key.$id])->find();
        if($row){
            $model->save(['verify_code'=>$code,'verify_time'=>time()],['key'=>$key.$id]);
        }else{
            $model->save(['key'=>$key.$id,'verify_code'=>$code,'verify_time'=>time()]);
        }
    ...
    public function checkDb($code, $id = '')
    {
        $key = $this->authcode($this->seKey) . $id;
        // 验证码不能为空
       // $secode = Session::get($key, '');
        $model = new \app\common\model\Captcha();
        $secode = $model->where(['key'=>$key])->find();
