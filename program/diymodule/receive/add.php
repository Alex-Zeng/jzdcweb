<?php
		...
		if($_POST['title']!='' && $_POST['content']!=''){
			$sql="insert into ".self::$table_pre."module (`title`,`width`,`height`,`content`,`editor`,`time`,`title_visible`) values ('".$_POST['title']."','".$_POST['width']."','".$_POST['height']."','".$_POST['content']."','".$_SESSION['jzdc']['id']."','$time','".$_POST['title_visible']."')";	
			//echo $sql;
			if($pdo->exec($sql)){
				...
				exit("{'state':'success','info':'<span class=success>&nbsp;</span><script>window.location.href=\"index.php?jzdc=".self::$config['class_name'].".admin\";</script>'}");
			}else{
