...
if($act=='update'){
	...
	//if(!in_array('0',$table_edit_power)){
		if(!in_array($_SESSION['jzdc']['group_id'],$table_edit_power)){exit("{'state':'fail','info':'<span class=fail>".self::$language['without'].self::$language['edit'].self::$language['power']."</span>'}");}	
	//}
	...
	foreach($r as $v){
        ...
        if ($v['name'] == 'status') {  //改状态
            $fields .= "`status`='1', refuse_reason='',";
        }else  if ($v['name'] == 'refuse_reason'){  //改拒绝理由
	...
	$values=trim($values,',');
	$edit_time=time();
	$editor=(isset($_SESSION['jzdc']['id']))?$_SESSION['jzdc']['id']:0;
	...
	if($pdo->exec($sql)){
        if($table_id==37){
            $sql="select `phone` from ".$pdo->index_pre."user where `id`='".$editor."'";
            $r2=$pdo->query($sql,2)->fetch(2);
            if ($r2){
                $msg = send_notice_sms(self::$config, $pdo, $r2["phone"], 'cert_submit');
            }
        }
		...
		exit("{'state':'success','info':'<span class=success>".self::$language['submit'].self::$language['success']." ".$view.' <a href="./index.php?jzdc=form.data_show_detail&table_id='.$table_id.'&id='.$id.'" class="edit">'.self::$language['view'].'</a>'."</span>'}");
