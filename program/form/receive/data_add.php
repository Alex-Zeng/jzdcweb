...
if($act=='add'){
	...
	if(!in_array('0',$table_add_power)){
		if(!isset($_SESSION['jzdc']['group_id'])){$_SESSION['jzdc']['group_id']='0';}
		if(!in_array($_SESSION['jzdc']['group_id'],$table_add_power)){exit("{'state':'fail','info':'<span class=fail>".self::$language['without'].self::$language['add'].self::$language['power']."</span>'}");}	
	}
	...
	$values=trim($values,',');
	$write_time=time();
	$writer=(isset($_SESSION['jzdc']['id']))?$_SESSION['jzdc']['id']:0;
	
	$sql="insert into ".self::$table_pre.$table_name." (".$fields.",`write_time`,`writer`,`publish`) values (".$values.",'".$write_time."','".$writer."','0')";
	...
	if($pdo->exec($sql)){
		...
		/*if($inform_user!='' && ($sms_inform==1 || $email_inform==1)){
			...
			if($default_publish && $read_state){
				$url='http://'.self::$config['web']['domain'].'/index.php?jzdc=form.data_show_detail&table_id='.$table_id.'&id='.$id;
			}else{
				$url='http://'.self::$config['web']['domain'].'/index.php?jzdc=form.data_edit&table_id='.$table_id.'&id='.$id;
			}
			...
			/*if($r2['email']!='' && $email_inform==1){
				...
				email(self::$config,self::$language,$pdo,'jzdc',$r2['email'],$e_title,$e_content);	
			}
			if($r2['phone']!='' && $sms_inform==1){
				//$e_title=$_SERVER['HTTP_HOST'].'('.self::$config['web']['name'].') '.$table_description.' '.self::$language['receive_new_information'];
				//$content=$e_title.' '.self::$language['view'].self::$language['url'].': '.$url;
				//sms(self::$config,self::$language,$pdo,'jzdc',$r2['phone'],$content);
			}
		}*/
        if($table_id==37){
            $sql="select `phone` from ".$pdo->index_pre."user where `id`='".$writer."'";
            $r2=$pdo->query($sql,2)->fetch(2);
            if ($r2){
                send_notice_sms(self::$config, $pdo, $r2["phone"], 'cert_submit');
            }
        }
        
		if($default_publish){$view='<a href="./index.php?jzdc=form.data_show_detail&table_id='.$table_id.'&id='.$id.'" class="view">'.self::$language['view'].'</a>';}else{$view='';}
        //exit("{'state':'success','info':'<span class=success>".self::$language['submit'].self::$language['success']." ".$view.' <a href="./index.php?jzdc=form.data_add&table_id='.$table_id.'" class="return_button">'.self::$language['return'].'</a>'."</span>'}");
        exit("{'state':'success','info':'<span class=success>".self::$language['submit'].self::$language['success']." ".$view."</span>'}");
	}else{
