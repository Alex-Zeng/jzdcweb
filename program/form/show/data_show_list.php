...
$module['head_field']='';
$module['body_field']='';
$module['search_placeholder']=self::$language['writer'].' ';
...
foreach($r as $v){
	...
	if($v['input_type']=='select' || $v['input_type']=='radio' || $v['input_type']=='checkbox'){
		...
		foreach($temp as $vv){
			...
			$options.='<option value="'.$vv.'" '.$selected.'>'.$label.'</option>';
		}
...
if($_GET['search']!=''){
	$where=" and (`username` like '%".$_GET['search']."%' or `editor` like '%".$_GET['search']."%' or ".$search_able.")";
}
if(@$_GET['order']==''){
...
if ($v["status"] == 1){
	...
	$list.="<tr id='tr_".$v['id']."'>
	".$filed_data."
  <td class=operation_td>".$review."</td>
</tr>
";	
}
