...
$sql="select `money`,`real_name`,`openid` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."'";
$r=$pdo->query($sql,2)->fetch(2);
$module['user_money']=$r['money'];
$module['real_name']=$r['real_name'];

if($module['real_name']==''){header('location:./index.php?jzdc=index.edit_user&field=real_name');exit;}
...
$sql="select `billing_info` from ".$pdo->index_pre."withdraw where `username`='".$_SESSION['jzdc']['username']."' order by `id` desc limit 0,1";
$r=$pdo->query($sql,2)->fetch(2);
