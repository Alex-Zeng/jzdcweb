...
$sql="select * from ".$pdo->index_pre."share where `user_id`=".$_SESSION['jzdc']['id'];
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	...
	//get sum visit reg 
	if($where==''){
		$sum_sql="select count(id) as c from ".$pdo->index_pre."share_visit where `user_id`=".$_SESSION['jzdc']['id'];
		$r=$pdo->query($sum_sql,2)->fetch(2);
		$visit=$r['c'];
		$sum_sql="select count(id) as c from ".$pdo->index_pre."user where `introducer`='".$_SESSION['jzdc']['username']."'";
		$r=$pdo->query($sum_sql,2)->fetch(2);
		$reg=$r['c'];
...
foreach($r as $v){
	...
	$list.="<tr id='tr_".$v['id']."'>
	<td><a href='./index.php?jzdc=".$v['url']."' target=_blank class=title>".$v['title']."</a></td>
	<td><a href='./index.php?jzdc=index.my_share_visit&id=".$v['id']."' target=_blank class=visit>".$visit."</a></td>
	<td><a href='./index.php?jzdc=index.my_share_reg&id=".$v['id']."' target=_blank class=reg>".$reg."</a></td>
