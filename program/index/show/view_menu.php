<?php
		...
		$module['groups']=index::get_groups($pdo,0,"index.php?jzdc=index.view_menu&program=$program&id=");
		...
		if($id==0){
			...
			header("location:index.php?jzdc=index.view_menu&id=".$r['id']);	
		}
		...
		foreach($stmt as $v){
			...
			$module['list'].="<li><a href=index.php?jzdc={$v['url']}  target=_blank><img src={$v['icon_path']}><br/><span>".$language['pages'][$v['url']]['name']."</span></a><div><input type='text' id='sequence_".$v['id']."' name='sequence_".$v['id']."' value='".$v['sequence']."' class='menu_sequence' /></div></li>";
		}
