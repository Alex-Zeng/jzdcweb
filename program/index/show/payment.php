...
if($payment==''){
	...
	foreach($r as $v){
		if(is_dir($dir.$v) && $v!='.' && $v!='..'){
			...
			$online.="<a href='./index.php?jzdc=index.payment&payment=".$v."'><img src='".$dir.$v."/".$config['provider_icon']."' alt='".$config['provider_name']."' />".$config['provider_name']."</a>";
		}
	}
...
if($payment!=''){
	...
	$online="<div align='center'><img src='./payment/".$payment."/".$config['provider_icon']."' alt='".$config['provider_name']."' title='".$config['provider_name']."'> <a href='./index.php?jzdc=index.payment'>".self::$language['return']."</a></div><iframe src='./payment/".$payment."/config_panel.php' width='100%' height='800' scrolling='no'  frameborder='no'></iframe>";	
}
