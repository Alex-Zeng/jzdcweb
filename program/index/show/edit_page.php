<?php
		...
		$module['groups']=index::get_groups($pdo,0,"index.php?jzdc=index.edit_page&program=".$program."&id=");
		$id=intval(@$_GET['id']);
		if($id==0){
			$sql="select `id` from ".$pdo->index_pre."group where `parent`=0 order by `sequence` desc limit 0,1";
			$r=$pdo->query($sql,2)->fetch(2);
			header("location:index.php?jzdc=index.edit_page&id=".$r['id']);	
		}
		...
		foreach($stmt as $v){
			...
			foreach($pages as $key=>$v2){
				if($pages[$key]['require_login']==1){
					...
					$module['list'].="<li><input class='checkbox' type='checkbox' id='$temp[0]' name='$temp[0]' url='$temp[0]' $checked /> <a href=index.php?jzdc={$key} target=_blank>".$language['pages'][$key]['name']."</a><br/>".self::$language['suggest'].": ".$language['pages'][$key]['power_suggest']."</li>";
