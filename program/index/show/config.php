		...
		function echo_html($language,$pdo,$key,$v){
			...
			switch ($key){
				...
				case ($key=='language' && $upkey=='web'):
					...
					$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].'</span><span class="content"><select id='.$key2.' name='.$key2.'>'.$option.'</select><span id='.$key2.'_state ></span> <a href="./index.php?jzdc=index.language_list&program=os_language">'.$language['edit'].'</a></span></div>';
					break;
				...
				case ($key=='authcode_push_mode' && $upkey=='web'):
					...
					$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].'</span><span class="content"><select id='.$key2.' name='.$key2.' jzdc_value='.$v.'>'.$option.'</select><span id='.$key2.'_state ></span></span></div>';
					break;
				...
				case ($key=='phone_country' && $upkey=='reg_set'):
					$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].'</span><span class="content"><select id='.$key2.' name='.$key2.' jzdc_value='.$v.'>'.get_phone_country_opton($language['phone_country']).'</select><span id='.$key2.'_state ></span></span></div>';
					break;
					
				case ($key=='check' && $upkey=='reg_set'):
					
					$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].'</span><span class="content"><select id='.$key2.' name='.$key2.' jzdc_value='.$v.'><option value=image>'.$language['config_language']['reg_set']['check_option']['image'].'</option><option value=phone>'.$language['config_language']['reg_set']['check_option']['phone'].'</option><option value=email>'.$language['config_language']['reg_set']['check_option']['email'].'</option><option value=weixin>'.$language['config_language']['reg_set']['check_option']['weixin'].'</option></select><span id='.$key2.'_state ></span></span></div>';
					break;
				...
				case ($key=='apikey' && $upkey=='sms'):
					$div='<div id=div_'.$key2.'><span class="m_label">'.$config_language[$key].'</span><span class="content"><input type="text" id="'.$key2.'" name="'.$key2.'" value=\''.$v.'\' /><span id='.$key2.'_state ></span> <a href=https://www.yunpian.com/entry?method=register&utm_source=jzdc target=_blank>'.$language['create'].'</a> <a href=http://www.lansion.cn/index.php?jzdc=doc.show&key=yunpian target=_blank>'.$language['tutorial'].'</a></span></div>';
					break;
