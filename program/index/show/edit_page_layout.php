<?php
		...
		foreach($r as $v){
			...
			$module['program_list'].='<a href="./index.php?jzdc=index.edit_page_layout&program='.$v['name'].'"><img src=./program/'.$v['name'].'/icon.png /><br />'.$p_language[$v['name']]['program_name'].'</a>';
			}
		}
		...
		if($program!=''){
			...
			if($url!=''){
				...
				echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=index.edit_page_layout">'.$p_language['index']['pages']['index.edit_page_layout']['name'].'</a><a href="./index.php?jzdc=index.edit_page_layout&program='.$program.'">'.$p_language[$program]['program_name'].'</a><span class=text>'.$p_language[$program]['pages'][$url]['name'].'</span></div>';	
			}else{
				echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=index.edit_page_layout">'.$p_language['index']['pages']['index.edit_page_layout']['name'].'</a><span class=text>'.$p_language[$program]['program_name'].'</span></div>';	
			}
			...
			foreach($r as $v){
				...
				<td><a href=index.php?jzdc='.$v['url'].' target=_blank class=url><span class=name>'.@$p_language[$program]['pages'][$v['url']]['name'].'</span><span class=url2>'.$v['url'].'</span></a></td>
			  ...
			  '.get_tutorial(self::$language,$v['tutorial']).'
                ...
                <td class=operation_td><a href="./index.php?jzdc='.$v['url'].'&edit_page_layout=true"  target=_blank class=visual_edit>'.self::$language['visual_edit'].'</a><br class=br_none /><a href=# onclick="return update('.$v['id'].')"  class=submit>'.self::$language['submit'].'</a><br class=br_none />'.$del.'<span id=state_'.$v['id'].' class=state></span></td></tr>
';
			}
