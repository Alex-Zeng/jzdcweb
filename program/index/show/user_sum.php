...
$sql="select `money`,`credits` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."'";
$r=$pdo->query($sql,2)->fetch(2);
$module['user_money']=$r['money'];
$module['credits']=$r['credits'];

$sql="select `id` from ".$pdo->index_pre."user_login where `userid`=".$_SESSION['jzdc']['id']." order by `id` desc limit 0,1";
$r=$pdo->query($sql,2)->fetch(2);
if($r['id']==''){$r['id']=0;}
$sql="select * from ".$pdo->index_pre."user_login where `userid`=".$_SESSION['jzdc']['id']." and `id`<".$r['id']." order by `id` desc limit 0,1";
$r=$pdo->query($sql,2)->fetch(2);
...
$sql="select sum(`money`) as c from ".$pdo->index_pre."recharge where `username`='".$_SESSION['jzdc']['username']."' and `state`=4";
$r=$pdo->query($sql,2)->fetch(2);
if($r['c']==''){$r['c']=0;}
$module['recharge']=$r['c'];

$sql="select sum(`money`) as c from ".$pdo->index_pre."withdraw where `username`='".$_SESSION['jzdc']['username']."' and `state`=3";
$r=$pdo->query($sql,2)->fetch(2);
if($r['c']==''){$r['c']=0;}
$module['withdraw']=$r['c'];
