...
if(@$_GET['group']!=''){
	...
	if($r2['id']==$_SESSION['jzdc']['group_id']){$sql2.=" and `id`='".$_SESSION['jzdc']['id']."'";$manager_list='';}
	//echo $sql2;
	$r2=$pdo->query($sql2,2);
	foreach($r2 as $v){
...
$sql2="select count(id) as c from ".$pdo->index_pre."user where `group`='".$_SESSION['jzdc']['group_id']."'";
$r2=$pdo->query($sql2,2)->fetch(2);
if($r2['c']==1 || $_SESSION['jzdc']['group_id']==1){
	$groups=index::get_group_sub_ids($pdo,$_SESSION['jzdc']['group_id']);	
	$groups=trim($groups,",");
	if($groups==''){$groups=0;}
	$where.=" and `group` in ($groups)";
	//echo $groups;
}else{
	$ids=index::get_user_sub_ids($pdo,$_SESSION['jzdc']['id']);	
	$ids=trim($ids,",");
	if($ids==''){$ids=0;}
	$where.=" and `id` in ($ids)";
	//echo $managers;
...
foreach($r as $v){
	if($v['geolocation']!=''){
		...
		$v['geolocation']='<a class="fa fa-map-marker" href="http://'.self::$config['web']['map_api'].'.lansion.cn/get_point.php?id=position&point='.$temp[1].','.$temp[0].'" target="_blank"></a>';	
	}
	if($v['openid']!=''){	
		$v['openid']='<a href="./index.php?jzdc=weixin.dialog&wid='.self::$config['web']['wid'].'&openid='.$v['openid'].'" target=_blank class=openid></a>';
	}
	...
	if($_COOKIE['jzdc_device']=='pc'){
		...
		<td>".$v['username'].$v['openid']."<br />".$v['real_name']."<br />".$v['email']."<br />".$v['phone']."</td>
		
		<td><a href='index.php?jzdc=index.edit_user_group&id=".$v['id']."' target=_blank>".self::get_group_name($pdo,$v['group'])."</a></td>
		<!--
		<td><a href=./index.php?jzdc=index.credits_admin&search=".urlencode($v['username'])." target=_blank>".$v['cumulative_credits']."</a></td>
		<td><a href=./index.php?jzdc=index.money_log_admin&username=".urlencode($v['username'])." target=_blank>".$v['money']."</a><br />
			<a href=./index.php?jzdc=index.user_recharge&id=".$v['id']." target=_blank>".self::$language['recharge']."</a><br />
			<a href=./index.php?jzdc=index.user_debit&id=".$v['id']." target=_blank>".self::$language['debit']."</a>
		</td>
		-->
		<td>".get_time(self::$config['other']['date_style'],self::$config['other']['timeoffset'],self::$language,$v['reg_time'])." <a href=./index.php?jzdc=index.login_log&id=".$v['id']." target=_blank class=login_num>".$v['login_num']."</a></td>
	...
	<a href='index.php?jzdc=index.admin_edit_user&id=".$v['id']."' class=edit target='_blank'>".self::$language['edit']."</a> 
...
";	
		...
		<td class=u_info>
		<div><span class=u_label>".self::$language['username'].":</span><span class=u_value>".$v['username'].$v['openid']."</span></div>
		<div><span class=u_label>".self::$language['real_name'].":</span><span class=u_value>".$v['real_name']."</span></div>
		<div><span class=u_label>".self::$language['user_group'].":</span><span class=u_value><a href='index.php?jzdc=index.edit_user_group&id=".$v['id']."' target=_blank>".self::get_group_name($pdo,$v['group'])."</a></span></div>
		...
		<!--
		<td><a href=./index.php?jzdc=index.cumulative_credits&search=".urlencode($v['username'])." target=_blank>".$v['cumulative_credits']."</a></td>
		<td><a href=./index.php?jzdc=index.money_log_admin&username=".urlencode($v['username'])." target=_blank>".$v['money']."</a><br /><a href=./index.php?jzdc=index.user_recharge&id=".$v['id']." target=_blank>".self::$language['recharge']."</a><br /><a href=./index.php?jzdc=index.user_debit&id=".$v['id']." target=_blank>".self::$language['debit']."</a></td>
		-->
		<td>".get_time(self::$config['other']['date_style'],self::$config['other']['timeoffset'],self::$language,$v['reg_time'])." <a href=./index.php?jzdc=index.login_log&id=".$v['id']." target=_blank class=login_num>".$v['login_num']."</a></td>
		<td><select id='user_state_".$v['id']."' name='user_state_".$v['id']."'>".get_user_state(self::$language,$v['state'])."</select></td>
		<td class=operation_td>	<a href='#' class=submit onclick='return update(".$v['id'].")'>".self::$language['submit']."</a> 
...
$module['filter'].='<br/><span class="m_label">'.self::$language['user_group'].'</span><span class="option"><select id=group name=group><option value="">'.self::$language['all'].'</option>'.index::get_group_select($pdo,'-1',$_SESSION['jzdc']['group_id']).'</select> &nbsp; &nbsp; &nbsp; '.self::$language['parent'].'<select id=manager name=manager>'.$manager_list.'</select> </span>';
...
$module['placeholder']=self::$language['can'].self::$language['search'].':'.self::$language['username'].' '.self::$language['phone'].' ';
