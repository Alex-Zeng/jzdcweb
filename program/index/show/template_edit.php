...
$module['dir']=get_dir_link($dir,'./index.php?jzdc=index.template_edit&dir=');
$r=scandir($path);
$list='';
...
foreach($r as $v){
	...
	if(is_dir($path.$v)){
		$style='file_icon_dir';
		$href='./index.php?jzdc=index.template_edit&dir='.$dir.$v;
		...
		if($postfix=='php' || $postfix=='txt' || $postfix=='js' || $postfix=='css'){
			$href='./index.php?jzdc=index.template_edit_file&path='.$dir.$v.'&refresh='.time().' target=_blank ';
		}else{
			$href=$path.$v.' target=_blank ';
		}	
		$replace="<a href='./index.php?jzdc=index.template_replace_file&path=".$dir.$v."&refresh=".time()."' target=_blank class='replace'>".self::$language['replace']."</span><span class=b_end></span>&nbsp;</a>";
	}
