...
$page_size=min($page_size,100);
			...
			$sql="select * from ".$pdo->index_pre."money_transfer where `username`='".$_SESSION['jzdc']['username']."'";
	...
	";	
		...
		if($sum==0){$list='<tr><td colspan="30" class=no_related_content_td><span class=no_related_content_span>'.self::$language['no_related_content'].'</span></td></tr>';}
			...
			$sql="select sum(money) as c from ".$pdo->index_pre."money_transfer  where `username`='".$_SESSION['jzdc']['username']."'";
