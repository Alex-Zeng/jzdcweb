...
$module['groups']=index::get_groups($pdo,0,"index.php?jzdc=index.edit_menu&program=".$program."&id=");
$id=intval(@$_GET['id']);
if($id==0){
	$sql="select `id` from ".$pdo->index_pre."group where `parent`=0 order by `sequence` desc limit 0,1";
	$r=$pdo->query($sql,2)->fetch(2);
	header("location:index.php?jzdc=index.edit_menu&id=".$r['id']);	
}
...
foreach($stmt as $v){
	...
	foreach($menu as $key=>$v2){
		...
		$module['list'].="<li><a href=index.php?jzdc={$key} target=_blank><img src={$v['icon_path']}><br/><span>".$language['pages'][$key]['name']."</span></a><div><input class='checkbox' type='checkbox' id='$key' name='$key' $checked /><div><div class=power_suggest>".self::$language['suggest'].":".$language['pages'][$key]['power_suggest']."</div></li>";
