<?php
$sql="select `openid` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."'";
...
if($t['id']==''){echo '<div style="line-height:100px; text-align:center;">'.self::$language['no_web_weixin'].' <a href=./index.php?jzdc=index.config#web__diy_meta><b>'.self::$language['set'].'</b></a></div>';return false;}



$sql="select `url`,`name` from ".$pdo->sys_pre."weixin_diy_qr where `wid`='".self::$config['web']['wid']."' and `key`='get_reg_authcode' limit 0,1";
$r=$pdo->query($sql,2)->fetch(2);
if($r['url']==''){echo '<div style="line-height:100px; text-align:center;">'.self::$language['no_set_weixin_qr'].' <a href=./index.php?jzdc=weixin.diy_qr&wid='.self::$config['web']['wid'].'><b>'.self::$language['set'].'</b></a></div>';return false;}
...
if($r['nickname']!=''){
	...
	//$sql="update ".$pdo->index_pre."user set `openid`='' where `id`='".$_SESSION['jzdc']['id']."'";
	//$pdo->exec($sql);
}
