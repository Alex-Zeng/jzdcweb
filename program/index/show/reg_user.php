<?php
if(isset($_SESSION['jzdc']['username']) && $_SESSION['jzdc']['username']!=''){header('location:./index.php?jzdc=index.user');exit;}
if(@$_GET['refresh']!=''){header('location:./index.php?jzdc=index.user');exit;}
...
if($r['reg_able']==0 && $group_id>0){
	...
	$backurl=isset($_SERVER['HTTP_REFERER'])?$_SERVER['HTTP_REFERER']:'./index.php?jzdc=index.user';
...
$module['module_name']=str_replace("::","_",$method);
	...
	if(self::$config['reg_set']['phone']){
		...
		if($_COOKIE['jzdc_device']=='pc'){
			$module['phone_country_select']='<select class=phone_country id=phone_country name=phone_country jzdc_value='.self::$config['reg_set']['phone_country'].'>'.get_phone_country_opton(self::$language['phone_country']).'</select>';
			$module['ohter_input'].='<div class=line><span class="m_label">'.self::$language['phone'].'：</span><span class=input_span><div class=phone_input_div><span class=phone_country_switch></span><input class="input_text" type="text" name="phone" id="phone" placeholder="该号码将与账号绑定"  maxlength="11" /></span><span id=phone_state></span></div></div>'.$module['phone_country_select'];
		}else{
			//$module['phone_country_select']='<select class=phone_country id=phone_country name=phone_country jzdc_value='.self::$config['reg_set']['phone_country'].'>'.get_phone_country_opton(self::$language['phone_country']).'</select>';
			$module['ohter_input'].='<div class=line><span class="m_label">'.self::$language['phone'].'：</span><span class=input_span><div class=phone_input_div>'.$module['phone_country_select'].'<input class="input_text" type="text" name="phone" id="phone" placeholder="该号码将与账号绑定"  maxlength="11" /></span></div></div><span id=phone_state></span>';
		}
	}
