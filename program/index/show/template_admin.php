...
function get_templates($program,$path,$language,$config){
	...
	foreach($r as $v){
		if($v!='.' && $v!='..' && is_dir($path.$v)){
			...
			$c.="<li id='".$program.'__'.$v."_li'><div class='icon_div'><a href=".$config['server_url']."index.php?jzdc=server.template_detail&path=".$program."/".$v." target=_blank><img src='".$path.$v."/icon.png' /><br><span>".$i['name']."</span></a></div><div class='info_div'>
		...
		<span class='m_label'>".$language['operation'].":</span><span class='info'>".$apply." <a href='./index.php?jzdc=index.template_edit&dir=".$path2.'/'.$v."/' class='edit'>".$language['edit']."</a>".$del." </span><br/>
		</div></li>";	
		}	
	}
...
if($program==''){
	...
	$module['list']="<ul class='nav nav-tabs'><li role='presentation' type=0><a href=./index.php?jzdc=index.template_admin&type=0&program=".@$_GET['program'].">".self::$language['reception'].self::$language['template']."</a></li><li role='presentation' type=1><a href=./index.php?jzdc=index.template_admin&type=1&program=".@$_GET['program'].">".self::$language['backstage'].self::$language['template']."</a></li><li role='presentation'><a id=template_market href='".self::$config['server_url']."index.php?jzdc=server.template_market&url=' target=_blank>".self::$language['templates_market']."</a></li></ul>
