...
$page=(isset($_GET['jzdc']) && @$_GET['jzdc']!='')?$_GET['jzdc']:"index.index";
$t=explode(".",$page);
$menu=require("./program/".$t[0]."/menu.php");
//var_dump($menu);
$position=array();
//$position[]="<a href='index.php'><span id=user_position_icon>&nbsp;</span>".self::$config['web']['name']."</a>";
$position[]="<a href='index.php?jzdc=index.user'>".self::$language['user_center']."</a>";

function user_position_get_name($key){
	if(!in_array($key,$_SESSION['jzdc']['page'])){return false;}
	global $language;
	if(isset($language['pages'][$key]['name'])){return  "<a href='index.php?jzdc=$key'>".$language['pages'][$key]['name'].'</a>';}	
	$temp=explode(".",$key);
