<?php
		...
		$module['groups']=index::get_groups($pdo,0,"index.php?jzdc=index.view_page&program=".$program."&id=");
		...
		if($id==0){
			...
			header("location:index.php?jzdc=index.view_page&id=".$r['id']);	
		}
		...
		foreach($stmt as $v){
			...
			foreach($pages as $key=>$v2){
				if($pages[$key]['require_login']==1){
					...
					$module['list'].="<li><a href=index.php?jzdc={$key} target=_blank>".$language['pages'][$key]['name']."</a></li>";
					}
