...
function echo_html($sys_language,$language,$program,$key,$v){
	...
	switch ($key){
		case ($key=='language'):
			...
			$div='<div id=div_'.$key.'><span class="m_label2">'.$config_language[$key].'</span><span class="content"><select id='.$key.' name='.$key.'>'.$option.'</select><span id='.$key.'_state ></span> <a href="./index.php?jzdc=index.language_list&program='.$program.'">'.$sys_language['edit'].'</a></span></div>';
			break;
		...
		case ($key=='template_0'):
			...
			$div='<div id=div_'.$key.'><span class="m_label2">'.$config_language[$key].'</span><span class="content"><select id='.$key.' name='.$key.'>'.$option.'</select> <a href=./index.php?jzdc=index.template_admin&program='.$program.'>'.$sys_language['edit'].'</a><span id='.$key.'_state ></span></span></div>';
			break;
...
if($program!='' && is_dir("./program/".@$_GET['program']."/")){
	...
	echo ' <div id="user_position_reset" style="display:none;"><a href="index.php"><span id=user_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="index.php?jzdc=index.user">'.self::$language['user_center'].'</a><a href="index.php?jzdc=index.program_config">'.self::$language['program_name'].'</a><span class=text>'.$program_language['program_name'].'</span></div>';	
	...
	foreach($r as $v){
		...
		$module['list'].="<li id='".$v['name']."_li'><div class='icon_div'><a href=".self::$config['server_url']."index.php?jzdc=server.program_detail&program=".$v['name']." target=_blank ><img src='./program/".$v['name']."/icon.png' /><br><span>".$language['program_name']."</span></a></div><div class='info_div'>
		...
		<span class='m_label'>".self::$language['operation'].":</span><span class='info'><a href='index.php?jzdc=index.program_config&program={$v['name']}' class=set>".self::$language['set']."</a><a href='index.php?jzdc=index.program_backup&program={$v['name']}' class=set>".self::$language['backup']."</a><a href='index.php?jzdc=index.program_recovery&program={$v['name']}' class=set>".self::$language['recover']."</a>".$uninstall."</span><br/>
		</div></li>";	
	}
	...
	foreach($r as $v){
		...
		$wait_installation.="<li id='".$v."_li'><div class='icon_div'><a href=".self::$config['server_url']."index.php?jzdc=server.program_detail&program=".$v." target=_blank ><img src='./program/".$v."/icon.png' /><br><span>".$language['program_name']."</a></span><br/>
	...
	$module['list']=$wait_installation."<div id=program_banner><span id=installed_programs>".self::$language['installed_programs']."</span> <a id=program_market href='".self::$config['server_url']."index.php?jzdc=server.program_market&url=' target=_blank>".self::$language['programs_market']."</a></div><ul id=icons>{$module['list']}<div style='clear:both;'></div></ul>";
