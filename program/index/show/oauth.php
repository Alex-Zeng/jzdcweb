...
if($oauth==''){
	...
	foreach($r as $v){
		if(is_dir($dir.$v) && $v!='.' && $v!='..'){
			...
			$online[$config['sequence']]="<a href='./index.php?jzdc=index.oauth&oauth=".$v."'><img src='".$dir.$v."/icon.png' alt='".$config['name']."' />".$config['name']."</a>";
	...
	$online="<div align='center'><img src='./oauth/".$oauth."/icon.png' alt='".$config['name']."' title='".$config['name']."'> <a href='./index.php?jzdc=index.oauth'>".self::$language['return']."</a></div><iframe src='./oauth/".$oauth."/config_panel.php' width='100%' height='800' scrolling='no'  frameborder='no'></iframe>";	
}
