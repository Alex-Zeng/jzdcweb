...
if($act=='up_new'){	
	...
	$r=scandir($path);			
	if(!is_dir($path.'/'.$r[2]) || !is_file($path.'/'.$r[2].'/info.txt')){exit("{'state':'fail','info':'<span class=fail>&nbsp;</span>".self::$language['the_template_is_not_for_Lansion']."'}");}
	$template_info=get_txt_info($path.'/'.$r[2].'/info.txt');
	if($template_info['for']==''){exit("{'state':'fail','info':'<span class=fail>&nbsp;</span>".self::$language['the_template_is_not_for_Lansion']."'}");}
