...
if(@$_GET['act']=='unlogin'){
	...
	setcookie("jzdc_id",'',time()-3600);
	//echo "退出成功！";
	$backurl=isset($_SERVER['HTTP_REFERER'])?$_SERVER['HTTP_REFERER']:'./index.php?jzdc=index.login';
	if(!strpos($backurl,'?')){$backurl.='?refresh='.time();}else{$backurl.='&refresh='.time();}
	//$backurl=isset($_GET['backurl'])?$_GET['backurl']:'index.php?jzdc=index.login';
