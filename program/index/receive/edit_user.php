...
function is_same($pdo,$field,$v){
	$sql="select count(id) as c from ".$pdo->index_pre."user where `$field`='".$v."' and `id`!='".$_SESSION['jzdc']['id']."'";
...
switch ($update) {
	case 'introducer':
		$sql="select `$update` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."'";
		$v=$pdo->query($sql,2)->fetch(2);
		...
		if($v['id']>=$_SESSION['jzdc']['id']){exit("{'state':'fail','info':'<span class=fail>".self::$language['introducer'].self::$language['illegal']."</span>'}");}
		break;
	case 'real_name':
		$sql="select `$update` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."'";
		$v=$pdo->query($sql,2)->fetch(2);
		...
		break;
	case 'manager':
		$sql="select `group` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."'";
	...
	case 'old_password':	
		...
		$sql="select `password` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."'";
		//echo $sql;
		$v=$pdo->query($sql,2)->fetch(2);
	...
	case 'new_password':	
		if(mb_strlen($$update,'utf-8')<6){
			exit("{'state':'fail','info':'<span class=fail>密码应为6~20位字符</span>'}");
		}else{
...
if($update=='icon' || $update=='license_photo_front' || $update=='license_photo_reverse'){
	...
	if($update=='icon'){$_SESSION['jzdc']['icon']="./program/index/user_icon/".$$update;}
	//$$update=trim($$update,"|");
}
$sql="update ".$pdo->index_pre."user set `$update`='".$$update."' where `id`='".$_SESSION['jzdc']['id']."'";
