...
//echo $sql;
		...
		foreach($t1 as $k=>$v){
			if($v!=''){
				...
				preg_match_all('#<li><a href="index\.php\?jzdc=(.*)" target="_self"><img src="\./templates/.*/page_icon/.*\.png"><br><span>(.*)</span></a></li>#iUs',$v,$match);
				$sub='';
			
				foreach($match[0] as $k2=>$v2){
					$sub.='<a href="index.php?jzdc='.$match[1][$k2].'">'.$match[2][$k2].'</a>';
				}
				if($sub==''){
					$composite.='<a href="index.php?jzdc='.$parent_url.'">'.$parent_name.'</a>';
				}else{
					$map.='<div class=line><div class=parent><a href="index.php?jzdc='.$parent_url.'">'.$parent_name.'</a></div><div class=sub>'.$sub.'</div></div>';
				}
			}			
		}
		...
		$sql="select count(id) as c from ".$pdo->index_pre."group_menu where `group_id`=".$_SESSION['jzdc']['group_id'];
		$r=$pdo->query($sql,2)->fetch(2);
		if($sum<$r['c']){exit('submit:'.$sum.'< sum:'.$r['c'].' '.self::$language['fail']);}
		
		$sql="update ".$pdo->index_pre."group set `map`='".safe_str($map)."',`map_update_token`='".$map_update_token."' where `id`=".$_SESSION['jzdc']['group_id'];
		$pdo->exec($sql);
