...
if($act=='add'){
	...
	$reason=self::$language['administrator'].'('.$_SESSION['jzdc']['username'].')'.self::$language['execute'].self::$language['pages']['index.user_recharge']['name'].','.self::$language['reason'].':'.safe_str($_POST['reason']);
	...
	if($pdo->exec($sql)){
		...
		if(operator_money(self::$config,self::$language,$pdo,$username,$money,$reason,'index')){
			exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span> <a href=./index.php?jzdc=index.money_log_admin&username=".$username." target=_blank>".self::$language['view']."</a>'}");
		}else{
