...
if($act=='talk_title_new'){
	...
	foreach($r as $v){
		$v=de_safe_str($v);
		$list.="<tr id=tr_".$v['id']."><td><a href=./index.php?jzdc=talk.content&id=".$v['id']." class=title>".$v['title']."</a></td><td>".$v['username']."</td><td colspan=10></td></tr>";
	}
	$result=array();
...
if($act=='talk_content_new'){
	...
	foreach($r as $v){
		...
		$ids.=$v['id'].',';
		$operation='';
		if($manager==@$_SESSION['jzdc']['username']){
			if($v['visible']){
...
if($act=='load_wx_icon'){
	if(is_url($_SESSION['jzdc']['icon']) && strlen($_SESSION['jzdc']['icon'])>100){
		...
		$img=@file_get_contents($_SESSION['jzdc']['icon']);
		if(!$img || strlen($img)<(1024)){
			$url=str_replace('wx.qlogo.cn','182.254.18.178',$_SESSION['jzdc']['icon']);
		...
		if($img && file_put_contents($path,$img)){
			...
			$sql="update ".$pdo->index_pre."user set `icon`='".$_SESSION['jzdc']['icon']."' where `id`=".$_SESSION['jzdc']['id'];
			$pdo->exec($sql);
			//echo $sql;
		}		
	}
...
if($act=='share'){
	...
	$url=explode('index.php?jzdc=',$_POST['url']);
	if(isset($url[1])){
	...
	$time=time();
	$visit_is_ip=false;
	if(isset($_SESSION['jzdc']['username'])){$visit=$_SESSION['jzdc']['username'];}else{$visit=get_ip();$visit_is_ip=true;}
	...
	if($visit_is_ip){
		...
		if($share==$_SESSION['jzdc']['id']){exit('is your self');}
	...
	$pdo->exec($sql);
	$reason=str_replace('{page_title}','<a href=./index.php?jzdc='.$url.'>'.$title.'</a>',self::$language['share_reson']);
