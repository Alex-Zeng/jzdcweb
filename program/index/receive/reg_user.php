...
if($act=='get_verification_code'){
	...
	switch(self::$config['reg_set']['check']){
		...
		case 'email':
			...
			if(email(self::$config,self::$language,$pdo,'jzdc',$email,$title,$content)){
...
if($act=='check_user'){
    ...
    $_GET['username']=safe_str(@$_POST['username'],1,0);
    ...
    $v=$stmt->fetch(2);
    if($v['c']!=0 || strtolower($_GET['username'])=='jzdc'){
        exit("{'state':'fail','info':'".self::$language['username'].self::$language['exist']."'}");
    }
    if(is_match(self::$config['phone_country_reg'][86],$_GET['username'])){
        exit("{'state':'fail','info':'用户名不能是手机号码'}");
    }

    exit("{'state':'success','info':'可以注册'}");
...
if($act=='check_phone'){
    ...
    if(!is_match(self::$config['phone_country_reg'][$phone_country],$phone)){
        exit("{'state':'fail','info':'请输入有效的手机号码','key':'phone'}");
    }
    //是否被占用
    $sql="select `id` from ".$pdo->index_pre."user where `phone`='".$phone."'";
    $r=$pdo->query($sql,2)->fetch(2);
...
if(self::$config['reg_set']['phone']){
	...
	if(!is_match(self::$config['phone_country_reg'][86],$_GET['phone'])){exit("{'errType':'phone','errInfo':'<span class=fail>".self::$language['phone'].self::$language['pattern_err']."</span>'}");}
...
$stmt=$pdo->query($sql,2);
$v=$stmt->fetch(2);
if($v['c']!=0 || strtolower($_GET['username'])=='jzdc'){exit("{'errType':'username','errInfo':'".self::$language['username'].self::$language['exist']."'}");}
if(is_match(self::$config['phone_country_reg'][86],$_GET['username'])){exit("{'errType':'username','errInfo':'用户名不能是手机号码'}");}
...
//echo $manager;
$sql="insert into ".$pdo->index_pre."user (`nickname`,`email`,`username`,`password`,`reg_time`,`reg_ip`,`group`,`state`,`manager`,`phone`,`openid`,`phone_country`) values ('".$_GET['username']."','".$_GET['email']."','".$_GET['username']."','".$_GET['password']."','$time','$ip','".$_GET['group']."',$state,$manager,'".$_GET['phone']."','".$openid."','86')";
if(isset($_SESSION['share'])){
	...
	$sql="insert into ".$pdo->index_pre."user (`nickname`,`email`,`username`,`password`,`reg_time`,`reg_ip`,`group`,`state`,`manager`,`phone`,`introducer`,`openid`,`phone_country`) values ('".$_GET['username']."','".$_GET['email']."','".$_GET['username']."','".$_GET['password']."','$time','$ip','".$_GET['group']."',$state,$manager,'".$_GET['phone']."','".$r['username']."','".$openid."','86')";
}
