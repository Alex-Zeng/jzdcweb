...
$sql="select `id` from ".$pdo->index_pre."withdraw where `username`='".$_SESSION['jzdc']['username']."' and `time`>".$today." limit ".self::$config['withdraw_set']['max_time'].",1";
...
if($act=='submit'){
	...
	$sql="select `real_name`,`money` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."'";
	...
	$sql="insert into ".$pdo->index_pre."withdraw (`username`,`money`,`billing_info`,`time`,`ip`,`state`,`method`,`rate`) values ('".$_SESSION['jzdc']['username']."','$money','".$billing_info." ".$user['real_name']."','".time()."','".get_ip()."','1','".$method."','".self::$config['withdraw_set']['rate']."')";
	if($pdo->exec($sql)){
		$id=$pdo->lastInsertId();
		...
		if(self::$config['withdraw_set']['before_mode'] ){
			...
			if(operator_money(self::$config,self::$language,$pdo,$_SESSION['jzdc']['username'],'-'.($money+$rate_money),$reason,self::$config['class_name'])){
		...
		if($method==1 && $money<self::$config['withdraw_set']['wx_withdraw_auto_pay']){
			//$r=wexin_transfers($pdo,$_SESSION['jzdc']['username'],self::$config['web']['name'].' '.self::$language['withdraw'],$money,'index');
			$r=wexin_red_pack($pdo,$_SESSION['jzdc']['username'],self::$config['web']['name'].' '.self::$language['withdraw'],self::$config['web']['name'],$money,'index');
			if($r){
				$sql="update ".$pdo->index_pre."withdraw set `state`='3',`operator`='".$_SESSION['jzdc']['username']."',`pay_time`='".time()."' where `id`='$id' and `state`='1'";
				if($pdo->exec($sql)){}
				if(!self::$config['withdraw_set']['before_mode'] ){
					$reason=self::$language['withdraw'].'+'.self::$language['withdraw_rate'].'('.self::$config['withdraw_set']['rate'].'% '.self::$language['money_symbol'].$rate_money.')';
					if(operator_money(self::$config,self::$language,$pdo,$_SESSION['jzdc']['username'],'-'.($money+$rate_money),$reason,self::$config['class_name'])){
