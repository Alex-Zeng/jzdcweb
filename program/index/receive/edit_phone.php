...
if($act=='get_verification_code'){
	...
	$password=md5($phone);
	$sql="select `id`,`openid`,`password` from ".$pdo->index_pre."user where `phone`='".$phone."' and `id`!=".$_SESSION['jzdc']['id']." limit 0,1";
	...
	$sql="select `phone`,`phone_country` from ".$pdo->index_pre."user where `id`=".$_SESSION['jzdc']['id'];
	$r=$pdo->query($sql,2)->fetch(2);
...
if($act=='update'){
	...
	$sql="select `id`,`openid`,`password`,`credits`,`money`,`username` from ".$pdo->index_pre."user where `phone`='".$phone."' and `id`!=".$_SESSION['jzdc']['id']." limit 0,1";
	$r=$pdo->query($sql,2)->fetch(2);
	$password=md5($phone);
	if($r['id']!=''){
		//if($r['openid']!=''){exit("{'state':'fail','info':'".self::$language['phone'].self::$language['exist']."','key':'phone'}");}
	...
	$sql="update ".$pdo->index_pre."user set `phone`='".$phone."',`phone_country`='".$phone_country."' where `id`=".$_SESSION['jzdc']['id'];
	if($pdo->exec($sql)){
		if($r['id']!=''){
			if($r['openid']==''){
				
				$reason=$phone.self::$language['member_merge_import'];
				if($r['credits']>0){operation_credits($pdo,self::$config,self::$language,$_SESSION['jzdc']['username'],$r['credits'],$reason,'other');}
				if($r['money']>0){operator_money(self::$config,self::$language,$pdo,$_SESSION['jzdc']['username'],$r['money'],$reason,'index',false);}
				$sql="delete from ".$pdo->index_pre."user where `id`=".$r['id'];
				$pdo->exec($sql);
				
				$sql="update ".$pdo->index_pre."user set `introducer`='".$_SESSION['jzdc']['username']."' where `introducer`='".$r['username']."'";
				$pdo->exec($sql);
