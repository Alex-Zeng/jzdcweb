<?php
		...
		$id=intval(@$_GET['id']);
		$forbid=array('id','username','group','money','state','identifying','transaction_password','email','phone','introducer');
		...
		switch ($update) {
			...
			case 'homepage':
				...
				break;
            case 'password':
                if(empty($$update)){exit("{'state':'fail','info':'<span class=fail>".self::$language['homepage'].self::$language['pattern_err']."</span>'}");}
                $$update=md5($$update);
                break;
