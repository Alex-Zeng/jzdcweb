...
if($act=='go_module_template'){
	...
	$path='./templates/'.$require_login.'/'.$module[0].'/'.$program_config['program']['template_'.$require_login].'/'.$_COOKIE['jzdc_device'].'/'.$module[1].'.php';
	if(!file_exists($path)){exit('template '.self::$language['not_exist_file']);}
	header('location:./index.php?jzdc=index.template_edit_file&path='.$path);
	exit();
}
