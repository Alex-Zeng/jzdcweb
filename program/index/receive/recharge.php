<?php
	...
	$_POST['title']=self::$language['for2'].self::$language['account'].$_SESSION['jzdc']['username'].self::$language['recharge'];
	...
	if($_POST['pay_info']=='' && $_POST['pay_photo']==''){$state=2;$method='online_payment';}else{$state=1;$method='offline_payment';$_POST['return_url']='./index.php?jzdc=index.recharge_log';$_POST['payment']='offline_pay';}
	$sql="insert into ".$pdo->index_pre."recharge (`username`,`money`,`time`,`state`,`title`,`return_url`,`pay_info`,`pay_photo`,`method`,`type`) values ('".$_SESSION['jzdc']['username']."','".$_POST['money']."','".time()."','".$state."','".$_POST['title']."','".$_POST['return_url']."','".$_POST['pay_info']."','".$_POST['pay_photo']."','$method','".safe_str($_POST['payment'])."')";
	...
	if($pdo->exec($sql)){
		...
		if($state==1){
			...
			//exit("{'state':'success','info':'<span class=success>&nbsp;</span>".self::$language['submit'].self::$language['success'].",".self::$language['recharge_state'][1]." <a href='./index.php?jzdc=index.user'>".self::$language['return']."</a>'}");
...
';
				...
				echo '<form id="payment_form" name="payment_form" method="post" action="./index.php?jzdc=scanpay.pay&id='.$t['id'].'">
  <input type="hidden" name="success_fun" id="success_fun" value="update_recharge|'.$in_id.'" />
  <input type="hidden" name="money" id="money" value="'.$_POST['money'].'" />
  <input type="hidden" name="payer" id="payer" value="'.$_SESSION['jzdc']['username'].'" />
