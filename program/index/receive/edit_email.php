...
if($act=='get_verification_code'){
	...
	$sql="select `id` from ".$pdo->index_pre."user where `email`='".$email."' and `id`!=".$_SESSION['jzdc']['id'];
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['id']!=''){exit("{'state':'fail','info':'".self::$language['email'].self::$language['exist']."','key':'email'}");}
	
	$sql="select `email` from ".$pdo->index_pre."user where `id`=".$_SESSION['jzdc']['id'];
	$r=$pdo->query($sql,2)->fetch(2);
...
if($act=='update'){
	...
	$sql="update ".$pdo->index_pre."user set `email`='".$email."' where `id`=".$_SESSION['jzdc']['id'];
	if($pdo->exec($sql)){
