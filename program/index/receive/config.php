...
if(count($key)==2){
	...
	if($key[0]=='web'){
		if($key[1]=='name' || $key[1]=='jzdc_username' || $key[1]=='jzdc_password' || $key[1]=='domain' || $key[1]=='url' || $key[1]=='title'){
			if($key[1]=='domain'){
			...
			if($key[1]=='jzdc_password'){
				$url=self::$config['server_url'].'receive.php?target=server::synchronization&update=site&id='.self::$config['web']['site_id'].'&key='.self::$config['web']['site_key'].'&field='.$key[1].'&v='.$$update.'&username='.urlencode(self::$config['web']['jzdc_username']);
			
			}
			$c=@file_get_contents($url);
			$c=trim($c);	
			if($key[1]=='jzdc_username' || $key[1]=='jzdc_password' ){
		...
		if($key[1]=='wid' && $$update!=''){
			...
			if($t['id']==''){exit("{'state':'fail','info':'<span class=fail>".self::$language['not_exist']." <a href=./index.php?jzdc=weixin.account_add ><b>".self::$language['add']."</b></a></span>'}");}
		...
		if($key[1]=='domain'){
			...
			if($c!='ok'){exit("{'state':'fail','info':'<span class=fail>".self::$language['fail']."</span>','alert_str':'".self::$language['need_to_connnect_to_the_Lansion_server_can_be_used']."'}");}
