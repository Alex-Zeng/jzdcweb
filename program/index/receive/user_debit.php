...
if($act=='add'){
	...
	$reason=self::$language['administrator'].'('.$_SESSION['jzdc']['username'].')'.self::$language['execute'].self::$language['pages']['index.user_debit']['name'].','.self::$language['reason'].':'.safe_str($_POST['reason']);
	if(operator_money(self::$config,self::$language,$pdo,$username,'-'.$money,$reason,'index')){
		exit("{'state':'success','info':'<span class=success>".self::$language['success']." <a href=./index.php?jzdc=index.money_log_admin&username=".$username.">".self::$language['view']."</a></span>'}");
	}else{
