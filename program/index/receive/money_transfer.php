...
if($act=='submit'){
	...
	if($payee==$_SESSION['jzdc']['username']){exit("{'state':'fail','info':'<span class=fail>&nbsp;</span>".self::$language['can_not_be_yourself']."','key':'payee'}");}
	...
	$sql="select `transaction_password`,`money` from ".$pdo->index_pre."user where `username`='".$_SESSION['jzdc']['username']."'";
	$r=$pdo->query($sql,2)->fetch(2);
	...
	$sql="insert into ".$pdo->index_pre."money_transfer (`username`,`money`,`remark`,`payee`,`time`,`ip`) values ('".$_SESSION['jzdc']['username']."','$money','$remark','$payee','".time()."','".get_ip()."')";
	if($pdo->exec($sql)){
		$insret_id=$pdo->lastInsertId();
		if(operator_money(self::$config,self::$language,$pdo,$_SESSION['jzdc']['username'],'-'.$money,self::$language['transfer'].self::$language['to'].$payee,self::$config['class_name'])){
			if(operator_money(self::$config,self::$language,$pdo,$payee,$money,$_SESSION['jzdc']['username'].self::$language['transfer'],self::$config['class_name'])){
				exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span>'}");
			}else{
				$sql="delete from ".$pdo->index_pre."money_transfer where `id`='$insret_id'";
				if(!$pdo->exec($sql)){add_err_log($sql);}
				operator_money(self::$config,self::$language,$pdo,$_SESSION['jzdc']['username'],$money,self::$language['return'].self::$language['transfer'].self::$language['to'].$payee,self::$config['class_name']);
