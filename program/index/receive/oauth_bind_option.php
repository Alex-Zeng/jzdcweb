...
if($act=='create'){
	...
	$v=$stmt->fetch(2);
	if($v['id']!='' || strtolower($_GET['username'])=='jzdc'){$_GET['username'].='_'.get_verification_code(5);}
	$sql="select `id` from ".$pdo->index_pre."user where `username`='".$_GET['username']."' limit 0,1";
	$stmt=$pdo->query($sql,2);
	$v=$stmt->fetch(2);
	if($v['id']!='' || strtolower($_GET['username'])=='jzdc'){exit("{'state':'fail','info':'exist'}");	}
	...
	$sql="insert into ".$pdo->index_pre."user (`nickname`,`username`,`password`,`reg_time`,`reg_ip`,`group`,`state`,`manager`,`gender`,`reg_oauth`,`openid`,`icon`) values ('".$_GET['username']."','".$_GET['username']."','".$_GET['password']."','$time','$ip','".$_GET['group']."',1,$manager,'".$gender."',1,'".$openid."','".$_SESSION['oauth']['icon']."')";
