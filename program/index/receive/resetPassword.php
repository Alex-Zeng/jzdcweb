...
if($act=='sendCheckCode'){
	...
	if(is_email($_GET['username'])){
		...
		$state=email(self::$config,self::$language,$pdo,'jzdc',$_GET['username'],$title,$content);
		if($state){
			$state='success';
	...
	if(preg_match(self::$config['phone_country_reg'][$phone_country],$_GET['username'])){
		...
		$state=sms(self::$config,self::$language,$pdo,'jzdc','+'.$phone_country.$_GET['username'],$identifying);
		//$state=true;
		if($state){
			$state='success';
			$info=self::$language['sent'];	
...
if($act=='reset'){
	...
	if($v['identifying']==''){exit ("{'errType':'identifying','errInfo':'".self::$language['identifying_expire']."<a href=index.php?jzdc=index.resetPassword>".self::$language['reset']."</a>'}");	}
