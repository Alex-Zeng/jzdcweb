<?php
if(isset($_GET['jzdc']) && $_GET['jzdc']=='slider.show'){$id=intval($_GET['id']);}else{$id=intval($temp[1]);}
...
if($id>0){
	...
	if($module['width']=='100%' && $module['style']=='width_full' && $_COOKIE['jzdc_device']=='pc'){
	...
	$t_path='./templates/'.$m_require_login.'/'.$class.'/'.self::$config['program']['template_'.$m_require_login].'/'.$_COOKIE['jzdc_device'].'/style/'.$module['style'].'/'.str_replace($class."::","",$method).'.php';
