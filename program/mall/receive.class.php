...
//		var_dump($weight<$weight_all);
	...
	//===============================================================================================更新购物车数据
 	function update_cart($pdo,$table_pre){
		if(!isset($_SESSION['jzdc']['username'])){return false;}
		
		if(@$_COOKIE['mall_cart']==''){
			$sql="select `last_time` from ".$pdo->index_pre."user where `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
			$r=$pdo->query($sql,2)->fetch(2);
			if(time()<$r['last_time']+60){
				$sql="select * from ".$table_pre."cart where `username`='".$_SESSION['jzdc']['username']."'";
				$r=$pdo->query($sql,2);
				$mall_cart=array();
				foreach($r as $v){
		...
		if(@$_COOKIE['mall_cart']!=''){				
			...
			$sql="select `id`,`key` from ".$table_pre."cart where `username`='".$_SESSION['jzdc']['username']."'";
			$r=$pdo->query($sql,2);
			$key=array();
			foreach($r as $v){
				if(isset($cart[$v['key']])){
			...
			foreach($cart as $k=>$v){
				if(!isset($key[$k])){
					$sql="insert into ".$table_pre."cart (`username`,`key`,`quantity`,`price`,`time`) values ('".$_SESSION['jzdc']['username']."','".safe_str($k)."','".floatval($v['quantity'])."','".floatval($v['price'])."','".(floatval($v['time'])/1000)."')";
					$pdo->exec($sql);
				}	
			}
			...
			$sql="delete from ".$table_pre."cart where `username`='".$_SESSION['jzdc']['username']."'";
			$pdo->exec($sql);	
		}
	}
		...
		if($g_ids!=''){
			...
			if(count($email)>0 && $order['buyer']!=''){
				if($user['email']!=''){
					...
					email($config,$language,$pdo,'jzdc',$user['email'],$email_t,$email_content);
...
//======================================================================================================= 网上自助提交的订单符合减商品库存条件时，减商品库存
		...
		if(intval($r['decrease_quantity'])==intval($order['state']) && $order['inventory_decrease']==0){
			...
			foreach($r as $v){
				...
				if($pdo->exec($sql)){
					...
					$sql="insert into ".$table_pre."goods_quantity_log (`goods_id`,`quantity`,`username`,`time`,`order_id`,`batch_id`,`shop_id`,`in_price`,`out_price`,`s_id`) values ('".$v['goods_id']."','-".$v['quantity']."','jzdc','".time()."','".$order['id']."','".$batch_id."','".$order['shop_id']."','".$in_price."','".$v['transaction_price']."','".$v['s_id']."')";
					$pdo->exec($sql);
    ...
    //======================================================================================================= 交易完成统计销量
    function add_goods_sold($pdo,$table_pre,$order){
        $sql="select `id`,`goods_id`,`s_id`,`quantity`,`transaction_price` from ".$table_pre."order_goods where `order_id`=".$order['id'];
        $r=$pdo->query($sql,2);
        foreach($r as $v){
            $sql="update ".$table_pre."goods set `sold`=`sold`+".$v['quantity']." where `id`=".$v['goods_id'];
            $pdo->exec($sql);
        }
		...
		if($r['big']!=0 && $r['quantity']>0){
			...
			if($pdo->exec($sql)){
				...
				if($batch_id!=''){
					...
					$sql="insert into ".self::$table_pre."goods_deduct_stock (`goods_id`,`s_id`,`quantity`,`money`,`reason`,`time`,`username`,`shop_id`) values ('".$r['big']."','".$option_id."','".$d_big_quantity."','".($batch['price']*$d_big_quantity)."','".self::$language['auto_deleave_big_box']."','".time()."','jzdc','".$g['shop_id']."')";
...
//======================================================================================================= 收银台，交易提醒买家。
	function checkout_order_notice($language,$config,$pdo,$table_pre,$order){
		...
		if($order_set['checkout_order_notice_email'] && is_email($buyer['email'])){			
			email($config,$language,$pdo,'jzdc',$buyer['email'],$title,'<div style="font-size:30px;">'.$title.'</div>');
		}
		if($order_set['checkout_order_notice_sms'] && preg_match($config['other']['reg_phone'],$buyer['phone'])){
			//sms($config,$language,$pdo,'jzdc',$buyer['phone'],$title);
		}
	}
...
//======================================================================================================= 发货后，通知买家
	function send_notice($language,$config,$pdo,$table_pre,$order){
		...
		if($order_set['send_notice_email'] && is_email($buyer['email'])){
			...
			email($config,$language,$pdo,'jzdc',$buyer['email'],$title,$content);
		}
		if($order_set['send_notice_sms'] && preg_match($config['other']['reg_phone'],$buyer['phone'])){
			//if(self::sms_fees($pdo,$seller,$order['shop_id'],$order['id'],$title,true)){sms($config,$language,$pdo,'jzdc',$buyer['phone'],$title);}
		}
	}
...
//======================================================================================================= 更新店铺自定义模块
	function update_module($pdo,$del_id=0){
		...
		foreach($r as $v){
			$id=$v['id'];
			$module_config['mall.diymodule_show_'.$id]['edit_url']='index.php?jzdc=mall.diymodule_edit&id='.$id.'&';
...
//======================================================================================================= 返回商品单位的名称
	function get_type_position($pdo,$id){
		...
		$r=$pdo->query($sql,2)->fetch(2);
		$position='<a href="./index.php?jzdc=mall.goods_list&type='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>';
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
			$position='<a href="./index.php?jzdc=mall.goods_list&type='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>'.$position;
		}
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
...
//======================================================================================================= 根据店内商品分类返回位置条信息
	function get_shop_type_position($pdo,$id){
		...
		$r=$pdo->query($sql,2)->fetch(2);
		$position='<a href="./index.php?jzdc=mall.shop_goods_list&shop_id='.SHOP_ID.'&type='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>';
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."shop_type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
			$position='<a href="./index.php?jzdc=mall.shop_goods_list&shop_id='.SHOP_ID.'&type='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>'.$position;
		}
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."shop_type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
...
//======================================================================================================= 返回店内标签信息
	function get_shop_tags_name($pdo,$tag){
		...
		$temp='';
		foreach($r as $v){
			$temp.='<a href=./index.php?jzdc=mall.shop_goods_list&shop_id='.SHOP_ID.'&tag='.$v['id'].'>'.$v['name'].'</a> , ';
		}
		$temp=trim($temp,' , ');
		return $temp;
	...
	function get_store_tags_name($pdo,$tag){
		...
		$temp='';
		foreach($r as $v){
			$temp.='<a href=./index.php?jzdc=mall.shop_list&tag='.$v['id'].'>'.$v['name'].'</a> , ';
		}
		$temp=trim($temp,' , ');
		return $temp;
...
//======================================================================================================= 获取平台标签 并返回 HTML
	function get_tags_name($pdo,$tag){
		...
		$temp='';
		foreach($r as $v){
			$temp.='<a href=./index.php?jzdc=mall.goods_list&tag='.$v['id'].'>'.$v['name'].'</a> , ';
		}
		$temp=trim($temp,' , ');
		return $temp;
...
//======================================================================================================= 更新店铺导航条
	function update_shop_navigation($pdo,$language,$shop_id=0){
		...
		foreach($r as $v){
			...
			$temp3='';
			foreach($r3 as $v3){
				$temp3.='<li><a href="./index.php?jzdc=mall.shop_goods_list&type='.$v3['id'].'"  id="stype_3_'.$v3['id'].'"><span>'.$v3['name'].'</span></a></li>';	
			}
			if($temp3!=''){
				$temp2.='<li><a href="./index.php?jzdc=mall.shop_goods_list&type='.$v['id'].'" id="stype_2_'.$v['id'].'"><span>'.$v['name'].'</span><i class="fa fa-angle-right"></i></a><ul>'.$temp3.'</ul></li>';
			}else{
				$temp2.='<li><a href="./index.php?jzdc=mall.shop_goods_list&type='.$v['id'].'" id="stype_2_'.$v['id'].'"><span>'.$v['name'].'</span></a></li>';
			}
			
		}
		
		if($temp2!=''){
			$temp1.='<li id=stype><a href="./index.php?jzdc=mall.shop_goods_list&shop_id='.$shop_id.'"><span>'.$language['shop_whole_goods'].'</span><i class="fa fa-angle-down"></i></a><ul>'.$temp2.'</ul></li>';
		}else{
			$temp1.='<li id=stype><a href="./index.php?jzdc=mall.shop_goods_list&shop_id='.$shop_id.'"><span>'.$language['shop_whole_goods'].'</span></a></li>';
		}
...
//=======================================================================================================  更新店铺手机版底部菜单
	function update_shop_menu($pdo,$language,$shop_id=0){
		...
		foreach($r as $v){
			$v=de_safe_str($v);
			$list.='<div class=type_1 href=./index.php?jzdc=mall.shop_goods_list&shop_id='.$shop_id.'&type='.$v['id'].'><span class=name>'.$v['name'].'</span>';
			$list2='';
			$sql="select `id`,`name` from ".$pdo->sys_pre."mall_shop_type where `shop_id`=".$shop_id." and `parent`=".$v['id']." order by `sequence` desc";
			$r2=$pdo->query($sql,2);
			foreach($r2 as $v2){
				$v2=de_safe_str($v2);
				$list2.='<a href=./index.php?jzdc=mall.shop_goods_list&shop_id='.$shop_id.'&type='.$v2['id'].' >'.$v2['name'].'</a>';	
			}
		...
		$list='<div class=shop_goods_type><div class=contents><div class=title>'.$shop_name.'</div>
				<div class=list>
					<div class=type_1 href=./index.php?jzdc=mall.shop_goods_list&shop_id='.$shop_id.'><span class=name>'.$language['all_goods'].'</span></div>
				'.$list.'
				</div>
...
//=======================================================================================================  申请开店，审核后，初始化店铺数据_____________start
	...
	//======================================================================================================================diypage contact
		...
		$sql="insert into ".self::$table_pre."diypage (`title`,`content`,`phone_content`,`shop_id`,`time`,`creater`) values ('".self::$language['contact']."','".$contact_pc."','".$contact_phone."','".$shop_id."','".time()."','jzdc')";
		$pdo->exec($sql);
		$contact_id=$pdo->lastInsertId();
		
	//=======================url config
		$all_goods_url='./index.php?jzdc=mall.shop_goods_list&shop_id='.$shop_id;
		$index_url='./index.php?jzdc=mall.shop_index&shop_id='.$shop_id;
		$contact_url="./index.php?jzdc=mall.diypage_show&shop_id=".$shop_id."&id=".$contact_id;
		
	//======================================================================================================================navigation
		$sql="insert into ".self::$table_pre."navigation (`name`,`url`,`sequence`,`open_target`,`shop_id`) values('".self::$language['index']."','".$index_url."','999','_self','".$shop_id."'),('".self::$language['contact']."','".$contact_url."','-1','_self','".$shop_id."')";
		$pdo->exec($sql);
	...
	*/
	...
	//======================================================================================================================update_shop_other
		$data['pc_menu']='';
		$data['phone_menu']='';
		$data['head_phone']='<img src="./templates/0/mall/default/shop_default_data/shop_phone_banner.png" />';
	...
	//======================================================================================================================page
		...
		$sql="insert into ".self::$table_pre."page (`head`,`left`,`right`,`full`,`phone`,`layout`,`url`,`shop_id`,`bottom`) values('mall.head,mall.nav','".$left."','".$right."','".$full."','','full','mall.shop_index','".$shop_id."','mall.foot')";
		$pdo->exec($sql);

		
		$sql="insert into ".self::$table_pre."page (`head`,`left`,`right`,`full`,`phone`,`layout`,`url`,`shop_id`,`bottom`) values('mall.head,mall.nav,','','',',mall.diypage_show','','full','mall.diypage_show','".$shop_id."','mall.foot')";
		$pdo->exec($sql);

		$sql="insert into ".self::$table_pre."page (`head`,`left`,`right`,`full`,`phone`,`layout`,`url`,`shop_id`,`bottom`) values('mall.head,mall.nav,','','',',mall.shop_goods_list','','full','mall.shop_goods_list','".$shop_id."','mall.foot')";
		$pdo->exec($sql);
		
		$sql="insert into ".self::$table_pre."page (`head`,`left`,`right`,`full`,`phone`,`layout`,`url`,`shop_id`,`bottom`) values('mall.head,mall.nav,','','',',mall.coupon','','full','mall.coupon','".$shop_id."','mall.foot')";
		$pdo->exec($sql);
...
function operation_finance($language,$pdo,$table_pre,$shop_id,$money,$type,$reason){
	...
	$sql="insert into ".$table_pre."finance (`type`,`time`,`money`,`before_money`,`after_money`,`operator`,`reason`,`shop_id`,`shop_name`) values ('".$type."','".time()."','".$money."','".$before_money."','".$after_money."','".@$_SESSION['jzdc']['username']."','".$reason."','".$shop_id."','".$shop_name."')";
	if($pdo->exec($sql)){
		return true;
	}else{
		return false;
...
function operation_shop_finance($language,$pdo,$table_pre,$shop_id,$money,$type,$reason){
	...
	//echo $money;
	$after_money=$before_money+$money;
	$sql="insert into ".$table_pre."shop_finance (`type`,`time`,`money`,`before_money`,`after_money`,`operator`,`reason`,`shop_id`) values ('".$type."','".time()."','".$money."','".$before_money."','".$after_money."','".@$_SESSION['jzdc']['username']."','".$reason."','".$shop_id."')";
	if($pdo->exec($sql)){
		return true;
	}else{
		return false;
...
function operation_agent_finance($language,$pdo,$table_pre,$shop_id,$money,$type,$reason,$shop_name,$agent){
	...
	//echo $money;
	$after_money=$before_money+$money;
	$sql="insert into ".$table_pre."agent_finance (`type`,`time`,`money`,`before_money`,`after_money`,`operator`,`reason`,`shop_id`,`shop_name`,`username`) values ('".$type."','".time()."','".$money."','".$before_money."','".$after_money."','".@$_SESSION['jzdc']['username']."','".$reason."','".$shop_id."','".$shop_name."','".$agent."')";
	if($pdo->exec($sql)){
		return true;
	}else{
		return false;
...
function exe_pay_order_fees($pdo,$seller,$r){
	$annual_shop_order_fees=self::$config['annual_shop_order_fees'];
	$times_shop_order_fees=self::$config['times_shop_order_fees'];
	$sql="select `tag` from ".self::$table_pre."shop where `id`=".$r['shop_id'];
	$temp=$pdo->query($sql,2)->fetch(2);
	if($temp['tag']!=''){
		$temp=explode('|',$temp['tag']);
		$tag_ids='';
		foreach($temp as $v){
			if(is_numeric($v)){$tag_ids.=$v.',';}
		}
		$tag_ids=trim($tag_ids,',');
		if($tag_ids!=''){
			$sql="select `rate` from ".self::$table_pre."store_tag where `id` in (".$tag_ids.") order by `rate` desc limit 0,1";
			$temp=$pdo->query($sql,2)->fetch(2);
			$times_shop_order_fees=$temp['rate'];
			$sql="select `annual_rate` from ".self::$table_pre."store_tag where `id` in (".$tag_ids.") order by `annual_rate` desc limit 0,1";
			$temp=$pdo->query($sql,2)->fetch(2);
			$annual_shop_order_fees=$temp['annual_rate'];
	...
	if($annual_shop_order_fees==0 || $times_shop_order_fees==0 ){return true;}
	if(($r['pay_method']=='balance' || $r['pay_method']=='online_payment' || $r['pay_method']=='cash_on_delivery') && $r['cashier']=='jzdc' ){
		$sql="select `annual_fees` from ".self::$table_pre."shop where `id`=".$r['shop_id'];
		$shop=$pdo->query($sql,2)->fetch(2);
		if(floatval($shop['annual_fees'])>floatval($r['add_time'])){
			if($annual_shop_order_fees==0){return true;}
			$reason=self::$language['payment'].' '.$r['out_id'].self::$language['order_postfix'].' '.self::$language['annual_shop_order_fees'].$annual_shop_order_fees.'%';
			$money='-'.$r['actual_money']*($annual_shop_order_fees/100);
		}else{
			if($times_shop_order_fees==0){return true;}
			$reason=self::$language['payment'].' '.$r['out_id'].self::$language['order_postfix'].' '.self::$language['times_shop_order_fees'].$times_shop_order_fees.'%';
			$money='-'.$r['actual_money']*($times_shop_order_fees/100);
		}
		if(operator_money(self::$config,self::$language,$pdo,$seller,$money,$reason,'mall',true)){
...
function exe_pay_ad_fees($pdo,$seller,$r){
	...
	if(($r['share']!='' && $r['share']!=$r['buyer']) && $r['cashier']=='jzdc' ){
...
//=======================================================================================================把逾期未确认收货的订单，设为交易成功
    function auto_receipt_expire_order($pdo){
        ...
        //待收货, 待质检
        $sql="select * from ".self::$table_pre."order where (`state`=6 or `state`=7 ) and `send_time`<".$expire_time;
        $r2=$pdo->query($sql,2);
        foreach($r2 as $r){
            //echo $r['id'].'<br />';

            if(empty($r['pay_date'])){
                $sql1="select * from ".self::$table_pre."order_pay where `order_id`='".$r['id']."' limit 1";
                $pay_rec=$pdo->query($sql1,2)->fetch(2);
                if($pay_rec['id']==''){continue;}

                //没有账期 -->待打款至供应商
                $sql="update ".self::$table_pre."order set `state`='11',`receipt`=1,`receipt_time`='".time()."' where `id`=".$r['id'];
            }else{
                //有账期 -->账期中
                $sql="update ".self::$table_pre."order set `state`='9',`receipt`=1,`receipt_time`='".time()."' where `id`=".$r['id'];
            }
            $pdo->exec($sql);
        }

        //账期中
        $sql="select * from ".self::$table_pre."order where `state`=9 ";
        $r2=$pdo->query($sql,2);
        foreach($r2 as $r){
            //echo $r['id'].'<br />';
            if(empty($r['pay_date']) || strtotime("now")>strtotime($r['pay_date'])){
                //账期中 -->逾期中
                $sql="update ".self::$table_pre."order set `state`='10' where `id`=".$r['id'];
                ...
                if($pdo->exec($sql)){
                    ...
                    if ($r2){
                        ...
                        $msg = send_notice_sms(self::$config, $pdo, $r2["phone"], 'order_out_date', $param);
                    }
                }
            }
        }
...
//=======================================================================================================【店铺进货，添加库存】
	function add_goods_batch($pdo,$goods_id,$quantity,$price,$supplier,$expiration=0){
		...
		$sql="insert into ".self::$table_pre."goods_batch (`goods_id`,`quantity`,`price`,`add_time`,`sell_out_time`,`left`,`shop_id`,`supplier`,`expiration`,`payment`) values ('".$goods_id."','".$quantity."','".$price."','".$time."','0','".$quantity."','".SHOP_ID."','".intval($supplier)."','".$expiration."','".$payment."')";
...
//======================================================================================================= 购物后 自动添加买家为店内会员
	function add_shop_buyer($pdo,$username,$shop_id){
		...
		$sql="insert into ".self::$table_pre."shop_buyer (`username`,`time`,`shop_id`,`group_id`,`phone`,`email`) value ('".$username."','".time()."','".$shop_id."','".$group_id."','".$user['phone']."','".$user['email']."')";
		return $pdo->exec($sql);	
	}
...
//======================================================================================================= 交易成功后返积分
	function give_credits($pdo,$r){
		...
		$credits_multiple=self::$config['give_credits'];
		$sql="select `group` from ".$pdo->index_pre."user where `username`='".$r['buyer']."' limit 0,1";
		$t=$pdo->query($sql,2)->fetch(2);
		if($t['group']==''){return false;}
		$sql="select `credits` from ".self::$table_pre."group_set where `group_id`=".$t['group']." limit 0,1";
		$t=$pdo->query($sql,2)->fetch(2);
		if(isset($t['credits'])){$credits_multiple=$t['credits'];}
		$number=$credits_multiple*$r['actual_money'];
		$reason='';
		if($r['cashier']=='' || $r['cashier']=='jzdc'){
			operation_credits($pdo,self::$config,self::$language,$r['buyer'],$number,$reason,'buy');
...
//======================================================================================================= 申请退货后，退回送的积分
	function untread_credits($pdo,$r){
		if($r['buyer']==''){return false;}
		if($r['give_web_credits']>0){operation_credits($pdo,self::$config,self::$language,$r['buyer'],'-'.$r['actual_money']*self::$config['give_credits'],'','buy_return');}
		if($r['give_shop_credits']>0){
			...
			$v=$pdo->query($sql,2)->fetch(2);
			$number=$v['buy_credits_multiple']*$r['actual_money'];		
			self::operator_shop_buyer_credits($pdo,$r['buyer'],'-'.$number,self::$language['credits_type']['buy_return'],$r['shop_id']);
		}
...
//======================================================================================================= 店内会员自动升级
	function shop_group_auto_upgrade($pdo,$username,$shop_id){
		...
		$sql="select `id`,`credits` from ".self::$table_pre."shop_buyer_group where `id`!=".$user['group_id']."   and `shop_id`=".$shop_id." and `credits`>".$group['credits']." and `credits`<".($user['cumulative_credits']+1)."  order by `credits` desc limit 1";
		$r=$pdo->query($sql,2)->fetch(2);
		if($r['id']==1){return false;}
		if($r['id']!='' && $r['credits']>0){
			$sql="update ".self::$table_pre."shop_buyer set `group_id`=".$r['id']." where `username`='".$username."'  and `shop_id`=".$shop_id."  limit 1";
			$r=$pdo->exec($sql);
		}
...
//======================================================================================================= 获取店内会员折扣
	function shop_buyer_group_discount($pdo,$shop_id){
		$sql="select `group_id` from ".self::$table_pre."shop_buyer where `shop_id`=".$shop_id." and `username`='".$_SESSION['jzdc']['username']."'  limit 0,1";
		$r=$pdo->query($sql,2)->fetch(2);
...
//======================================================================================================= 返回适用红包 抵扣金额
	function get_red_coupon($pdo,$table_pre,$promotion_money,$all_money,$shop_id){
		...
		$sql="select `coupon_id` from ".self::$table_pre."my_coupon where `username`='".$_SESSION['jzdc']['username']."' and `use_time`=0 and `coupon_id` in (".$red_ids.")";
		$r=$pdo->query($sql,2);
		$useable='';
		foreach($r as $v){
			$useable.=$v['coupon_id'].',';	
...
//================================================================================================================================= 分店上架总部商品 start
	function goods_up($pdo,$goods_id){
		...
		if($goods['option_enable']==0){
			$sql="insert into ".self::$table_pre."goods (`type`,`brand`,`unit`,`logistics_weight`,`logistics_volume`,`free_shipping`,`supplier`,`title`,`advantage`,`icon`,`multi_angle_img`,`detail`,`sales_promotion`,`time`,`username`,`bar_code`,`w_price`,`e_price`,`cost_price`,`inventory`,`option_enable`,`virtual_auto_delivery`,`m_detail`,`shelf_life`,`shop_id`,`shop_type`,`shop_tag`,`state`,`limit`,`limit_cycle`,`db_id`,`store_code`,`mall_state`) values ('".intval($goods['type'])."','".intval($goods['brand'])."','".intval($goods['unit'])."','".intval($goods['logistics_weight'])."','".floatval($goods['logistics_volume'])."','".intval($goods['free_shipping'])."','".intval($goods['supplier'])."','".$goods['title']."','".$goods['advantage']."','".$goods['icon_new']."','".$goods['multi_angle_img_new']."','".$goods['detail']."','".intval($goods['sales_promotion'])."','".$time."','".$_SESSION['jzdc']['username']."','".$goods['bar_code']."','".floatval($goods['w_price'])."','".floatval($goods['e_price'])."','".floatval($goods['cost_price'])."','0','0','".$goods['virtual_auto_delivery']."','".$goods['m_detail']."','".intval($goods['shelf_life'])."','".SHOP_ID."','".intval($goods['shop_type'])."','".$goods['tag']."','".$goods_state."','".intval($goods['limit'])."','".safe_str($goods['limit_cycle'])."','".$goods['id']."','".$goods['store_code']."',1)";
				
		}else{
			$sql="insert into ".self::$table_pre."goods (`type`,`brand`,`unit`,`logistics_weight`,`logistics_volume`,`free_shipping`,`position`,`supplier`,`title`,`advantage`,`icon`,`multi_angle_img`,`detail`,`sales_promotion`,`time`,`username`,`option_enable`,`virtual_auto_delivery`,`m_detail`,`shelf_life`,`shop_id`,`shop_type`,`shop_tag`,`state`,`limit`,`limit_cycle`,`db_id`,`speci_store_code`,`mall_state`) values ('".intval($goods['type'])."','".intval($goods['brand'])."','".intval($goods['unit'])."','".intval($goods['logistics_weight'])."','".floatval($goods['logistics_volume'])."','".intval($goods['free_shipping'])."','".intval($goods['position'])."','".intval($goods['supplier'])."','".$goods['title']."','".$goods['advantage']."','".$goods['icon_new']."','".$goods['multi_angle_img_new']."','".$goods['detail']."','".intval($goods['sales_promotion'])."','".$time."','".$_SESSION['jzdc']['username']."','1','".$goods['virtual_auto_delivery']."','".$goods['m_detail']."','".intval($goods['shelf_life'])."','".SHOP_ID."','".intval($goods['shop_type'])."','".$goods['tag']."','".$goods_state."','".intval($goods['limit'])."','".safe_str($goods['limit_cycle'])."','".$goods['id']."','".$goods['speci_store_code']."',1)";
		}
...
//================================================================================================================================= 分店上架总部商品 start	
	...
	function get_headquarters_name($pdo){
		$sql="select `name` from ".self::$table_pre."headquarters where `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
		$r=$pdo->query($sql,2)->fetch(2);
		return $r['name'];	
	}
	...
	function stocktake_loss($pdo,$goods_id,$s_id,$quantity){
		...
		if($pdo->exec($sql)){
			...
			$sql="insert into ".self::$table_pre."goods_loss (`goods_id`,`s_id`,`quantity`,`money`,`reason`,`time`,`username`,`shop_id`) values ('".$goods_id."','".$s_id."','".$quantity."','".(self::get_cost_price($pdo,$all_id)*$quantity)."','".self::$language['stocktake'].self::$language['loss']."','".$time."','".$_SESSION['jzdc']['username']."','".SHOP_ID."')";
			if($pdo->exec($sql)){
				return true;
			}else{
				return false;
	...
	function web_credits_subsidy($pdo,$seller,$order){//买家购物使用了平台积分抵扣并订单金额没经过平台，导致店家少收钱，系统执行补贴
		...
		$reason=str_replace('{order_id}','<a href=./index.php?jzdc=mall.order_admin&search='.$order['out_id'].' target=_blank>'.$order['out_id'].'</a>',self::$language['web_credits_deduction_subsidy']);
