...
if($act=='del'){
	$id=intval(@$_GET['id']);
	if($id==0){exit('id err');}
	$sql="delete from ".self::$table_pre."receiver where `id`='".$id."' and `username`='".$_SESSION['jzdc']['username']."'";
...
if($act=='get_receiver'){
	...
	$r=de_safe_str($r);
	if($r['post_code']!='' && $r['post_code']!='0'){
	    $r['post_code']='('.$r['post_code'].')';
	}else{
        $r['post_code']='';
    }
...
if($act=='set_sequence'){
	...
	foreach($temp as $k=>$v){
		...
		$v[1]=intval($v[1]);
		if($v[0]!=0){
			$sql="update ".self::$table_pre."receiver set `sequence`='".$v[1]."' where `id`='".$v[0]."' and `username`='".$_SESSION['jzdc']['username']."'";
			$pdo->exec($sql);	
		}	
	}
	echo 'done';
