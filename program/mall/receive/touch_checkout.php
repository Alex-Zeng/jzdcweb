...
if($act=='submit_checkout'){
	...
	if($_POST['user']!=''){
		...
		if($pay_method=='shop_balance'){
			if($buyer['shop_balance']<$payable){
				...
				$reason=str_replace('{buyer}',$buyer['username'],$reason);
				if(!self::operator_shop_buyer_balance($pdo,$buyer['username'],$remaining,$reason,$_SESSION['jzdc']['username'])){
					exit('{"state":"fail","info":"<span class=fail>'.self::$language['insufficient_balance'].'</span>"}');
				}
	...
	if($pay_method=='cash' && is_numeric($_POST['received_money'])){
		...
		if($received_money<$payable && !jzdc_number_difference($received_money,$payable,0.3)){exit('{"state":"fail","info":"<span class=fail>received_money err</span>"}');}
	...
	if(!$pdo->exec($sql)){
		...
		if($pay_method=='alipay' || $pay_method=='weixin'){
			...
			if($pay['settlement']!=0){
				...
				if($ac['is_web']){
					$reason=str_replace('{order_id}','<a href=./index.php?jzdc=mall.order_admin&search='.$order['out_id'].' target=_blank>'.$order['out_id'].'</a>',self::$language['add_order_money_template']);
		...
		<div class=order>
			...
			'.$order['buyer_remark'].' '.de_safe_str($_POST['address']).'
        </div>
';	
		}
		...
		if(intval(@$_COOKIE['checkout_print_set'])==2){
			...
			foreach($r as $v){
				...
				foreach($r2 as $v2){
					...
					if($v2['barcode']=='0'){$v2['barcode']='';}	
					$list2.="<div class=goods><span class=goods_id>".$v2['barcode']."</span><span class=goods_name>".$v2['title']."</span><span class=goods_price>".$o_price.$v2['price']."</span><span class=goods_quantity>".self::format_quantity($v2['quantity']).$v2['unit']."</span><span class=goods_money>".$v2['price']*$v2['quantity']."</span></div>";	
				}
				$sql="select * from ".self::$table_pre."shop where `id`=".SHOP_ID;
...
if($act=='add_checkout'){
	...
	$r=$pdo->query($sql,2)->fetch(2);
	$url='http://'.self::$config['web']['domain'].'/index.php?jzdc=mall.checkout_pay|||id=';
	if($r['id']!=''){
...
if($act=='submit'){
	...
	<td class=goods_td'.$have_spec.'><a href=./index.php?jzdc=mall.goods&id='.$r['id'].' class=goods_info target=_blank><span class=title>'.$g[$key]['title'].'</span><span class=spec>'.$spec_info.'</span></a></td>
