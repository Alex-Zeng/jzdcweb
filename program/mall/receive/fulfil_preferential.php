...
if($act=='update'){
	...
	if(intval($_POST['id'])==0){
		$sql="insert into ".self::$table_pre."fulfil_preferential (`shop_id`,`discount`,`start_time`,`end_time`,`time`,`username`,`use_method`,`less_money`,`min_money`,`join_goods`) values ('".SHOP_ID."','".$discount."','".$start_time."','".$end_time."','".time()."','".$_SESSION['jzdc']['username']."','".$use_method."','".$less_money."','".$min_money."','".intval(@$_POST['join_goods'])."')";
	}else{
		$sql="update ".self::$table_pre."fulfil_preferential set `discount`='".$discount."',`start_time`='".$start_time."',`end_time`='".$end_time."',`time`='".time()."',`username`='".$_SESSION['jzdc']['username']."',`use_method`='".$use_method."',`join_goods`='".intval(@$_POST['join_goods'])."',`less_money`='".$less_money."',`min_money`='".$min_money."' where `id`='".intval($_POST['id'])."' and `shop_id`=".SHOP_ID;	
	}
	//echo $sql;
	if($pdo->exec($sql)){
