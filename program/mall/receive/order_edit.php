...
if($act=='edit_quantity'){
	...
	if($pdo->exec($sql)){
		...
		if($pdo->exec($sql)){
			...
			if($pdo->exec($sql)){
				$sql="insert into ".self::$table_pre."goods_quantity_log (`goods_id`,`quantity`,`username`,`time`) values ('".$r['goods_id']."','-".$change_quantity."','jzdc','".time()."')";
				$pdo->exec($sql);
			}
...
switch($act){
	...
	case 'order_state_2'://================================================================================================================【设为已发货】
		...
		push_send_order_info($pdo,self::$config,self::$language,$r['buyer'],$title,$r['out_id'],self::get_express_name($pdo,self::$table_pre,$r['express']),$r['express_code'],$url);		  
				...
				$msg.='<a href=http://'.self::$config['web']['domain'].'/index.php?jzdc=mall.my_order&search='.$r['out_id'].' target=_blank>http://'.self::$config['web']['domain'].'/index.php?jzdc=mall.my_order&search='.$r['out_id'].'</a>';
