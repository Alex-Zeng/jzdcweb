...
if($act=='add'){
	...
	foreach($usernames as $v){
		...
		if($pdo->exec($sql)){			
			if($balance>0){
				$sql="insert into ".$pdo->index_pre."recharge (`username`,`money`,`time`,`state`,`title`,`return_url`,`pay_info`,`pay_photo`,`method`) values ('".$v."','".$balance."','".$time."','4','".self::$language['operator'].$_SESSION['jzdc']['username']."','','','','offline_payment')";
				if($pdo->exec($sql)){
