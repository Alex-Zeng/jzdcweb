...
if($act=='add'){
	...
	$sql="insert into ".$pdo->index_pre."user (`username`,`nickname`,`phone`,`email`,`password`,`transaction_password`,`reg_time`,`state`,`group`,`introducer`,`openid`) values ('".$username."','".$username."','".$phone."','".$email."','".md5($_POST['password'])."','".md5($_POST['transaction_password'])."','".time()."','1','".$group."','".$introducer."','".$openid."')";
	if($pdo->exec($sql)){
