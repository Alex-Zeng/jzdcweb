...
if($act=='update'){	
	...
	foreach($cashier_a as $v){
		...
		if($v==$_SESSION['jzdc']['username']){exit("{'state':'fail','info':'<span class=fail>".self::$language['can_not_be'].self::$language['shop_master']."</span>','key':'cashier'}");}
	...
	foreach($storekeeper_a as $v){
		...
		if($v==$_SESSION['jzdc']['username']){exit("{'state':'fail','info':'<span class=fail>".self::$language['can_not_be'].self::$language['shop_master']."</span>','key':'storekeeper'}");}
