...
if($act=='add'){
	...
	if($pdo->exec($sql)){
		...
		$sql="insert into ".self::$table_pre."goods_loss (`goods_id`,`s_id`,`quantity`,`money`,`reason`,`time`,`username`,`shop_id`) values ('".$id."','".$option_id."','".$quantity."','".(self::get_cost_price($pdo,$goods_id)*$quantity)."','".$reason."','".$time."','".$_SESSION['jzdc']['username']."','".SHOP_ID."')";
		if($pdo->exec($sql)){
