...
if($act=='bidding'){
	...
	$time=time()-86400;	
	$ip=get_ip();
	if(@$_SESSION['jzdc']['username']!=''){
		$sql="select `id` from ".self::$table_pre."bidding where `gid`=".$id." and `time`>".$time." and `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
	}else{
	...
	$r=$pdo->query($sql,2)->fetch(2);
	$seller=$r['username'];
	if(@$_SESSION['jzdc']['username']==$seller){exit('self');}
	...
	$reason=str_replace('{goods_name}','<a href=./index.php?jzdc=mall.goods&id='.$id.' target=_blank>'.$goods_title.'</a>',self::$language['bidding_show_pay_reason']);
	if(operator_money(self::$config,self::$language,$pdo,$seller,'-'.$money,$reason,'mall')){
		$temp=explode('jzdc=',$_POST['src_url']);
		$_POST['src_url']=@$temp[1];
		$sql="insert into ".self::$table_pre."bidding (`gid`,`time`,`src_title`,`src_url`,`username`,`money`,`shop_id`,`ip`) values ('".$id."','".time()."','".$_POST['src_title']."','".$_POST['src_url']."','".@$_SESSION['jzdc']['username']."','-".$money."','".$shop_id."','".$ip."')";
		$pdo->exec($sql);
