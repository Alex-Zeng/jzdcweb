...
if($act=='add_favorite'){
	...
	if(!isset($_SESSION['jzdc']['username'])){
		if($_COOKIE['jzdc_device']=='pc'){
			exit("{'state':'fail','info':'<a href=./index.php?jzdc=index.login class=fail>".self::$language['please_login']."</a>'}");
		}else{
			exit("{'state':'fail','info':'login'}");	
		}
	}else if($_SESSION['jzdc']['group_id']=='6'){
        if($_COOKIE['jzdc_device']=='pc'){
            exit("{'state':'fail','info':'<a href=./index.php?jzdc=index.user class=fail>请先完成企业认证</a>'}");
        }else{
            exit("{'state':'fail','info':'upgrade'}");
        }
    }
	
	$sql="select `id` from ".self::$table_pre."favorite where `goods_id`='".$id."' and `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['id']!=''){
		$sql="update ".self::$table_pre."favorite set `time`='".time()."' where `id`=".$r['id'];
	}else{
		$sql="insert into ".self::$table_pre."favorite (`username`,`goods_id`,`time`) values ('".$_SESSION['jzdc']['username']."','".$id."','".time()."')";	
	}
	if($pdo->exec($sql)){
...
if($act=='get_sold'){
	...
	$sql="select `id`,`title`,`quantity`,`time`,`price`,`buyer` from ".self::$table_pre."order_goods where `goods_id`='".$id."' and `time`>".$start_time." and `id`<".$last_id." order by `id` desc limit 0,".self::$config['monthly_sold_page_size'];
	$r=$pdo->query($sql,2);
	$temp='';
	foreach($r as $v){
		$v=de_safe_str($v);
...
if($id>0){
	...
	if(isset($_SESSION['jzdc']['username'])){
		$sql="select `id` from ".self::$table_pre."visit where `goods_id`='".$id."' and `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
		$r=$pdo->query($sql,2)->fetch(2);
		if($r['id']==''){
			$sql="insert into ".self::$table_pre."visit (`goods_id`,`username`,`time`) values ('".$id."','".$_SESSION['jzdc']['username']."','".time()."')";	
		}else{
