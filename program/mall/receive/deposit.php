...
if($act=='add_deposit'){
	...
	$sql="select `money` from ".$pdo->index_pre."user where `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['money']<$money){exit("{'state':'fail','info':'<span class=fail>".self::$language['insufficient_balance'].",".self::$language['please'].' <a href=./index.php?jzdc=index.recharge target=_blank>'.self::$language['recharge']."</a> </span>'}");}
	...
	if(operator_money(self::$config,self::$language,$pdo,$_SESSION['jzdc']['username'],'-'.$money,$reason,'mall')){
