...
if($act=='pay_agent'){
	...
	$reason='<a href=./index.php?jzdc=mall.shop_index&shop_id='.$id.' target=_blank>'.$r['name'].'</a> '.self::$language['finance_type_agent'][0];
...
if($act=='update'){	
	...
	$sql="update ".self::$table_pre."shop set `state`='".$state."',`sequence`='".$sequence."',`trusteeship`='".$trusteeship."' where `id`=".$id;
	if($pdo->exec($sql)){
