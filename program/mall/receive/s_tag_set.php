...
if($act=='submit'){
	...
	if($pdo->exec($sql)){
		$html=self::$language['tag'].':'.self::$language['store'].self::$language['tag'].": ".self::get_shop_tags_name($pdo,$tag).' <a href=./index.php?jzdc=mall.s_tag_set&c_id='.$_GET['c_id'].'&id='.$tag.' class=set>&nbsp;</a>';
