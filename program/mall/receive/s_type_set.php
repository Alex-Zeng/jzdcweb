...
if($act=='save'){
	...
	if($pdo->exec($sql)){
		$html=self::$language['mall'].self::$language['type'].": ".str_replace('/a>','/a> >',self::get_type_position($pdo,$type)).' <a href=./index.php?jzdc=mall.s_type_set&c_id='.$_GET['c_id'].'&id='.$type.' class=set>&nbsp;</a>';
