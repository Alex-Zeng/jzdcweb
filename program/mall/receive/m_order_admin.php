...
switch ($act){
	...
	case 'force_refund':
		...
		if($r['receipt']==1  && ($r['pay_method']=='balance' || $r['pay_method']=='online_payment') ){
			...
			$reason=str_replace('{order_id}','<a href=./index.php?jzdc=mall.order_admin&search='.$r['out_id'].' target=_blank>'.$r['out_id'].'</a>',$reason);
		...
		if($r['pay_method']=='balance' || $r['pay_method']=='online_payment' ){
			if($r['actual_money']==$r['refund_amount']){$reason=self::$language['refund_money_template_all'];}else{$reason=self::$language['refund_money_template_part'];}
			$reason=str_replace('{order_id}','<a href=./index.php?jzdc=mall.my_order&search='.$r['out_id'].' target=_blank>'.$r['out_id'].'</a>',$reason);
			$reason=str_replace('{sum_money}',$r['refund_amount'],$reason);
		...
		if($pdo->exec($sql)){
			if($r['pay_method']=='balance' || $r['pay_method']=='online_payment' ){
				...
				$r2=$pdo->query($sql,2)->fetch(2);
				$content=self::$language['view'].' <a href="http://'.self::$config['web']['domain'].'/index.php?jzdc=mall.index.php?jzdc=index.money_log"  target=_blank>http://'.self::$config['web']['domain'].'/index.php?jzdc=index.money_log</a>';
				email(self::$config,self::$language,$pdo,'jzdc',$r2['email'],$title,$content);	
			}
