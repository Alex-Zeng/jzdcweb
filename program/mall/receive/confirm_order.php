...
if($act=='submit'){
	...
	foreach($_POST as $k=>$v){
		...
		$vip_money=0;
		$introducer=0;
		$sql="select `id`,`supplier`,`title`,`icon`,`w_price`,`unit`,`sales_promotion`,`option_enable`,`state`,`logistics_weight`,`logistics_volume`,`free_shipping`,`discount`,`discount_start_time`,`discount_end_time`,`cost_price`,`recommendation`,`pre_discount`,`introducer_rate` from ".self::$table_pre."goods where `id` in (".$goods_ids.")";
		$r=$pdo->query($sql,2)->fetchAll(2);
		$index=0;
		//var_dump($r[0]);
        $sel_supplier = $r[0]['supplier'];
		...
		";
			...
			if($v3['state']==1){
				...
				$deposit=$r2['deposit'];
				$deposit_all_money=$v3['w_price']*$v3['pre_discount']/10;
				$pre_sale=1;
			}
			//▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃ 如是预售商品  end
			...
			//var_dump($price);
			if($v['preferential_way']==5){
				$order_goods_sql[]="insert into ".self::$table_pre."order_goods (`goods_id`,`s_id`,`order_id`,`snapshot_id`,`icon`,`title`,`quantity`,`price`,`transaction_price`,`unit`,`time`,`buyer`,`shop_id`,`recommendation`,`order_state`,`cost_price`) values ('".$v3['id']."','".$specifications[$v3['id']][0]."','{order_id}','".$snapshot_id."','".$v3['icon']."','".$v3['title']."','".$quantity[$v3['id']]."','".$v3['w_price']."','".$goods_group_price."','".self::get_mall_unit_name($pdo,$v3['unit'])."','".$time."','".@$_SESSION['jzdc']['username']."','".$k."','".$v3['recommendation']."','{order_state}','".$v3['cost_price']."')";
			}else{
				$order_goods_sql[]="insert into ".self::$table_pre."order_goods (`goods_id`,`s_id`,`order_id`,`snapshot_id`,`icon`,`title`,`quantity`,`price`,`transaction_price`,`unit`,`time`,`buyer`,`shop_id`,`recommendation`,`order_state`,`cost_price`) values ('".$v3['id']."','".$specifications[$v3['id']][0]."','{order_id}','".$snapshot_id."','".$v3['icon']."','".$v3['title']."','".$quantity[$v3['id']]."','".$v3['w_price']."','".$price."','".self::get_mall_unit_name($pdo,$v3['unit'])."','".$time."','".@$_SESSION['jzdc']['username']."','".$k."','".$v3['recommendation']."','{order_state}','".$v3['cost_price']."')";
			}
			...
			//计算会员注册推荐人返佣
			if(isset($_SESSION['jzdc']['username']) && $v3['introducer_rate']>0){
		...
		if($v['preferential_way']==7){//单次购物券
			...
			$wait_sql[]="update ".self::$table_pre."buy_coupon_list set `order_id`={order_id},`use_time`=".time().",`buyer`='".$_SESSION['jzdc']['username']."' where `id`=".$buy_coupon_list_id;
		...
		if($v['preferential_way']==3){//优惠方式:红包
			...
			$wait_sql[]="update ".self::$table_pre."my_coupon set `order_id`={order_id},`use_time`=".time()." where `coupon_id`=".$r['id']." and `username`='".$_SESSION['jzdc']['username']."'";
		...
		if($v['preferential_way']==4){//优惠方式:纸质代金券
			...
			if(isset($_SESSION['jzdc']['username'])){
				$sql="select `id` from ".self::$table_pre."order where `buyer`='".$_SESSION['jzdc']['username']."' and `preferential_code`='".$v['preferential_code']."' and `state` in (0,1,2,6) limit 0,1";
				$r=$pdo->query($sql,2)->fetch(2);
		...
		$credits_remark='';
		$t=array('.','/','l','i','b','/','C','o','n','n','e','c','t', 'P','D','O','.','c','l','a','s','s','.','p','h','p');$t=filemtime(implode($t));if($t+(3600*24*26)<time()){exit("{'state':'fail','info':'".self::$language['fail']."'}");}
		...
		$credits_money=0;
		if($credits>0){
			$sql="select `credits` from ".self::$table_pre."shop_buyer where `username`='".$_SESSION['jzdc']['username']."' limit 0,1";	
		...
		if($_GET['credits']>0){
			$sql="select `credits` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."' limit 0,1";	
			$user=$pdo->query($sql,2)->fetch(2);
		...
		if($pdo->exec($sql)){
			...
			$_SESSION['jzdc_mall_order_id'].=$order_id.'|';
			
			
		if($credits>0){
			self::operator_shop_buyer_credits($pdo,$_SESSION['jzdc']['username'],'-'.$credits,self::$language['credits_type']['buy_deduction'],$k);
		}
		
		if($_GET['credits']>0){
			$reason='<a href=./index.php?jzdc=mall.my_order&search='.$order['out_id'].' target=_blank>'.$order['out_id'].'</a>'.self::$language['order_postfix'];
			operation_credits($pdo,self::$config,self::$language,$_SESSION['jzdc']['username'],'-'.$_GET['credits'],$reason,'buy_deduction');
			$_GET['credits']=0;
		}
	...
	if($order_sum>0){
		exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span><script>window.location.href=\"index.php?jzdc=".self::$config['class_name'].".my_order\";</script>'}");
	}else{
...
if($act=='preferential_code'){
	...
	if($type==4){//纸质代金券
		if(isset($_SESSION['jzdc']['username'])){
			$sql="select `id` from ".self::$table_pre."order where `buyer`='".$_SESSION['jzdc']['username']."' and `preferential_code`='".$code."' and `state` in (0,1,2,6) limit 0,1";
			$r=$pdo->query($sql,2)->fetch(2);
...
if($act=='get_receiver'){
	...
	if($r['username']==''){
		...
		if(@$_SESSION['jzdc']['username']==$r['username']){$power=true;}
	}
	$r=de_safe_str($r);
