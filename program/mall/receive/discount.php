...
if($r['id']==''){
	$sql="insert into ".self::$table_pre."discount (`shop_id`,`rate`,`start_time`,`end_time`,`time`,`username`,`join_goods`) values ('".SHOP_ID."','".$rate."','".$start_time."','".$end_time."','".time()."','".$_SESSION['jzdc']['username']."','".intval(@$_POST['join_goods'])."')";
}else{
	$sql="update ".self::$table_pre."discount set `rate`='".$rate."',`start_time`='".$start_time."',`end_time`='".$end_time."',`time`='".time()."',`username`='".$_SESSION['jzdc']['username']."',`join_goods`='".intval(@$_POST['join_goods'])."' where  `shop_id`=".SHOP_ID;	
}
if($pdo->exec($sql)){
