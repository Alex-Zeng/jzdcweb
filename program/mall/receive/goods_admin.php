...
if($act=='update_bidding_show'){
	...
	$sql="select `money` from ".$pdo->index_pre."user where `username`='".$_SESSION['jzdc']['username']."'";
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['money']<$_GET['bidding_show']){exit("{'state':'fail','info':'<span class=fail>".self::$language['insufficient_balance'].",".self::$language['please'].' <a href=./index.php?jzdc=index.recharge target=_blank>'.self::$language['recharge']."</a> </span>'}");}
...
if($act=='mass_select'){
	...
	foreach($r as $v){
		...
		$array[$v['id']]['url']='./index.php?jzdc=mall.goods&id='.$v['id'];	
	}
...
</script><body onload="autoSubmit();" ><form id=myForm action="./index.php?jzdc=weixin.mass&wid='.self::$config['web']['wid'].'" method="post"><input type=hidden id=new name=new value=\''.$value.'\'  /><input type="submit" value="Submit" /></form></body>';
	exit;
}