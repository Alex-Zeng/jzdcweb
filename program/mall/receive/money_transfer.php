...
if($act=='submit'){
	...
	$sql="select `transaction_password` from ".$pdo->index_pre."user where `username`='".$_SESSION['jzdc']['username']."'";
	$r=$pdo->query($sql,2)->fetch(2);
	...
	$sql="select `balance` from ".self::$table_pre."shop_buyer where `shop_id`=".$shop_id." and `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
	$r=$pdo->query($sql,2)->fetch(2);
	...
	if(self::operator_shop_buyer_balance($pdo,$_SESSION['jzdc']['username'],'-'.$money,$reason)){
		$reason=$_SESSION['jzdc']['username'].self::$language['transfer'].'('.$remark.')';
