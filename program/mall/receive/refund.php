...
if($act=='refund'){
	...
	foreach($r as $v){
		...
		$pdo->exec($sql);
		//新增退货记录
		$sql="insert into ".self::$table_pre."refund (`shop_id`,`order_id`,`out_id`,`goods_id`,`s_id`,`quantity`,`username`,`time`,`reason`,`buyer`) value ('".SHOP_ID."','".$order['id']."','".$order['out_id']."','".$v['goods_id']."','".$v['s_id']."','".floatval($_POST[$v['id']])."','".$_SESSION['jzdc']['username']."','".$time."','".safe_str($_GET['reason'])."','".$order['buyer']."')";
		$pdo->exec($sql);		
		...
		//退平台积分 退店内积分
		if($order['buyer']!=''){
			if($order['give_web_credits']>0){
				operation_credits($pdo,self::$config,self::$language,$order['buyer'],'-'.$return_money*self::$config['give_credits'],'','buy_return');
			}
			if($order['give_shop_credits']>0){
				$sql="select `group_id`,`id` from ".self::$table_pre."shop_buyer where `username`='".$order['buyer']."' and `shop_id`=".$order['shop_id']." limit 0,1";
				...
				if($user['group_id']!=''){
					...
					$number=$v['buy_credits_multiple']*$return_money;		
					self::operator_shop_buyer_credits($pdo,$order['buyer'],'-'.$number,self::$language['credits_type']['buy_return'],$order['shop_id']);
				}
			}
		}
