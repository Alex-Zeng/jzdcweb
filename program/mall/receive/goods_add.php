...
if(@$_GET['act']=='get_goods_info'){
	...
	if(self::$config['web']['jzdc_username']=='' || self::$config['web']['jzdc_password']=='' ){
		$err=array();
		$err['state']='fail';
		$err['info']='lansion.cn '.self::$language['account'].self::$language['password'].self::$language['is_null'].' , <a href=./index.php?jzdc=index.config>'.self::$language['set'].'</a>';
		exit(json_encode($err));
	}
	$r=file_get_contents('http://www.lansion.cn/receive.php?target=server.goods&username='.urlencode(self::$config['web']['jzdc_username']).'&password='.urlencode(self::$config['web']['jzdc_password']).'&act=get_goods_info&barcode='.$barcode);
	file_put_contents('url.txt','http://www.lansion.cn/receive.php?target=server.goods&username='.urlencode(self::$config['web']['jzdc_username']).'&password='.urlencode(self::$config['web']['jzdc_password']).'&act=get_goods_info&barcode='.$barcode);
	$r=trim($r);
...
if(in_array('mall.goods_admin',$_SESSION['jzdc']['function'])){$exist_view_url="goods_admin";}else{$exist_view_url="goods_db";}
...
if(self::barcode_repeat($pdo,SHOP_ID)==0){
	if($_POST['option_type']==0){
		if($_POST['bar_code']!=''){
			...
			$r=$pdo->query($sql,2)->fetch(2);
			if($r['c']>0){exit("{'state':'fail','info':'<span class=fail>".self::$language['exist_same'].self::$language['bar_code']."</span> <a href=./index.php?jzdc=mall.".$exist_view_url."&search=".$_POST['bar_code']." target=_blank class=view>".self::$language['view']."</a>','id':'bar_code'}");}
		}
	}else{
		$temp3=array();
		...
		foreach($_POST['specifications'] as $key=>$v){
			...
			if($temp3['barcode']!=''){
				...
				$r=$pdo->query($sql,2)->fetch(2);
				if($r['c']>0){exit("{'state':'fail','info':'<span class=fail>".self::$language['exist_same'].self::$language['bar_code'].": ".$temp3['barcode']." </span> <a href=./index.php?jzdc=mall.".$exist_view_url."&search=".$temp3['barcode']." target=_blank class=view>".self::$language['view']."</a>'}");}
			}
		}
...
if($_POST['option_type']==0){
	if($_POST['store_code']!=''){
		...
		$r=$pdo->query($sql,2)->fetch(2);
		if($r['c']>0){exit("{'state':'fail','info':'<span class=fail>".self::$language['exist_same'].self::$language['store_code']."</span> <a href=./index.php?jzdc=mall.".$exist_view_url."&search=".$_POST['store_code']." target=_blank class=view>".self::$language['view']."</a>','id':'store_code'}");}
	}
}else{
	$temp3=array();
	...
	foreach($_POST['specifications'] as $key=>$v){
		...
		if($temp3['store_code']!=''){
			...
			$r=$pdo->query($sql,2)->fetch(2);
			if($r['c']>0){exit("{'state':'fail','info':'<span class=fail>".self::$language['exist_same'].self::$language['store_code'].": ".$temp3['store_code']." </span> <a href=./index.php?jzdc=mall.".$exist_view_url."&search=".$temp3['store_code']." target=_blank class=view>".self::$language['view']."</a>'}");}
		}
	}
...
if($pdo->exec($sql)){
	...
	exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span><script>window.location.href=\"index.php?jzdc=".self::$config['class_name'].".goods&id=".$insret_id."\";</script>'}");
}else{
	exit("{'state':'fail','info':'<span class=fail>".self::$language['fail']."</span>'}");
}
