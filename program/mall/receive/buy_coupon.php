...
if($act=='add'){
	...
	$sql="insert into ".self::$table_pre."buy_coupon (`name`,`amount`,`min_money`,`start_time`,`end_time`,`shop_id`,`time`,`username`,`sum_quantity`,`join_goods`) values ('".$_GET['name']."','".$_GET['amount']."','".$_GET['min_money']."','".$_GET['start_time']."','".$_GET['end_time']."','".SHOP_ID."','".time()."','".$_SESSION['jzdc']['username']."','".$_GET['sum_quantity']."','".intval(@$_GET['join_goods'])."')";
	if($pdo->exec($sql)){
