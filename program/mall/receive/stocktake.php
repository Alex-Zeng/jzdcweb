...
if($act=='add'){
	...
	$sql="insert into ".self::$table_pre."stocktake (`name`,`username`,`shop_id`,`time`) values ('".$_GET['name']."','".$_SESSION['jzdc']['username']."','".SHOP_ID."','".time()."')";
	if($pdo->exec($sql)){
		...
		foreach($r as $v){
			...
			if($v['option_enable']){
				...
				foreach($t as $vv){
					...
					$sql="insert into ".self::$table_pre."stocktake_goods (`goods_id`,`s_id`,`shop_id`,`title`,`stocktake_id`,`username`,`shop_type`,`position`) values ('".$v['id']."','".$vv['id']."','".SHOP_ID."','".$v['title']."','".$stocktake_id."','".$_SESSION['jzdc']['username']."','".$v['shop_type']."','".$v['position']."')";
					$pdo->exec($sql);	
				}	
			}else{
				$sql="insert into ".self::$table_pre."stocktake_goods (`goods_id`,`s_id`,`shop_id`,`title`,`stocktake_id`,`username`,`shop_type`,`position`) values ('".$v['id']."','0','".SHOP_ID."','".$v['title']."','".$stocktake_id."','".$_SESSION['jzdc']['username']."','".$v['shop_type']."','".$v['position']."')";
				$pdo->exec($sql);	
			}
		}
