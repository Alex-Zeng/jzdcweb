...
if($act=='submit_checkout'){
	...
	if($pay_method=='cash' && is_numeric($_POST['received_money'])){
		...
		if($received_money<$payable  && !jzdc_number_difference($pay['money'],$payable,0.3)){exit('{"state":"fail","info":"<span class=fail>received_money err</span>"}');}
...
if($act=='submit'){
	...
	if(is_numeric($barcode)){
		...
		<td class=goods_td><a href=./index.php?jzdc=mall.goods&id='.$r['id'].' class=goods_info target=_blank>
