<?php
if(!isset($_SESSION['jzdc']['username'])){exit('unlogin');}
$act=@$_GET['act'];
...
if($act=='add'){
	...
	*/
	$sql="select `state` from ".self::$table_pre."shop where `username`='".$_SESSION['jzdc']['username']."' order by `id` desc limit 0,1";
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['state']!=''){
		switch($r['state']){
			case 0:
	...
	if($pdo->exec($sql)){
		...
		$content=$title;
		email(self::$config,self::$language,$pdo,'jzdc',self::$config['mall_master_email'],$title,$content);
