...
if($act=='submit'){
	...
	if($pdo->exec($sql)){
		$html=self::$language['store'].self::$language['type'].": ".str_replace('/a>','/a> >',self::get_shop_type_position($pdo,intval($_GET['shop_type']))).' <a href=./index.php?jzdc=mall.s_shop_type_set&c_id='.$_GET['c_id'].'&id='.intval($_GET['shop_type']).' class=set>&nbsp;</a>';
