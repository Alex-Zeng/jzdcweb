...
self::update_config_file('type_update_time',time());
		...
		if($act=='update'){
			...
			$_GET['name']=safe_str(@$_GET['name']);
			$_GET['url']=safe_str(@$_GET['url']);
			$sql="update ".self::$table_pre."type set `parent`='".$_GET['parent']."',`name`='".$_GET['name']."',`url`='".$_GET['url']."',`sequence`='".$_GET['sequence']."',`visible`='".$_GET['visible']."' where `id`='".$_GET['id']."'";
			if($pdo->exec($sql)){
		...
		if($act=='submit_select'){
			...
			foreach($_POST as $v){
				...
				$sql="update ".self::$table_pre."type set `parent`='".$v['parent']."',`name`='".$v['name']."',`url`='".$v['url']."',`sequence`='".$v['sequence']."' where `id`='".$v['id']."'";
