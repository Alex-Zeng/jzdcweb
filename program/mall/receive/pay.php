<?php
$id=intval(@$_GET['id']);
if($id==0 && @$_SESSION['jzdc_mall_order_id']==''){exit("{'state':'fail','info':'id err'}");}
$ids=array();
$for_id=$id;
if($id==0){
...
if(isset($_SESSION['jzdc']['username'])){
	if($_SESSION['jzdc']['username']!=$r['buyer']){exit("{'state':'fail','info':'<span class=fail>".self::$language['act_noPower']."</span>'}");}	
}else{
...
if($act=='get_verification_code'){
	authcode_push($pdo,self::$config,self::$language,$_SESSION['jzdc']['username']);
	
	/*
	$sql="select `phone` from ".$pdo->index_pre."user where `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
	$r=$pdo->query($sql,2)->fetch(2);
	$phone=$r['phone'];
	...
	if(sms(self::$config,self::$language,$pdo,'jzdc',$phone,$_SESSION['verification_code'])){
...
if($pay_method=='online_payment'){
	if(count($ids)>1){
		...
		foreach($r2 as $v){
			...
			$r['id'].=$v['id'].',';
			$r['id_html'].='<a href="./index.php?jzdc=mall.my_order&id='.$v['id'].'" target=_blank >'.$v['id'].'</a>,';
		}
		$r['id']=trim($r['id'],',');
		$id=0;
	}
	...
	$return_url='http://'.self::$config['web']['domain'].'/index.php?jzdc=mall.pay&id='.$id.'&act=online_payment';
	$return_function='mall.update_order_state';
	$sql="insert into ".$pdo->index_pre."recharge (`username`,`money`,`time`,`state`,`title`,`return_url`,`pay_info`,`pay_photo`,`method`,`for_id`,`type`,`return_function`) values ('".@$_SESSION['jzdc']['username']."','".$r['actual_money']."','".time()."','2','".$_POST['title']."','".$return_url."','','','online_payment','".$for_id."','".safe_str($_POST['payment'])."','".$return_function."')";
	if($pdo->exec($sql)){
...
if($pay_method=='balance'){
	switch(self::$config['web']['balance_pay_check']){
		...
		case 'password':
			...
			$sql="select `transaction_password` from ".$pdo->index_pre."user where `id`=".$_SESSION['jzdc']['id'];
			$r=$pdo->query($sql,2)->fetch(2);
	...
	foreach($r as $v){
		...
		if(operator_money(self::$config,self::$language,$pdo,$_SESSION['jzdc']['username'],'-'.$v['actual_money'],$reason,'mall')){
			...
			if($pdo->exec($sql)){
				...
				self::update_card_state($pdo,$_SESSION['jzdc']['username']);
	...
	if($success_sum>0){
		if($automatic_delivery>0){exit("{'state':'success','info':'<span class=success>".self::$language['success'].",".self::$language['have_automatic_delivery']."</span>  <a href=./index.php?jzdc=mall.my_order class=view style=\'color:#000;\'>".self::$language['view']."</a>'}");	}
		$_SESSION['verification_code']='';
		exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span> <a href=./index.php?jzdc=mall.my_order class=view>".self::$language['view']."</a>'}");
	}else{
