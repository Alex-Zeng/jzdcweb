...
if($act=='import'){
	...
	foreach($goods as $k=>$v){
		...
		$time=time();
		$sql="insert into ".self::$table_pre."goods (`type`,`unit`,`logistics_weight`,`free_shipping`,`supplier`,`title`,`icon`,`detail`,`time`,`username`,`bar_code`,`w_price`,`e_price`,`cost_price`,`inventory`,`option_enable`,`shelf_life`,`shop_id`,`state`,`store_code`,`grade`,`contain`,`habitat`,`share`,`brand`,`mall_state`,`shop_type`) values ('".intval($vv[13])."','".intval($vv[5])."','".floatval($vv[6])."','0','".intval($vv[14])."','".$vv[0]."','default.png','".$vv[9]."','".$time."','".$_SESSION['jzdc']['username']."','".safe_str($vv[1])."','".floatval($vv[3])."','".floatval($vv[3])."','".floatval($vv[4])."','".floatval($vv[10])."','0','".intval($vv[11])."','".SHOP_ID."','2','".$vv[2]."','".$vv[8]."','".$vv[6]."','".$vv[7]."','".$share."','".$vv[15]."','".self::$config['goods_check']."','".$vv[16]."')";
