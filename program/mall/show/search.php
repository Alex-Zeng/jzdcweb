...
foreach($temp as $v){
	if($v==''){continue;}
	$module['hot_search'].='<a href="./index.php?jzdc=mall.goods_list&search='.urlencode($v).'&click=true">'.$v.'</a> ';	
}
...
$module['jzdc_search_placeholder']=self::$config['web']['search_placeholder'];
...
$list='';
foreach($r as $v){
	$list.='<a href="./index.php?jzdc=mall.goods_list&search='.urldecode($v).'">'.$v.'</a>';	
}
$module['list']=$list;
