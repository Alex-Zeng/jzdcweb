<?php
if(!isset($_SESSION['jzdc']['username'])){echo "<script>window.location.href='./index.php?jzdc=index.login';</script>";return false;}
...
$sql="select `word_id` from ".self::$table_pre."interest_word_user where `username`='".$_SESSION['jzdc']['username']."' order by `last_time` desc";
	$sum_sql=$sql;
...
foreach($r as $v){
	...
	foreach($r2 as $v){
		...
		$list.="<div class=goods id=g_".$v['id']."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=goods_a><img src='./program/mall/img_thumb/".$v['icon']."' /><span class=title>".$v['title']."</span><span class=price_span>".$w_price."/".self::get_mall_unit_name($pdo,$v['unit'])."</span></a></div>";
		$i2++;
		if($i2>1){$owe--;}
	}
	////////echo $i2.'<hr />';
