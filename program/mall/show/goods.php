...
if($module['data']['mall_state']!=1){
	if(!isset($_SESSION['jzdc']['group_id'])){echo '<div style="text-align:center;line-height:100px;"><span style="border:#CCC 1px solid; padding:10px;">'.str_replace('{goods_state}',self::$language['mall_state'][$module['data']['mall_state']],self::$language['goods_check_notice']).'</span></div>';return false;}	
	
	if(in_array('mall.m_goods_admin',$_SESSION['jzdc']['page']) || in_array('mall.goods_admin',$_SESSION['jzdc']['page']) || in_array('mall.goods_db',$_SESSION['jzdc']['page'])){
...
if(self::$config['agency'] && isset($_GET['store_id'])){
	...
	$module['agency_store']='<style>'.file_get_contents('./templates/0/agency/'.$agency_config['program']['template_0'].'/'.$_COOKIE['jzdc_device'].'/shop_style/'.$r['template'].'/main.css').'</style>';
	if($_COOKIE['jzdc_device']=='pc'){
		$module['agency_store'].='<div id="agency_head">
    <div style="text-align:center;">
        <div class="default_value">
            <a class="s_logo" href="index.php?jzdc=agency.shop&store_id='.$_GET['store_id'].'"><img src="program/agency/shop_icon/'.$r['icon'].'" /></a><span class="s_name">'.$r['name'].'</span><span class="s_name_postfix">'.self::$language['the_store'].'</span> 
        </div>
    </div>
</div>    
';	
 ...
 <br /><div class="goods_store_info">
        <a class="s_logo" href="index.php?jzdc=agency.shop&store_id='.$_GET['store_id'].'"><img src="program/agency/shop_icon/'.$r['icon'].'"></a><div class=info>
        	<div class=store_name><span class="s_name">'.$r['name'].'</span><span class="s_name_postfix">'.self::$language['the_store'].'</span> </div>
            <div class=buttons>
            	<a href="index.php?jzdc=agency.shop&store_id='.$_GET['store_id'].'" class=goods_type>'.self::$language['view_all_goods'].'</a><a href=./index.php?jzdc=agency.spread&store_id='.$_GET['store_id'].' class=qr_code>'.self::$language['qr_code'].'</a>
            </div>
        </div>
    </div>
';	
...
if(isset($_SESSION['jzdc'][SHOP_ID.'_group_id'])){
	$sql="select `discount` from ".self::$table_pre."goods_group_discount where `goods_id`=".$id." and `group_id`=".$_SESSION['jzdc'][SHOP_ID.'_group_id']." limit 0,1";
...
if($module['data']['option_enable']){
	...
	if($module['data']['state']==1){
		...
		//---如是店内会员，并折扣大于 店铺限时折扣
		if(isset($_SESSION['jzdc'][SHOP_ID.'_discount']) and ($_SESSION['jzdc'][SHOP_ID.'_discount']<10 || $goods_group_discount<10) and ($_SESSION['jzdc'][SHOP_ID.'_discount'] < $discount || $goods_group_discount< $discount)  and $module['data']['sales_promotion']){
			if($goods_group_discount==100){$goods_group_discount=$_SESSION['jzdc'][SHOP_ID.'_discount'];}
			...
			$module['price_div_html']='<div id=have_discount>
								...
								<div class=discount_price><span class=price_label>'.$_SESSION['jzdc'][SHOP_ID.'_shop_group'].self::$language['price2'].':</span><span class=money_symbol>'.self::$language['money_symbol'].'</span><span class=price_value>'.$temp.'</span>  <span class=discount_value><span class=discount_number>'.$goods_group_discount.'</span>'.self::$language['discount'].'</span></div>
	...
	if($module['data']['state']==1){
		...
		<div class=pre_div>
			<div class=pre_price><span class=price_label>'.self::$language['pre_price1'].'</span><span class=money_symbol>'.self::$language['money_symbol'].'</span><span class=price_value>'.sprintf("%.2f",$module['data']['w_price']*$module['data']['pre_discount']/10).'</span></div>
			<!--<div class=pre_deposit><span class=price_label>'.self::$language['deposit2'].'</span><span class=money_symbol>'.self::$language['money_symbol'].'</span><span class=deposit_value>'.$module['data']['deposit'].'</span><span class=reduction_div>'.self::$language['deposit2'].self::$language['reduce'].'<span class=reduction> '.self::$language['money_symbol'].$module['data']['reduction'].'</span></span><a href="#" class=pre_sale_rules>'.self::$language['pre_sale_rules'].'</a></div>-->
		</div>
		</div>';		
	}else{
...
if(isset($_SESSION['jzdc']['group_id'])){
	$sql="select `credits` from ".self::$table_pre."group_set where `group_id`=".$_SESSION['jzdc']['group_id']." limit 0,1";
...
if($module['data']['relevance_package']!=''){
	...
	foreach($r as $v){
		...
		foreach($r2 as $v2){
			...
			$temp[$v2['id']]='<div class=goods_div id=goods_div_'.$v2['id'].'><a href="./index.php?jzdc=mall.goods&id='.$v2['id'].'" target=_blank class=goods_a><img src="./program/mall/img_thumb/'.$v2['icon'].'" /><div>'.$w_price.'<br /><span class=goods_name>'.$v2['title'].'</span></div></a></div><div class="add_symbol">&nbsp;</div>';	
		}
		...
		<a href="./index.php?jzdc=mall.package&id='.$v['id'].'" target=_blank class=view user_color=button>'.self::$language['view'].self::$language['package'].'</a>
		</div></div>';	
	}
...
if($module['data']['relevance_goods']!=''){
	...
	foreach($r2 as $v){
		...
		$temp[$v['id']]='<a href="./index.php?jzdc=mall.goods&id='.$v['id'].'" target=_blank class=goods_a><img src="./program/mall/img_thumb/'.$v['icon'].'" /><div>'.$w_price.'<br /><span class=title>'.$v['title'].'</span></div></a>';	
	}
...
if(self::$config['show_sold']){
	...
	$sql="select `id`,`title`,`quantity`,`time`,`price`,`buyer` from ".self::$table_pre."order_goods where `goods_id`='".$module['data']['id']."' and `time`>".$start_time." order by `id` desc limit 0,".self::$config['monthly_sold_page_size'];
	$r=$pdo->query($sql,2);
	$temp='';
	foreach($r as $v){
		$v=de_safe_str($v);
		...
		if($v['buyer']==''){$v['buyer']=self::$language['tourist'];}else{$v['buyer']=encryption_str($v['buyer']);}
		if($_COOKIE['jzdc_device']!='pc'){
...
$module['data']['detail']=preg_replace("#<img(.*)src#iUs","<img$1src",$module['data']['detail']);
...
$module['position']="<span id=current_position_text>".self::$language['current_position']."</span><a href='./index.php?jzdc=mall.shop_index&shop_id=".SHOP_ID."'><span id=visitor_position_icon>&nbsp;</span>".self::$config['shop_name']."</a><a href=./index.php?jzdc=mall.shop_goods_list&shop_id=".SHOP_ID.">".self::$language['all_goods']."</a>";
...
if($_COOKIE['jzdc_device']=='phone'){
	$sql="select `id`,`name`,`domain`,`evaluation_0`,`evaluation_1`,`evaluation_2`,`talk_type`,`talk_account`,`deposit` from ".self::$table_pre."shop where `id`=".SHOP_ID;
	$r=$pdo->query($sql,2)->fetch(2);
    ...
    //check if exist in my collect
    $sql="select `id` from ".self::$table_pre."favorite where `username`='".$_SESSION['jzdc']['username']."' and `goods_id`='".$id."' limit 1";
    $r3=$pdo->query($sql,2)->fetch(2);
    if ($r3){
        $module['favorite']="<a href='./index.php?jzdc=mall.my_collect' class='favorite' >已收藏</a>";
        $module['favorite_css']=".goods_bottom_div .favorite:before{ display:block; font: normal normal normal 1rem/1 FontAwesome; content:\"\\f005\";}";
    }else{
        $module['favorite']="<a href='#' class='favorite' >".self::$language['favorite']."</a>";
        $module['favorite_css']=".goods_bottom_div .favorite:before{ display:block; font: normal normal normal 1rem/1 FontAwesome; content:\"\\f006\";}";
    }
}

//查询供应商名称
if($module['data']['supplier'] > 0){
    $sql="select real_name from jzdc_index_user where `id`=".$module['data']['supplier'];
