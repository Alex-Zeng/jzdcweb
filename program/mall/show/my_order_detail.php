<?php
if($_COOKIE['jzdc_device']=='pc'){header('location:index.php?jzdc=mall.my_order&id='.$_GET['id']);exit;}
...
$sql="select * from ".self::$table_pre."order where `buyer`='".$_SESSION['jzdc']['username']."' and `state`!=9";
...
foreach($r as $v){
	...
	foreach($r2 as $v2){
		...
		if($v['state']==8){//comment
        	...
        	<span class=icon>
            	<a href=./index.php?jzdc=mall.goods&id=".$v2['goods_id']."><img src=./program/mall/order_icon/".$v2['icon']." /></a>
            </span><span class=other>
            	<div class=title>".$v2['title']."</div>
                <div class=price>".self::$language['price'].":".$o_price.self::$language['money_symbol'].str_replace('.00','',$v2['price'])." &nbsp; ".self::$language['quantity'].":".self::format_quantity($v2['quantity']).$v2['unit']." <a href=./index.php?jzdc=mall.goods_snapshot&id=".$v2['snapshot_id']." target=_balnk>".self::$language['snapshot']."</a> ".$add_comment."</div>
            </span>
	...
	switch($v['state']){
		case 0:
			...
			$state_remark='<div class=pay_time_limit>'.self::$language['pay_time_limit'].'<br />'.self::get_pay_time_limit(self::$language,self::$config['pay_time_limit'],$v['add_time']).'</div><a href="./index.php?jzdc=mall.pay&id='.$v['id'].'" class=go_pay target=_blank>'.self::$language['select_pay_method'].'</a>';
			break;
		case 1:
			$act="";
	...
	$list.="<div class=order_div id=order_".$v['id']." href='./index.php?jzdc=mall.my_order_detail&id=".$v['id']."'>
	...
	<div class=buyer_info><span class=order_id>".self::$language['order_number'].": <span class=value>".$v['out_id']."</span></span> <span class=goods_money>".self::$language['goods_money'].": <span class=value>".$goods_money2.self::$language['yuan']."</span></span> 
	<!--<span class=express_cost_buyer>".self::$language['freight_costs'].": <span class=value>".str_replace('.00','',$v['express_cost_buyer']).self::$language['yuan']."</span></span><span class=invoice>".$v['invoice']."</span> --></div>	
