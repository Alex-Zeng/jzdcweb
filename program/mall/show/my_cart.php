...
if(isset($_COOKIE['mall_cart'])){
	if($_COOKIE['mall_cart']!=''){
		...
		foreach($mall_cart as $key=>$v){
			...
			//var_dump($price);
			$favorite='';
			if(isset($_SESSION['jzdc'])){$favorite="<a href='#' class=move_to_favorites>".self::$language['move_to_favorites']."</a><br />";}		
			...
			if(!isset($shops[$r['shop_id']]['index'])){
            	...
            	<a class=name href='./index.php?jzdc=mall.shop_index&shop_id=".$r['shop_id']."' target=_blank>".$r2['name']."</a>
			...
			if($_COOKIE['jzdc_device']=='pc'){
				...
				<div class=tr id='tr_".$key."'>
                    ...
                    <a class=goods_info href=./index.php?jzdc=mall.goods&id=".$r['id']." target=_blank><img src='./program/mall/".$img."' /><div class=title_model><div class=title>".$r['title']."</div><div class=model>".$spec_info."</div></div></a>
				...
				$shops[$r['shop_id']]['shop_goods'].="<div class=goods id='goods_".$key."'>
                    <div class=checkbox_td><a  href=# class='m_checkbox goods_checkbox' d_id=".$key."></a></div><div class=goods_td>
                        <span class=icon><img src='./program/mall/".$img."' /></span><div class=goods_info>
                            <a href=./index.php?jzdc=mall.goods&id=".$r['id']." target=_blank class=title>".$r['title']."</a>
...
if($module['mall_cart']!=''){
	if($_COOKIE['jzdc_device']=='pc'){
		$module['sum_info']='<div class=sum_div>
                    ...
                    <span class=money_count_span>'.self::$language['sum'].'('.self::$language['excluding_freight_costs'].')<span class=money_symbol>'.self::$language['money_symbol'].'</span><span class=money_count>'.$money_count.'</span></span>
                	<a href="./index.php" class=continue_mallping>'.self::$language['continue_mallping'].'</a>
                	<a href="./index.php?jzdc=mall.confirm_order&goods_src=selected_goods" class=go_w_cashier user_color=button>'.self::$language['settlement'].'</a>
                </div>';
	}else{
		$module['sum_info']='<div class=sum_div><span class=money_count_span>'.self::$language['sum'].': <span class=money_symbol>'.self::$language['money_symbol'].'</span><span class=money_count>'.$money_count.'</span></span></div>';
	}
...
if(isset($_SESSION['jzdc'])){$module['operation_td_line_height']=10;}else{$module['operation_td_line_height']=45;}
