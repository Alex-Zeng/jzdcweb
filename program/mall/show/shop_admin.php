...
if(intval(@$_GET['id'])!=0){
	...
	echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=mall.goods_admin">'.self::$language['pages']['mall.shop_admin']['name'].'</a><span class=text>'.$_GET['id'].'</span></div>';
}
...
foreach($r as $v){
	...
	$list.="<tr id='tr_".$v['id']."'>
	  <td><div class=info>
	  	<div class=icon><a class=icon_logo href=./index.php?jzdc=mall.shop_index&shop_id=".$v['id']." target=_blank><img src=./program/mall/shop_icon/".$v['id'].".png /></a><div class=icon_aside>
			<a href=./index.php?jzdc=mall.shop_index&shop_id=".$v['id']." target=_blank class=name>".$v['name']."</a>
			<div class=main_business>".$v['main_business']."</div>
			<div class=username>".self::$language['shop_master'].": <a href=./index.php?jzdc=index.admin_users&search=".urlencode($v['username'])."#index_admin_users_table target=_blank>".$v['username']."</a></div>
			<div class=annual_time_limit>".self::$language['annual_time_limit'].": ".get_time(self::$config['other']['date_style'],self::$config['other']['timeoffset'],self::$language,$v['annual_fees'])."</div>
			</div>
		</div>
		<div class=other>
			...
			<span class=tag>".self::$language['tag'].": ".self::get_store_tags_name($pdo,$v['tag'])." <a href=./index.php?jzdc=mall.shop_tag_set&c_id=".$v['id']."&id=".$v['tag']." class=set>&nbsp;</a></span>
		</div>
	  </div></td>
	  <td class=goods>".$v['goods']."</td>
	  <td class=order><a href='./index.php?jzdc=mall.m_order_admin&seller=".urlencode($v['username'])."&state=6&#mall_m_order_admin_table' target=_blank >".$v['order']."</a></td>
	  ...
	  <td><select class=data_state id='data_state_".$v['id']."'  jzdc_value=".$v['state']."><option value=0>".self::$language['shop_state'][0]."</option><option value=1>".self::$language['shop_state'][1]."</option><option value=2>".self::$language['shop_state'][2]."</option><option value=3>".self::$language['shop_state'][3]."</option><option value=4>".self::$language['shop_state'][4]."</option><option value=5>".self::$language['shop_state'][5]."</option></select><input type=text class=cause placeholder=".self::$language['cause'].": /></td>
	  <td><select class=trusteeship id='trusteeship_".$v['id']."'  jzdc_value=".$v['trusteeship']."><option value=0>".self::$language['no']."</option><option value=1>".self::$language['yes']."</option></select></td>
	  <td class=operation_td><a href='#' onclick='return update(".$v['id'].")'  class='submit'>".self::$language['submit']."</a><a href='./index.php?jzdc=mall.shop_edit&id=".$v['id']."' class='edit'>".self::$language['view'].' '.self::$language['edit']."</a><a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
	</tr>
";	
...
$module['batch_msg']='<a href=./index.php?jzdc=index.admin_users&group='.self::$config['shopkeeper_group_id'].'#state_select target=_blank class=submit>'.self::$language['batch_msg'].'</a>';
