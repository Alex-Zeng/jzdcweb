...
$r=$pdo->query($sql,2)->fetch(2);
if($r['transaction_password']==''){echo ("<script>window.location.href='index.php?jzdc=index.edit_user&field=transaction_password'</script>");return false;}
...
$sql="select `balance` from ".self::$table_pre."shop_buyer where `username`='".$_SESSION['jzdc']['username']."'";
$r=$pdo->query($sql,2)->fetch(2);
