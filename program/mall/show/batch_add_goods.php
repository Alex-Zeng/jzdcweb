...
if($head_id==''){echo '<div style="line-height:100px; text-align:center;">'.self::$language['set_head_notice'].' <a href=./index.php?jzdc=mall.shop_config><b>'.self::$language['set'].'</b></a></div>';return false;}
...
foreach($r as $v){
	...
	<td><div class=title><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank>".$v['title']."</a></div><div class=type_tag><span class=type>".self::$language['mall'].self::$language['type'].": ".self::get_type_position($pdo,$v['type'])." ".$edit_type."</span><span class=shop_type>".self::$language['store'].self::$language['type'].": ".self::get_shop_type_position($pdo,$v['shop_type'])." </span></div>
	</td>
