...
function get_layout($language,$v){
	...
	if(@$_GET['url']!=''){
		...
		echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=mall.edit_page_layout">'.self::$language['pages']['mall.edit_page_layout']['name'].'</a><span class=text>'.$_GET['url'].'</span></div>';
	...
	foreach($r as $v){
		...
		<td><a href=index.php?jzdc='.$v['url'].' target=_blank class=url><span class=name>'.self::$language['pages'][$v['url']]['name'].'</span><span class=url2>'.$v['url'].'</span></a></td>
	  ...
	  '.get_layout(self::$language,$v['layout']).'
		...
		<td class=operation_td><a href="./index.php?jzdc='.$v['url'].'&edit_mall_layout=true"  target=_blank class=visual_edit>'.self::$language['visual_edit'].'</a><br class=br_none /><a href=# onclick="return update('.$v['id'].')"  class=submit>'.self::$language['submit'].'</a><br class=br_none /><span id=state_'.$v['id'].' class=state></span></td></tr>
';
	}
