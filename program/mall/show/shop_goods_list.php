...
foreach($r as $v){
	...
	if($_COOKIE['jzdc_device']=='phone' && $phone_goods_list_show_buy_button==0){
		$button='';	
	}else{
		if($_COOKIE['jzdc_device']=='phone'){
			$button="<div class=button_div><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank  class=add_cart user_color=button option_enable='".$v['option_enable']."'  s_id='".$sv['s_id']."' s_price='".$sv['s_price']."'>".self::$language['add_cart']."</a></div>";
		}else{
			$button="<div class=button_div><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank  class=add_cart user_color=button option_enable='".$v['option_enable']."'  s_id='".$sv['s_id']."' s_price='".$sv['s_price']."'>".self::$language['add_cart']."</a><a href=# class=favorite>".self::$language['favorite']."</a></div>";
		}
	...
	if($show_method!='show_line'){
		...
		$list.="<div class=goods id=g_".$v['id']."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=goods_a><img src='./program/mall/img_thumb/".$v['icon']."' /><span class=title>".$v['title']."</span><span class=price_span>".$w_price."/".self::get_mall_unit_name($pdo,$v['unit'])."</span>".$sum_div."</a>".$button."</div>";
	}else{
		if($_COOKIE['jzdc_device']=='phone'){
			$list.="<div class=line id=g_".$v['id']."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=goods_img><img src='./program/mall/img_thumb/".$v['icon']."' /></a><div class=good_info><div class=top2><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=title>".$v['title']."</a><span class=price_span>".$w_price."/".self::get_mall_unit_name($pdo,$v['unit'])."</span>".$button."</div><div class=bottom>".$sum_div."</div></div></div>";
