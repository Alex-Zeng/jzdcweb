...
foreach($r as $v){
	...
	$module['data'].='<fieldset id="f_'.$v['id'].'" class="out_fieldset"><legend>'.self::$language['full'].'<input type="text" class="min_money" value="'.$v['min_money'].'" />'.self::$language['yuan'].$state.' <span class=state></span></legend>
    	<div class=input_line><span class=m_label>'.self::$language['join_goods'].'</span><span class=input_span><select class=join_goods  jzdc_value="'.$v['join_goods'].'" ><option value="1">'.self::$language['join_goods_admin'][1].'</option><option value="0">'.self::$language['join_goods_admin'][0].'</option><option value="2">'.self::$language['join_goods_admin'][2].'</option></select> <span class=state></span></span></div>
    	...
    	<div class=input_line><span class=m_label>'.self::$language['use_method'].'</span><span class=input_span><select class=use_method  jzdc_value="'.$v['use_method'].'" ><option value="-1">'.self::$language['please_select'].'</option>'.get_use_method_option(self::$language['use_method_option']).'</select> <span class=state></span></span></div>
