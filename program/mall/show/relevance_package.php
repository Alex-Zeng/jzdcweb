...
echo '<div  style="display:none;" id="user_position_append"><a href=./index.php?jzdc=mall.goods_admin>'.self::$language['pages']['mall.goods_admin']['name'].'</a><a href="./index.php?jzdc=mall.goods_admin&id='.$goods_id.'">'.mb_substr($r['title'],0,30,'utf-8').'</a><span class=text>'.self::$language['pages']['mall.relevance_package']['name'].'</span></div>';
...
if($r['relevance_package']==''){
	...
	foreach($r as $v){
		...
		foreach($r2 as $v2){
			...
			$temp[$v2['id']]='<div class=goods_div id=goods_div_'.$v2['id'].'><a href="./index.php?jzdc=mall.goods&id='.$v2['id'].'" target=_blank class=goods_a><img src="./program/mall/img_thumb/'.$v2['icon'].'" /><br />'.$w_price.'<br /><span class=goods_name>'.$v2['title'].'</span></a></div><div class="add_symbol">&nbsp;</div>';	
		}
		...
		$list[$v['id']]='<div class=package_div id=package_div_'.$v['id'].'>'.$temp3.'<div class=result_div><a href="./index.php?jzdc=mall.package_admin&package_id='.$v['id'].'" target=_blank class=edit>'.self::$language['edit'].self::$language['package'].'</a><br /><span class=discount_value>'.str_replace('.00','',$v['discount']).self::$language['discount'].'</span><br /><a href=# class=move_to_left>&nbsp;</a><a href=# class=move_to_right>&nbsp;</a><a href=# class=delete_me>&nbsp;</a></div></div>';	
	}
	...
	$module['data']=$module['data'].'<div class=package_div id=new><a href="./index.php?jzdc=mall.package_admin&goods_id='.$goods_id.'&act=relevance_package&package_ids='.@$package_ids.'" class=goods_div id=add>&nbsp;</a></div>';
