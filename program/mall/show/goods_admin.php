<?php
$module['mass']='';
if(in_array('weixin.mass',$_SESSION['jzdc']['page'])){
	$sql="select `wid` from ".$pdo->sys_pre."weixin_account where `username`='".$_SESSION['jzdc']['username']."' or `manager` like '%,".$_SESSION['jzdc']['username'].",%' limit 0,1";
...
$sql="select `id`,`type`,`title`,`icon`,`sold`,`position`,`inventory`,`unit`,`supplier`,`visit`,`time`,`shop_sequence`,`state`,`shop_tag`,`shop_type`,`option_enable`,`bidding_show`,`mall_state`,`w_price`,`min_price` from ".self::$table_pre."goods where `shop_id`=".SHOP_ID;
...
if(intval(@$_GET['id'])!=0){
	...
	echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=mall.goods_admin">'.self::$language['pages']['mall.goods_admin']['name'].'</a><span class=text>'.$_GET['id'].'</span></div>';
}
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	$sum_sql=$sql.$where;
	$sum_sql=str_replace(" `id`,`type`,`title`,`icon`,`sold`,`position`,`inventory`,`unit`,`supplier`,`visit`,`time`,`shop_sequence`,`state`,`shop_tag`,`shop_type`,`option_enable`,`bidding_show`,`mall_state`,`w_price`,`min_price` "," count(id) as c ",$sum_sql);
...
foreach($r as $v){
	...
	//if($v['option_enable']==0){$edit_type="<a href=./index.php?jzdc=mall.s_type_set&c_id=".$v['id']."&id=".$v['type']." class=set>&nbsp;</a>";}else{$edit_type='';}
	$edit_type='';
	if($v['state']==0){
	...
	<td><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class='icon'><img src='./program/mall/img_thumb/".$v['icon']."'></a></td>
	<td><div class=title><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank>".$v['title']."</a></div><div class=type_tag><span class=type>".self::$language['mall'].self::$language['type'].": ".self::get_type_position($pdo,$v['type'])." ".$edit_type."</span></div>
  ...
  <a href='index.php?jzdc=".$class.".goods_edit&id=".$v['id']."&type=".$v['type']."'  target=_blank class=edit>".self::$language['edit']."</a><br /> 
...
function get_supplier_filter($pdo,$language,$table_pre){
	...
	//$sql="select * from ".$table_pre."supplier order by `sequence` desc";
    //从供应商用户组里面选择
