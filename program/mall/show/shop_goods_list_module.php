...
//echo $sql;
	...
	foreach($r as $v){
		...
		if(@$attribute[$_COOKIE['jzdc_device'].'_buy_button']){$buy_button="<div class=button_div><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank  class=add_cart option_enable='".$v['option_enable']."'  s_id='".$sv['s_id']."' s_price='".$sv['s_price']."'  title='".self::$language['add_cart']."'></a></div>";}
		
		if($attribute['show_method']!='show_line'){
			$list.="<div class=goods><a href='./index.php?jzdc=mall.goods&shop_id=".SHOP_ID."&id=".$v['id']."' target=".$attribute['target']." class=goods_a><img src='./program/mall/img_thumb/".$v['icon']."' /><span class=title>".$v['title']."</span></a><span class=price_span>".$w_price."/".self::get_mall_unit_name($pdo,$v['unit'])."</span>".$buy_button."</div>";
		}else{
			$list.="<div class=line><a href='./index.php?jzdc=mall.goods&shop_id=".SHOP_ID."&id=".$v['id']."' target=_blank class=goods_img><img src='./program/mall/img_thumb/".$v['icon']."' /></a><div class=good_info><a href='./index.php?jzdc=mall.goods&id=".$v['id']."'  target=".$attribute['target']." class=title>".$v['title']."</a><span class=price_span>".$w_price."/".self::get_mall_unit_name($pdo,$v['unit'])."</span>".$buy_button."</div></div>";
		}
	}
	...
	$module['title_link']='./index.php?jzdc=mall.shop_goods_list&shop_id='.SHOP_ID.'&type='.$id.'&tag='.$attribute['tag'];
	...
	if($attribute['show_sub']>0){
		...
		$r=$pdo->query($sql,2);
		foreach($r as $v){
			$module['sub_type'].='<a href="./index.php?jzdc=mall.shop_goods_list&shop_id='.SHOP_ID.'&type='.$v['id'].'"  target='.$attribute['target'].' d_id="'.$v['id'].'">'.$v['name'].'</a>';
			...
			foreach($r2 as $v2){
				...
				$buy_button='';
				if(@$attribute[$_COOKIE['jzdc_device'].'_buy_button']){$buy_button="<div class=button_div><a href='./index.php?jzdc=mall.goods&shop_id=".SHOP_ID."&id=".$v2['id']."' target=_blank  class=add_cart option_enable='".$v2['option_enable']."'  s_id='".$sv['s_id']."' s_price='".$sv['s_price']."' title='".self::$language['add_cart']."'></a></div>";}
				if($attribute['show_method']!='show_line'){
					$temp.="<div class=goods><a href='./index.php?jzdc=mall.goods&shop_id=".SHOP_ID."&id=".$v2['id']."' target=".$attribute['target']." class=goods_a><img wsrc='./program/mall/img_thumb/".$v2['icon']."' /><span class=title>".$v2['title']."/".self::get_mall_unit_name($pdo,$v2['unit'])."</span></a><span class=price_span>".$w_price."</span>".$buy_button."</div>";
				}else{
					$temp.="<div class=line><a href='./index.php?jzdc=mall.goods&shop_id=".SHOP_ID."&id=".$v2['id']."' target=_blank class=goods_img><img wsrc='./program/mall/img_thumb/".$v2['icon']."' /></a><div class=good_info><a href='./index.php?jzdc=mall.goods&id=".$v2['id']."'  target=".$attribute['target']." class=title>".$v2['title']."</a><span class=price_span>".$w_price."/".self::get_mall_unit_name($pdo,$v2['unit'])."</span>".$buy_button."</div></div>";
				}
