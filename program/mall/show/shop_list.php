...
if(!isset($_GET['geo'])){
	if(isset($_SESSION['jzdc']['id'])){
		$sql="select `geolocation` from ".$pdo->index_pre."user where `id`=".$_SESSION['jzdc']['id'];
...
$sql="select `id`,`name`,`main_business`,`area`,`address`,`goods`,`order`,`position` from ".self::$table_pre."shop where `state`=2 and `is_head`=0";
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	$sum_sql=$sql.$where;
	$sum_sql=str_replace(" `id`,`name`,`main_business`,`area`,`address`,`goods`,`order`,`position` "," count(id) as c ",$sum_sql);
...
foreach($r as $v){
	...
	if($_COOKIE['jzdc_device']=='pc'){
		...
		$goods='';
		foreach($r2 as $v2){
			$goods.='<a href=./index.php?jzdc=mall.goods&id='.$v2['id'].' target="_blank"><img src=./program/mall/img_thumb/'.$v2['icon'].' /></a>';
		}
		$list.="<div class=shop_div>
			<div class=info>
				<a href=./index.php?jzdc=mall.shop_index&shop_id=".$v['id']." class=icon target=_blank><img src=./program/mall/shop_icon/".$v['id'].".png /></a><div class=other>
					<a href=./index.php?jzdc=mall.shop_index&shop_id=".$v['id']." target=_blank class=name>".$v['name']." ".$distance."</a>
					<div class=sum><span class=m_label>".self::$language['goods']." : </span><b>".$v['goods']."</b> ".self::$language['a_jian']."</div>
					<div class=area><span class=m_label>".self::$language['address']." : </span><a href=# target=_blank class=navigate destination='".$destination."'>".$v['address']."</a></div>
