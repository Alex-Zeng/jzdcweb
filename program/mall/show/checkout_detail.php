...
foreach($r as $v){
	if($v['cashier']=='jzdc'){echo 'no checkout add';return false;}
	if($_SESSION['jzdc']['group_id']==self::$config['cashier_group_id'] && $v['cashier']!=$_SESSION['jzdc']['username']){echo 'no power';return false;}
	$goods_money=0;
	$v=de_safe_str($v);
	...
	foreach($r2 as $v2){
		...
		if($v['preferential_way']==5){$o_price='<span class=o_price>'.$v2['price'].'</span>';$v2['price']=$v2['transaction_price'];}else{$o_price='';}	
		  ...
		  <span class=icon>
			  ...
			  <div class=price>".self::$language['price'].":".$o_price.self::$language['money_symbol'].str_replace('.00','',$v2['price'])." &nbsp; ".self::$language['quantity'].":".self::format_quantity($v2['quantity']).$v2['unit']." <a href=./index.php?jzdc=mall.goods_snapshot&id=".$v2['snapshot_id']." target=_balnk>".self::$language['snapshot']."</a></div>
		  </span>
	  </div>";	
