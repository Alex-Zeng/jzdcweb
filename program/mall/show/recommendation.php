...
$sql="select `recommendation` from ".$pdo->index_pre."user where `id`='".$_SESSION['jzdc']['id']."'";
$r=$pdo->query($sql,2)->fetch(2);
$recommendation=$r['recommendation'];

$sql="select `goods_id` from ".self::$table_pre."order_goods where `buyer`='".$_SESSION['jzdc']['username']."' and `recommendation`=1 and `order_state`=6";
$r=$pdo->query($sql,2);
$goods_ids='';
foreach($r as $v){
...
foreach($r2 as $v2){
	...
	if($_COOKIE['jzdc_device']=='pc'){
		$list.='<div class=goods id=goods_'.$v2['id'].'><div class=gain><span>'.self::$language['buy'].'<b>'.$shop[$v2['shop_id']]['recommendation_discount'].'</b>'.self::$language['discount'].'</span>,<span>'.self::$language['recommendation_money'].'<b>'.$shop[$v2['shop_id']]['recommendation_rebate'].'</b>%</span></div><a href="./index.php?jzdc=mall.goods&id='.$v2['id'].'" target=_blank class=goods_a><img src="./program/mall/img_thumb/'.$v2['icon'].'" /><br />'.$w_price.'<span class=goods_name>'.$v2['title'].'</span></a><textarea placeholder="'.self::$language['promotion_word'].'" >'.str_replace('{code}',$recommendation,$v2['recommendation_slogan']).'</textarea><div class=state></div></div>';
	}else{
		$list.='<div class=goods id=goods_'.$v2['id'].'><div class=icon><a href="./index.php?jzdc=mall.goods&id='.$v2['id'].'" target=_blank class=goods_a><img src="./program/mall/img_thumb/'.$v2['icon'].'" /><br />'.$w_price.'</a></div><div class=other>
