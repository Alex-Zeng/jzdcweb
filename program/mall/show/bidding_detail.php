...
if(isset($_GET['shop_id'])){
	...
	if($shop_id>0){
		...
		$shop_name=$r2['name'];
		echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=mall.bidding_detail">'.self::$language['pages']['mall.bidding_detail']['name'].'</a><span class=text>'.$shop_name.'</span></div>';
	}
}
...
if(isset($_GET['gid'])){
	...
	if($gid>0){
		...
		$r2=$pdo->query($sql2,2)->fetch(2);
		$goods_name=$r2['title'];
		echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=mall.bidding_detail">'.self::$language['pages']['mall.bidding_detail']['name'].'</a><span class=text>'.$goods_name.'</span></div>';
	}
}
...
foreach($r as $v){
	...
	<td><a href=./index.php?jzdc=mall.goods&id=".$v['gid']." target=_blank class=goods_name>".$goods_name."</a></td>
	<td><a href=./index.php?jzdc=".$v['src_url']." target=_blank class=src_title>".$v['src_title']."</a></td>
	<td>".$v['money']."</td>
	<td><a href=./index.php?jzdc=mall.bidding_detail&shop_id=".$v['shop_id']." target=_blank>".$shop_name."</a></td>
