...
$sql="select `coupon_id` from ".self::$table_pre."my_coupon where `username`='".$_SESSION['jzdc']['username']."' and `use_time`=0 order by `id` desc limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	$sum_sql="select count(id) as c  from ".self::$table_pre."my_coupon where `username`='".$_SESSION['jzdc']['username']."' and `use_time`=0";
	//echo $sum_sql;
...
foreach($r as $v){
	...
	foreach($r3 as $v3){
		$goods_ids.=$v3['id'].',';		
		$goods.='<a href="./index.php?jzdc=mall.goods&id='.$v3['id'].'" target="_blank"><img src="./program/mall/img_thumb/'.$v3['icon'].'"><span class=price>'.$v3['min_price'].'</span></a>';	
	}
	...
	$list.='<div class=coupon d_id='.$v['coupon_id'].'>
            	<div class=info>
                    ...
                    <div class="i_body">
                    	<div class=shop_name><span class=m_label>'.self::$language['distribution_shop'].'：</span><span class=m_value><a href="./index.php?jzdc=mall.shop_index&shop_id='.$r2['shop_id'].'" target=_blank>'.$r2['shop_name'].'</a></span></div>
                ...
                <div class=act><a href="./index.php?jzdc=mall.shop_index&shop_id='.$r2['shop_id'].'"  target="_blank" class=go_shop></a><a href="#" class=del></a></div>
            </div>';
