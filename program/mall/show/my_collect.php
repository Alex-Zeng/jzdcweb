...
$sql="select `goods_id`,`time` from ".self::$table_pre."favorite where `username`='".$_SESSION['jzdc']['username']."' order by `id` desc limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	$sum_sql="select count(id) as c  from ".self::$table_pre."favorite where `username`='".$_SESSION['jzdc']['username']."'";
	//echo $sum_sql;
