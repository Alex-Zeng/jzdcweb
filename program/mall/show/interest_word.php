...
if(@$_GET['group_id']!=''){
	...
	echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=mall.interest_group">'.$groups[$group].'</a><span class=text>'.self::$language['pages']['mall.interest_word']['name'].'</span></div>';
}
...
foreach($r as $v){
	...
	$list.="<tr id='tr_".$v['id']."'>
	  ...
	  <td><input type='hidden' name='group_ids_".$v['id']."' id='group_ids_".$v['id']."' value='".$v['group_ids']."'  class='group_ids' /> <span class=group_ids_name>".$group."</span> <a href='./index.php?jzdc=mall.interest_word_grop&w_id=".$v['id']."' class=set_groups></a></td>
	  <td>".$v['day']."</td>
	  <td>".$v['week']."</td>
	  <td>".$v['month']."</td>
	  <td>".$v['year']."</td>
	  <td><a href='index.php?jzdc=mall.interest_user_list&word_id=".$v['id']."' class=link>".$v['sum']."</a></td>
