...
$sql="select * from ".self::$table_pre."shop where `agent`='".$_SESSION['jzdc']['username']."'";
...
foreach($r as $v){
	...
	$sql="select sum(actual_money) as c from ".self::$table_pre."order where `shop_id`=".$v['id']." and `cashier`='jzdc' and `state`=6";
	...
	$sql="select sum(money) as c from ".self::$table_pre."agent_finance where `shop_id`=".$v['id']." and `username`='".$_SESSION['jzdc']['username']."'";
	$r2=$pdo->query($sql,2)->fetch(2);
	$my_earning=abs($r2['c']);
	
	$list.="<tr id='tr_".$v['id']."'>
	  <td><div class=info>
	  	<br /><div class=icon><a href=./index.php?jzdc=mall.shop_index&shop_id=".$v['id']." target=_blank><img src=./program/mall/shop_icon/".$v['id'].".png /></a></div><div class=other>
	  ...
	  <td class=my_earning><a href='./index.php?jzdc=mall.agent_finance&shop_id=".$v['id']."' target=_blank>".$my_earning."</a></td>
