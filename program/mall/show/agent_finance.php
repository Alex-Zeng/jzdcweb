...
$sql="select * from ".self::$table_pre."agent_finance where `username`='".$_SESSION['jzdc']['username']."'";
...
foreach($r as $v){
	...
	<td><a href='./index.php?jzdc=mall.finance&shop_id=".$v['shop_id']."' class=shop_name>".$v['shop_name']."</a></td>
...
if(@$_GET['shop_id']!=''){
	echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=mall.agent_finance">'.self::$language['pages']['mall.agent_finance']['name'].'</a><span class=text>'.$v['shop_name'].'</span></div>';
}



$sql="select sum(money) as c from ".$pdo->index_pre."money_log where `username`='".$_SESSION['jzdc']['username']."' and `money`>0";
