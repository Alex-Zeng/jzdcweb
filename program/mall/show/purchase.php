...
foreach($r as $v){
	...
	$list.='<tr id=tr_'.$v['id'].'>
	<td><a href=./index.php?jzdc=mall.goods&id='.$goods['id'].' target=_blank class=goods_info><img src=./program/mall/img_thumb/'.$goods['icon'].' />'.$goods['title'].'</a></td>
	<td><a href=./index.php?jzdc=mall.supplier>'.self::get_supplier_name($pdo,$v['supplier']).'</a></td>
...
if(@$_GET['pay_wait']==1){
	echo ' <div id="user_position_reset" style="display:none;"><a href="index.php?jzdc=index.user">'.self::$language['user_center'].'</a><a href="index.php?jzdc=mall.seller">'.self::$language['pages']['mall.seller']['name'].'</a><a href=./index.php?jzdc=mall.purchase>'.self::$language['pages']['mall.purchase']['name'].'</a><span class=text>'.self::$language['pay_wait'].'</span></div>';
}