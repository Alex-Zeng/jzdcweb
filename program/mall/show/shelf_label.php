...
</div><div class=qr><img src="plugin/qrcode/?text='.str_replace('&','|||','http://'.self::$config['web']['domain'].'/index.php?jzdc=mall.goods&id='.$v['id'].'').'" /></div></div></div>';
		$i++;
	}else{
...
$t_path='./templates/'.$m_require_login.'/'.$class.'/'.self::$config['program']['template_'.$m_require_login].'/'.$_COOKIE['jzdc_device'].'/'.str_replace($class."::","",$method).'_'.$_GET['composing'].'.php';
