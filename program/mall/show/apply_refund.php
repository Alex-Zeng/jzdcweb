...
$sql="select `actual_money`,`express_cost_buyer`,`express_cost_seller`,`refund_reason`,`refund_amount`,`refund_remark`,`refund_voucher`,`buyer`,`id`,`state`,`pay_method` from ".self::$table_pre."order where `id`=".$id;
$r=$pdo->query($sql,2)->fetch(2);
$r=de_safe_str($r);
if(@$r['id']==''){echo self::$language['inoperable']; return false;}
if($r['buyer']!=$_SESSION['jzdc']['username']){echo self::$language['inoperable']; return false;}
