...
if($_GET['search']==''){
	...
	$r=$pdo->query($sql,2);
	foreach($r as $v){
		$module['data']['top_html'].='<a href=index.php?jzdc=mall.goods_list&type='.$v['id'].' id=type_a_'.$v['id'].'>'.$v['name'].'</a>';	
	}
...
$sql="select `id`,`icon`,`title`,`w_price`,`min_price`,`max_price`,`option_enable`,`monthly`,`satisfaction`,`sales_promotion`,`discount`,`discount_start_time`,`discount_end_time`,`shop_id`,`bidding_show`,`unit` from ".self::$table_pre."goods where `state`!=0 and `share`=0 and `mall_state`=1 and `online_forbid`=0";
...
$list='';
$time=time();
if($_COOKIE['jzdc_device']=='phone'){self::$language['monthly']=self::$language['monthly_sold_short'];}
$s_discount=array();
foreach($r as $v){
	$v=de_safe_str($v);
	...
	if($_COOKIE['jzdc_device']=='phone' && self::$config['phone_goods_list_show_buy_button']==false){
		$button='';	
	}else{
		if($_COOKIE['jzdc_device']=='phone'){
			$button="<div class=button_div><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank  class=add_cart option_enable='".$v['option_enable']."' s_id='".$sv['s_id']."' s_price='".$sv['s_price']."'>".self::$language['add_cart']."</a></div>";
		}else{
			$button="<div class=button_div><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank  class=add_cart option_enable='".$v['option_enable']."' s_id='".$sv['s_id']."' s_price='".$sv['s_price']."'>".self::$language['add_cart']."</a><a href=# class=favorite>".self::$language['favorite']."</a></div>";
		}
	...
	if($show_method!='show_line'){
		$list.="<div class=goods id=g_".$v['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=goods_a><img src='./program/mall/img_thumb/".$v['icon']."' /><span class=title>".$v['title']."</span><span class=price_span>".$w_price."/".self::get_mall_unit_name($pdo,$v['unit'])."</span>".$sum_div."</a>".$button."</div>";
	}else{
		if($_COOKIE['jzdc_device']=='phone'){
			$list.="<div class=line id=g_".$v['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=goods_img><img src='./program/mall/img_thumb/".$v['icon']."' /></a><div class=good_info><div class=top2><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=title>".$v['title']."</a><span class=price_span>".$w_price."/".self::get_mall_unit_name($pdo,$v['unit'])."</span>".$button."</div><div class=bottom>".$sum_div."</div></div></div>";

		}else{
			$list.="<div class=line id=g_".$v['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=goods_img><img src='./program/mall/img_thumb/".$v['icon']."' /></a><div class=good_info><div class=top2><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=title>".$v['title']."</a><span class=price_span>".$w_price."/".self::get_mall_unit_name($pdo,$v['unit'])."</span></div><div class=bottom>".$sum_div."".$button."</div></div></div>";
...
$module['list']=$list;
$module['page']=LansionDigitPage($sum,$_GET['current_page'],$page_size,'#'.$module['module_name'],false);


if(@$_GET['tag']!=''){
	$sql="select `name` from ".self::$table_pre."tag where `id`=".intval($_GET['tag']);
	$r=$pdo->query($sql,2)->fetch(2);
	$visitor_position_reset='<div id="visitor_position_reset" style="display:none;"><span id=current_position_text>'.self::$language['current_position'].'</span><a href="./index.php"><span id=visitor_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="./index.php?jzdc=mall.goods_list">'.self::$language['pages']['mall.goods_list']['name'].'</a>'.$r['name'].'</div>';
}elseif(@$_GET['search']!=''){
	$visitor_position_reset='<div id="visitor_position_reset" style="display:none;"><span id=current_position_text>'.self::$language['current_position'].'</span><a href="./index.php"><span id=visitor_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="./index.php?jzdc=mall.goods_list">'.self::$language['pages']['mall.goods_list']['name'].'</a>'.$_GET['search'].'</div>';
}else{
	$visitor_position_reset='<div id="visitor_position_reset" style="display:none;"><span id=current_position_text>'.self::$language['current_position'].'</span><a href="./index.php"><span id=visitor_position_icon>&nbsp;</span>'.self::$config['web']['name'].'</a><a href="./index.php?jzdc=mall.goods_list">'.self::$language['pages']['mall.goods_list']['name'].'</a>'.$this->get_type_position($pdo,$_GET['type']).'</div>';
}
