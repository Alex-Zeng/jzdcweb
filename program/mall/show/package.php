...
foreach($r as $v){
	...
	$goods_info[$v['id']]='<div class=g id=g_'.$v['id'].' spec=0 discount="'.$discount.'"><div class=color_selected_symbol>&nbsp;</div><div class=option_selected_symbol>&nbsp;</div><a href="./index.php?jzdc=mall.goods&id='.$v['id'].'" target=_balnk class=icon><img src="./program/mall/img_thumb/'.$v['icon'].'" /></a><span class=other><span class=title>'.$v['title'].'</span><span class=price_inventory><span class=normal><span class=m_label>'.self::$language['normal_price'].'</span><span class=money_symbol>'.self::$language['money_symbol'].'</span><span class=value value="'.$normal_price.'">'.$normal_price.'</span></span><span class=discount><span class=m_label>'.self::$language['package_price'].'</span><span class=money_symbol>'.self::$language['money_symbol'].'</span><span class=value value="'.$discount_price.'">'.$discount_price.'</span></span> <span class=inventory><span class=m_label>'.self::$language['inventory'].'</span><span class=value value='.self::format_quantity($v['inventory']).'>'.self::format_quantity($v['inventory']).'</span><span class=unit>'.self::get_mall_unit_name($pdo,$v['unit']).'</span></span></span><div class=spec>'.get_option_html(self::$language,$pdo,self::$table_pre,$v['id']).'<span class=json>'.@$_POST['specifications'].'</span></div></span></div>';
