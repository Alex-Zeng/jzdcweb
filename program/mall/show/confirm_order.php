...
if(!isset($_SESSION['jzdc']['username']) && self::$config['unlogin_buy']==false){
	echo '<a href=./index.php?jzdc=index.login style=" display:inline-block; width:30%; margin:auto; margin-left:35%; text-align:center; line-height:40px; margin-top:40px;background-color: #83c44e; color:#fff; border-radius:3px;">'.self::$language['please_login'].'</a>';
	exit('<script>window.location.href="./index.php?jzdc=index.login&relaod="+ Math.round(Math.random()*100);</script>');
}else if($_SESSION['jzdc']['group_id']<>4){
    //采购商才能购买
    echo '<a href=./index.php?jzdc=index.login style=" display:inline-block; width:30%; margin:auto; margin-left:35%; text-align:center; line-height:40px; margin-top:40px;background-color: #83c44e; color:#fff; border-radius:3px;">'.self::$language['buy_limit_tips'].'</a>';
    exit('<script>window.location.href="./index.php?jzdc=index.user</script>');
}
...
$_SESSION['token'][$method]=get_random(8);$module['action_url']="receive.php?token=".$_SESSION['token'][$method]."&target=".$method.'&'.$temp[1];


$module['use_web_credits_div']='';
if(isset($_SESSION['jzdc']['username'])){
	$sql="select `credits` from ".$pdo->index_pre."user where `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
...
//█████████████████████████████████████████████████████████████████████ 是否显示下单验证码
if(!isset($_SESSION['jzdc']['username']) && @$_GET['buy_method']!='unlogin'){$module['buy_method_div_display']='block';$module['confirm_order_div_display']='none';}else{$module['buy_method_div_display']='none';$module['confirm_order_div_display']='block';}
...
$module['show_more']='';
if(isset($_SESSION['jzdc']['username'])){
	$sql="select * from ".self::$table_pre."receiver where `username`='".$_SESSION['jzdc']['username']."' order by `sequence` desc";
	$r=$pdo->query($sql,2);
	$temp='';
	$i=0;
	foreach($r as $v){
...
if($goods_src=='goods_id'){//▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃ 如是单项购买
	...
	if(isset($_SESSION['jzdc']['username'])){
		...
		if($r2['username']==$_SESSION['jzdc']['username']){ echo '<div align="center" style="height:200px; line-height:200px;">'.self::$language['can_not_buy_your_goods'].'</div>'; return false;}
	}
	...
	if($r['limit']!=0 && $r['limit_cycle']!=''){
		...
		if(!isset($_SESSION['jzdc']['username'])){exit('<script>window.location.href="./index.php?jzdc=index.login";</script>');}
		if($r['limit_cycle']=='d'){
		...
		if($start_time){
			if($r['limit']==1){
				$sql="select `id` from ".self::$table_pre."order_goods where `goods_id`=".$id." and `buyer`='".$_SESSION['jzdc']['username']."' and `order_state`>0 and `time`>".$start_time." limit 0,1";
				$r2=$pdo->query($sql,2)->fetch(2);
				...
				$sql="select count(`quantity`) as c from ".self::$table_pre."order_goods where `goods_id`=".$id." and `buyer`='".$_SESSION['jzdc']['username']."' and `order_state`>0 and `time`>".$start_time." limit 0,10";
				$r2=$pdo->query($sql,2)->fetch(2);
...
foreach($r as $v){
	...
	//▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃ 移除限购，预售，商品 start
	if($_GET['goods_src']!='goods_id' &&  $v['recommendation']==1){$alone_buy.='<a href="./index.php?jzdc=mall.goods&id='.$v['id'].'">'.$v['title'].'</a><br />';continue;}
	if($v['limit']!=0 && $v['limit_cycle']!=''){
		if($quantity>$v['limit']){$alone_buy.='<a href="./index.php?jzdc=mall.goods&id='.$v['id'].'">'.$v['title'].'</a><br />';continue;}
		if(!isset($_SESSION['jzdc']['username'])){exit('<script>window.location.href="./index.php?jzdc=index.login";</script>');}
		if($v['limit_cycle']=='d'){
			$start_time=get_unixtime(date('Y-m-d',time()),self::$config['other']['date_style']);
		}
		...
		if($start_time){
			if($v['limit']==1){
				$sql="select `id` from ".self::$table_pre."order_goods where `goods_id`=".$v['id']." and `buyer`='".$_SESSION['jzdc']['username']."' and `order_state`>0 and `time`>".$start_time." limit 0,1";
				$v2=$pdo->query($sql,2)->fetch(2);
				if($v2['id']!=''){$alone_buy.='<a href="./index.php?jzdc=mall.goods&id='.$v['id'].'">'.$v['title'].'</a><br />';continue;}
			}else{
				$sql="select count(`quantity`) as c from ".self::$table_pre."order_goods where `goods_id`=".$v['id']." and `buyer`='".$_SESSION['jzdc']['username']."' and `order_state`>0 and `time`>".$start_time." limit 0,10";
				$v2=$pdo->query($sql,2)->fetch(2);
				if($v2['c']>=$v['limit']){$alone_buy.='<a href="./index.php?jzdc=mall.goods&id='.$v['id'].'">'.$v['title'].'</a><br />';continue;}
			}
		}
	}
	if($v['state']!=2 && $goods_src!='goods_id'){$alone_buy.='<a href="./index.php?jzdc=mall.goods&id='.$v['id'].'">'.$v['title'].'</a><br />';continue;}
	//▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃ 移除限购，预售，商品 end
...
foreach($goods_keys as $v){
	...
	if($goods[$v]['option_enable']){//如是有选项商品
		...
		//	$alone_buy.='<a href="./index.php?jzdc=mall.goods&id='.$v.'">['.self::$language['low_stocks'].']'.$goods[$v]['title'].'</a><br />';continue;
		//}
	...
	if($goods[$v]['state'] == 1){
		...
		if(isset($_SESSION['jzdc']['username'])){
			if(!isset($_SESSION['jzdc'][$goods[$v]['shop_id'].'_discount'])){
				$sql="select `group_id` from ".self::$table_pre."shop_buyer where `shop_id`=".$goods[$v]['shop_id']." and `username`='".$_SESSION['jzdc']['username']."'  limit 0,1";
				$r=$pdo->query($sql,2)->fetch(2);
				if($r['group_id']=='' || $r['group_id']<1){
					$_SESSION['jzdc'][$goods[$v]['shop_id'].'_shop_group']='';
					$_SESSION['jzdc'][$goods[$v]['shop_id'].'_discount']=10;
					$_SESSION['jzdc'][$goods[$v]['shop_id'].'_buy_credits_multiple']=1;
					$_SESSION['jzdc'][$goods[$v]['shop_id'].'_group_id']=0;
				}else{
					...
					$r=$pdo->query($sql,2)->fetch(2);
					$_SESSION['jzdc'][$goods[$v]['shop_id'].'_shop_group']=$r['name'];
					$_SESSION['jzdc'][$goods[$v]['shop_id'].'_discount']=$r['discount'];
					$_SESSION['jzdc'][$goods[$v]['shop_id'].'_buy_credits_multiple']=$r['buy_credits_multiple'];	
					$_SESSION['jzdc'][$goods[$v]['shop_id'].'_group_id']=$r['id'];
				}
		...
		//---如是店内会员，并折扣大于 店铺限时折扣
		if(isset($_SESSION['jzdc'][$goods[$v]['shop_id'].'_group_id'])){
			$sql="select `discount` from ".self::$table_pre."goods_group_discount where `goods_id`=".$goods[$v]['id']." and `group_id`=".$_SESSION['jzdc'][$goods[$v]['shop_id'].'_group_id']." limit 0,1";
		...
		if(isset($_SESSION['jzdc'][$goods[$v]['shop_id'].'_discount']) and ($_SESSION['jzdc'][$goods[$v]['shop_id'].'_discount']<10 || $goods_group_discount < 10)  and ($_SESSION['jzdc'][$goods[$v]['shop_id'].'_discount'] < $discount || $goods_group_discount < $discount) and $goods[$v]['sales_promotion']){
			if($goods_group_discount==100){
				$goods_group_discount=$_SESSION['jzdc'][$goods[$v]['shop_id'].'_discount'];
			}
	...
	if($_COOKIE['jzdc_device']=='pc'){//电脑版 html
		$shops[$goods[$v]['shop_id']]['shop_goods'].="
	<div class=tr goods_id=".$key." quantity=".$goods_info['q_'.$goods[$v]['id']].">
			<div class=goods_td>
			<a class=goods_info href=./index.php?jzdc=mall.goods&id=".$goods[$v]['id']." target=_blank><img src='./program/mall/".$goods[$v]['icon']."' /><div class=title_model><div class=title>".$goods[$v]['title']."</div><div class=model>".$goods[$v]['spec_info']."</div></div></a>
	...
	<div class=tr goods_id=".$key." quantity=".$goods_info['q_'.$goods[$v]['id']].">
			<div class=goods_td>
			<a class=goods_info href=./index.php?jzdc=mall.goods&id=".$goods[$v]['id']." target=_blank><img src='./program/mall/".$goods[$v]['icon']."' /><div class=title_model><div class=g_title>".$goods[$v]['title']."</div><div class=model>".$goods[$v]['spec_info']."</div></div></a>
			</div><div class=unit_price_td>
			<span class=price>".$price."</span>".$sales_promotion."
			<span class=favorable_price>".$favorable_price."</span>
			</div><div class=quantity_td>".$goods_info['q_'.$goods[$v]['id']]." <span class=unit>".self::get_mall_unit_name($pdo,$goods[$v]['unit'])."</span></div><div class=subtotal_td>".$subtotal."</div>
...
foreach($shops as $k=>$v){
	...
	if(isset($_SESSION['jzdc']['username'])){
		...
		$sql="select `credits` from ".self::$table_pre."shop_buyer where `username`='".$_SESSION['jzdc']['username']."' and `shop_id`=".$k." limit 0,1";
	...
	if(isset($_SESSION['jzdc'][$k.'_discount']) && $_SESSION['jzdc'][$k.'_shop_group']!=''){
		$module['preferential_way_option'].='<option value="5">'.$_SESSION['jzdc'][$k.'_shop_group'].self::$language['rebate'].'</option>';	
	}else{
    ...
    $module['goods_html'].="<div class=shop_div id=shop_".$k." red_coupon_amount='-".$red_coupon['amount']."' red_coupon_id=".$red_coupon['id']." weight=".$shops[$k]['weight']." full_pre='".$full_pre."' promotion_money='".$shops[$k]['promotion_money']."' all_money='".$shops[$k]['all_money']."' >"."
    <div class=shop_goods>".$shops[$k]['shop_goods']."</div>
    </div>";
}
//███████████████████████████████████████████████████████████████████ 整理 .shop_div end
...
//███████████████████████████████████████████████████████████████ 获取上一个订单的收货信息 start
if(isset($_SESSION['jzdc']['username'])){
	$sql="select `receiver_id`,`delivery_time` from ".self::$table_pre."order where `buyer`='".$_SESSION['jzdc']['username']."' order by `id` desc limit 0,1";
	$r=$pdo->query($sql,2)->fetch(2);
