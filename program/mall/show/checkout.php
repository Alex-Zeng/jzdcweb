...
if(isset($_COOKIE['checkout_desk'])){
	...
	<td class=goods_td><a href='./index.php?jzdc=mall.goods&id=".$r['id']."' target=_blank class=goods_info>
...
$sql="select `name`,`username` from ".self::$table_pre."shop where `id`=".SHOP_ID;
$r=$pdo->query($sql,2)->fetch(2);
...
$module['pay_method']='<option value="cash" selected>'.self::$language['pay_method']['cash'].'</option><option value="pos">'.self::$language['pay_method']['pos'].'</option>'.$scanpay_option.'<option value="alipay_p" >'.self::$language['pay_method']['alipay_p'].'</option><option value="weixin_p" >'.self::$language['pay_method']['weixin_p'].'</option><option value="meituan" >'.self::$language['pay_method']['meituan'].'</option><option value="nuomi" >'.self::$language['pay_method']['nuomi'].'</option><option value="other" >'.self::$language['pay_method']['other'].'</option>';

$module['pay_method_2']=$module['pay_method'];	

$module['pay_method_full_no_shop']=$module['pay_method_2'].'<option value="balance">'.self::$language['pay_method']['balance'].'</option>';
$module['pay_method_full_no_web']=$module['pay_method_2'].'<option value="shop_balance">'.self::$language['pay_method']['shop_balance'].'</option>';
$module['pay_method_full']=$module['pay_method_2'].'<option value="balance">'.self::$language['pay_method']['balance'].'</option><option value="shop_balance">'.self::$language['pay_method']['shop_balance'].'</option>';
...
foreach($r as $v){
	...
	$sql="select `id`,`title`,`e_price`,`unit`,`option_enable`,`py` from ".self::$table_pre."goods where `shop_id`=".SHOP_ID." and `state`!=0 and `share`=0 and `mall_state`=1 order by `shop_sequence` desc,`id` asc";
	$g=$pdo->query($sql,2);
	$module['goods']='';
	foreach($g as $gv){
		if($gv['option_enable']==1){
			...
			foreach($sg as $sgv){
				...
				$module['goods'].='<a goods_id='.$sgv['id'].' py="'.$gv['py'].'"  id_type=s_id key='.$gv['id'].'_'.$sgv['id'].'>'.de_safe_str($gv['title']).'<span class=option>'.$option_name.''.$color_name.'</span><span class=price price='.$sgv['e_price'].'>'.self::$language['money_symbol'].$sgv['e_price'].'/'.self::get_mall_unit_name($pdo,$gv['unit']).'</span></a>';	
			}
			
			
		}else{
			$module['goods'].='<a goods_id='.$gv['id'].' py="'.$gv['py'].'"  id_type=g_id key='.$gv['id'].'>'.de_safe_str($gv['title']).'<span class=price price='.$gv['e_price'].'>'.self::$language['money_symbol'].$gv['e_price'].'/'.self::get_mall_unit_name($pdo,$gv['unit']).'</span></a>';
