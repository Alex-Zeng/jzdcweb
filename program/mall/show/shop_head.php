...
$sql="select `id`,`head_pc`,`head_phone` from ".self::$table_pre."shop where `username`='".$_SESSION['jzdc']['username']."' and `state`=2 order by `id` desc limit 0,1";
...
$t_path='./templates/'.$m_require_login.'/'.$class.'/'.self::$config['program']['template_'.$m_require_login].'/'.$_COOKIE['jzdc_device'].'/'.str_replace($class."::","",$method).'_'.$type.'.php';
