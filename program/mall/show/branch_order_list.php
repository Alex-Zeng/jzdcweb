	...
	foreach($r2 as $v2){
		if($_COOKIE['jzdc_device']=='pc'){
        	...
        	<span class=icon>
                ...
                <div class=price>".self::$language['price'].":".self::$language['money_symbol'].str_replace('.00','',$v2['transaction_price'])." &nbsp; ".self::$language['quantity'].":".self::format_quantity($v2['quantity']).$v2['unit']." <a href=./index.php?jzdc=mall.goods_snapshot&id=".$v2['snapshot_id']." target=_balnk>".self::$language['snapshot']."</a></div>
            </span>
        </div>";	
		}
