...
if($package_id!=0){
	...
	echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=mall.package_admin">'.self::$language['pages']['mall.package_admin']['name'].'</a><span class=text>'.$package_id.'</span></div>';
}
...
foreach($r as $v){
	...
	foreach($r2 as $v2){
		...
		$temp[$v2['id']]='<div class=goods_div id=goods_div_'.$v2['id'].'><a href="./index.php?jzdc=mall.goods&id='.$v2['id'].'" target=_blank class=goods_a><img src="./program/mall/img_thumb/'.$v2['icon'].'" /><br />'.$w_price.'<br /><span class=goods_name>'.$v2['title'].'</span></a><a href=# class=move_to_left>&nbsp;</a><a href=# class=move_to_right>&nbsp;</a><a href=# class=delete_me>&nbsp;</a></div>';	
	...
	$list.='<div class=package_div id=package_div_'.$v['id'].'>'.$temp3.'<a href="./index.php?jzdc=mall.goods_admin&package_id='.$v['id'].'&act=relevance_package&goods_ids='.$v['goods_ids'].'" class=goods_div id=add>&nbsp;</a><div class="equal_symbol">&nbsp;</div><div class=goods_div style="text-align:left; margin-bottom: -75px;">'.self::$language['free_shipping'].' <select class=free_shipping jzdc_value='.$v['free_shipping'].'><option value=0>'.self::$language['no'].'</option><option value=1>'.self::$language['yes'].'</option></select><br /><input type=text class=discount value="'.$v['discount'].'" />'.self::$language['discount'].'<br /><br /><div class=operation_div><a href="#"  class="submit">'.self::$language['save'].'</a><br /><br /><a href="#" class="del">'.self::$language['del'].'</a><br /><span class="state"></span></a></div></div></div>';	
}
$module['list']=$list;
if($package_id==0 && $_GET['current_page']==1){$module['list']='<div class=package_div id=new><a href="./index.php?jzdc=mall.goods_admin&act=relevance_package" class=goods_div id=add>&nbsp;</a></div>'.$module['list'];}
