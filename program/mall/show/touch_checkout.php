...
foreach($r as $v){
	...
	$sql="select `id`,`title`,`e_price`,`unit`,`option_enable`,`icon`,`py` from ".self::$table_pre."goods where `shop_id`=".SHOP_ID." and `shop_type`=".$v['id']." and `state`!=0 and `share`=0 and `mall_state`=1 order by `shop_sequence` desc,`id` asc";
	$g=$pdo->query($sql,2);
	$goods='';
	foreach($g as $gv){
		if($gv['option_enable']==1){
	...
	foreach($r2 as $v2){
		...
		$sql="select `id`,`title`,`e_price`,`unit`,`option_enable`,`icon`,`py` from ".self::$table_pre."goods where `shop_id`=".SHOP_ID." and `shop_type`=".$v2['id']." and `state`!=0 and `share`=0 and `mall_state`=1 order by `shop_sequence` desc,`id` asc";
		$g=$pdo->query($sql,2);
		$goods='';
		foreach($g as $gv){
			if($gv['option_enable']==1){
	...
	$sql="select `id`,`title`,`e_price`,`unit`,`option_enable`,`icon`,`py` from ".self::$table_pre."goods where `shop_id`=".SHOP_ID." and `shop_type`=0 and `state`!=0 and `share`=0 and `mall_state`=1 order by `shop_sequence` desc,`id` asc";
	$g=$pdo->query($sql,2);
	$goods='';
	foreach($g as $gv){
		if($gv['option_enable']==1){
...
if(isset($_COOKIE['checkout_desk'])){
	...
	<td class=goods_td".$have_spec."><a href='./index.php?jzdc=mall.goods&id=".$r['id']."' target=_blank class=goods_info><span class=title>".$r['title']."</span><span class=spec>".$spec_info."</span></a></td>
