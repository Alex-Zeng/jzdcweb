...
if(!isset($_COOKIE['mall_cart'])){
	if(isset($_SESSION['jzdc']['username'])){ self::update_cart($pdo,self::$table_pre);}	
}else{
	if(isset($_SESSION['jzdc']['username'])){
		...
		$mall_cart=self::array_sort($mall_cart,'time',$type='desc');
			foreach($mall_cart as $key=>$v){
				$sql="select `id` from ".self::$table_pre."cart where `username`='".$_SESSION['jzdc']['username']."' and `key`='".$key."' limit 0,1";
	...
	if($_COOKIE['mall_cart']!='' && count(json_decode($_COOKIE['mall_cart'],true))>0 ){
		...
		foreach($mall_cart as $key=>$v){
			...
			$module['mall_cart'].='<div id=cart__'.$key.'>
                    	<a href="./index.php?jzdc=mall.goods&id='.$r['id'].'" class=goods_info>
                            <span class=img><img src="./program/mall/'.$img.'" /></span><span class=title_and_other>
                                <span class=title>'.$r['title'].'</span>
                                <span class=other><span class=price><span class=money_symbol>'.self::$language['money_symbol'].'</span>'.$price.'*'.$mall_cart[$key]['quantity'].'</span><span class=specifications>'.$spec_info.'</span></span>
                            </span>
		...
		if(count($mall_cart)>10){$module['view_all']='<a href="./index.php?jzdc=mall.my_cart" class=view_all>'.self::$language['view_all'].'</a>';}
	}	
}
if($module['mall_cart']!=''){
	$module['cart_html']='<div class=lately>'.self::$language['cart_lately_goods'].'</div>
				...
				'.$module['view_all'].'
				<div class=sum_div>
                	'.self::$language['sum_data'].'<span class=goods_count>'.count($mall_cart).'</span>'.self::$language['item'].self::$language['goods'].' 
                	...
                	<a href="./index.php?jzdc=mall.my_cart" class=go_w_cashier>'.self::$language['settlement'].'</a>
                </div>';
}
