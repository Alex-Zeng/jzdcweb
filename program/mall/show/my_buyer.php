...
foreach($r as $v){
	if(!isset($v['phone'])){
		$sql="ALTER TABLE  `jzdc_mall_shop_buyer` ADD  `phone` VARCHAR( 20 ) NULL COMMENT  '手机号',
ADD  `email` VARCHAR( 100 ) NULL COMMENT  '邮箱'";
	...
	$list.="<tr id='tr_".$v['id']."'>
	  ...
	  <td ><select class=group_id jzdc_value=".$v['group_id']." id=group_id_".$v['id'].">".self::get_shop_buyer_group_option($pdo,SHOP_ID)."</select></td>
	  <td><input type='text' name='chip_".$v['id']."' id='chip_".$v['id']."' value='".$v['chip']."'  class='chip' /></td>
	  <td class=operation_td><a href='#' onclick='return update(".$v['id'].")'  class='submit'>".self::$language['submit']."</a> <a href='./index.php?jzdc=mall.user_recharge&id=".$v['id']."' class='rehcarge'>".self::$language['recharge']."</a> <a href='./index.php?jzdc=mall.credits&id=".$v['id']."' class='credits'>".self::$language['credits']."</a> <a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
	</tr>
";	
