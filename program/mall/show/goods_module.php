...
$sql="select `id`,`icon`,`title`,`w_price`,`min_price`,`max_price`,`option_enable`,`monthly`,`sales_promotion`,`discount`,`discount_start_time`,`discount_end_time`,`shop_id`,`bidding_show`,`satisfaction`,`unit`,`multi_angle_img` from ".self::$table_pre."goods where `state`!=0 and `mall_state`=1 and `online_forbid`=0 and `share`=0 and `type` in(".$ids.") ".$tag_where." ".$store_tag_where." ".$circle_where." order by `".$attribute['sequence_field']."` ".$attribute['sequence_type'].",`bidding_show` desc limit ".$start.",".$attribute['quantity'];
	//echo $sql;
	$r=$pdo->query($sql,2);
	$list='';
	$list2='';
	...
	foreach($r as $v){
		$v=de_safe_str($v);
		$v['normal_w_price']=$v['w_price'];
		$v['normal_min_price']=$v['min_price'];
		$v['normal_max_price']=$v['max_price'];
		...
		if($v['option_enable']==0){
			...
			if(in_array('unit',$attribute['field'])){$w_price='<span class=money_value>'.trim(trim($v['w_price'],'0'),'.').'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=unit>/'.self::get_mall_unit_name($pdo,$v['unit']).'</span>';}
			if(in_array('normal_price',$attribute['field']) && $v['w_price']<$v['normal_w_price']){$w_price.='<span class=normal_price>'.self::$language['money_symbol'].$v['normal_w_price'].'</span>';}
			$sv=array();
			$sv['s_id']=0;
			$sv['s_price']=0;
		}else{
			...
			if($v['min_price']==$v['max_price']){
				...
				if(in_array('unit',$attribute['field'])){$w_price='<span class=money_value>'.trim(trim($v['min_price'],'0'),'.').'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=unit>/'.self::get_mall_unit_name($pdo,$v['unit']).'</span>';}
				if(in_array('normal_price',$attribute['field']) && $v['min_price']<$v['normal_min_price']){$w_price.='<span class=normal_price>'.self::$language['money_symbol'].$v['normal_min_price'].'</span>';}
			}else{
				$w_price='<span class=money_symbol>'.self::$language['money_symbol'].'</span><span class=money_value>'.$v['min_price'].'-'.$v['max_price'].'</span>';
				if(in_array('unit',$attribute['field'])){$w_price='<span class=money_value>'.trim(trim($v['min_price'],'0'),'.').'-'.trim(trim($v['max_price'],'0'),'.').'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=unit>/'.self::get_mall_unit_name($pdo,$v['unit']).'</span>';}
				if(in_array('normal_price',$attribute['field']) && $v['min_price']<$v['normal_min_price']){$w_price.='<span class=normal_price>'.self::$language['money_symbol'].$v['normal_min_price'].'-'.$v['normal_max_price'].'</span>';}

			}
		}
		$buy_button='';
		if(@$attribute[$_COOKIE['jzdc_device'].'_buy_button']){$buy_button="<div class=button_div><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank  class=add_cart user_color=button option_enable='".$v['option_enable']."' s_id='".$sv['s_id']."' s_price='".$sv['s_price']."'>".self::$language['add_cart']."</a></div>";}
		...
		if($attribute['show_method']!='show_line'){
			...
			if($_COOKIE['jzdc_device']=='pc'){
				$list.="<div class=goods id=g_".$v['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=".$attribute['target']." class=goods_a><span class=goods_img_div><img wsrc='./program/mall/img_thumb/".$v['icon']."' /></span>".$multi_angle_img."<span class=title>".$v['title']."</span></a><span class=price_span>".$w_price."</span>".$sum_div.$buy_button."</div>";
			}else{
				$list.="<div class=goods id=g_".$v['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=".$attribute['target']." class=goods_a><span class=goods_img_div><img wsrc='./program/mall/img_thumb/".$v['icon']."' /></span><span class=title>".$v['title']."</span></a><span class=price_span>".$w_price."</span>".$buy_button.$sum_div."</div>";
			}
			
		}else{
			if($_COOKIE['jzdc_device']=='pc'){
				$list.="<div class=line id=g_".$v['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=goods_img><img wsrc='./program/mall/img_thumb/".$v['icon']."' /></a><div class=good_info><a href='./index.php?jzdc=mall.goods&id=".$v['id']."'  target=".$attribute['target']." class=title>".$v['title']."</a><span class=price_span>".$w_price."</span>".$sum_div.$buy_button."</div></div>";
			}else{
				$list.="<div class=line id=g_".$v['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v['id']."' target=_blank class=goods_img><img wsrc='./program/mall/img_thumb/".$v['icon']."' /></a><div class=good_info><a href='./index.php?jzdc=mall.goods&id=".$v['id']."'  target=".$attribute['target']." class=title>".$v['title']."</a><span class=price_span>".$w_price."</span>".$buy_button.$sum_div."</div></div>";
			}
	...
	$module['title_link']='./index.php?jzdc=mall.goods_list&type='.$id.'&tag='.$attribute['tag'];
	...
	if($attribute['show_sub']>0){
		...
		$r=$pdo->query($sql,2);
		foreach($r as $v){
			$module['sub_type'].='<a href="./index.php?jzdc=mall.goods_list&type='.$v['id'].'"  target='.$attribute['target'].' d_id="'.$v['id'].'">'.$v['name'].'</a>';
			$sql2="select `id`,`icon`,`title`,`w_price`,`min_price`,`max_price`,`option_enable`,`monthly`,`sales_promotion`,`discount`,`discount_start_time`,`discount_end_time`,`bidding_show`,`satisfaction`,`unit`,`multi_angle_img` from ".self::$table_pre."goods where `state`!=0 and `mall_state`=1 and `online_forbid`=0  and `share`=0 and `type` in (".$ids=$this->get_type_ids($pdo,$v['id']).") ".$tag_where." ".$store_tag_where." ".$circle_where." order by `".$attribute['sequence_field']."` ".$attribute['sequence_type'].",`bidding_show` desc limit ".$start.",".$attribute['quantity'];
			$r2=$pdo->query($sql2,2);
			//echo $sql2;
			$temp='';
			foreach($r2 as $v2){
				...
				$v2=de_safe_str($v2);
				$v2['normal_w_price']=$v2['w_price'];
				$v2['normal_min_price']=$v2['min_price'];
				$v2['normal_max_price']=$v2['max_price'];
				...
				if($v2['option_enable']==0){
					...
					if(in_array('unit',$attribute['field'])){$w_price='<span class=money_value>'.trim(trim($v2['w_price'],'0'),'.').'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=unit>/'.self::get_mall_unit_name($pdo,$v2['unit']).'</span>';}
					if(in_array('normal_price',$attribute['field']) && $v2['w_price']<$v2['normal_w_price']){$w_price.='<span class=normal_price>'.self::$language['money_symbol'].$v2['normal_w_price'].'</span>';}
					$sv=array();
					$sv['s_id']=0;
					$sv['s_price']=0;
				}else{
					...
					if($v2['min_price']==$v2['max_price']){
						...
						if(in_array('unit',$attribute['field'])){$w_price='<span class=money_value>'.trim(trim($v2['min_price'],'0'),'.').'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=unit>/'.self::get_mall_unit_name($pdo,$v2['unit']).'</span>';}
						if(in_array('normal_price',$attribute['field']) && $v2['min_price']<$v2['normal_min_price']){$w_price.='<span class=normal_price>'.self::$language['money_symbol'].$v2['normal_min_price'].'</span>';}
					}else{
						$w_price='<span class=money_symbol>'.self::$language['money_symbol'].'</span><span class=money_value>'.$v2['min_price'].'-'.$v2['max_price'].'</span>';
						if(in_array('unit',$attribute['field'])){$w_price='<span class=money_value>'.trim(trim($v2['min_price'],'0'),'.').'-'.trim(trim($v2['max_price'],'0'),'.').'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=unit>/'.self::get_mall_unit_name($pdo,$v2['unit']).'</span>';}
						if(in_array('normal_price',$attribute['field']) && $v2['min_price']<$v2['normal_min_price']){$w_price.='<span class=normal_price>'.self::$language['money_symbol'].$v2['normal_min_price'].'-'.$v2['normal_max_price'].'</span>';}
		
					}
				}
						
				$buy_button='';
				if(@$attribute[$_COOKIE['jzdc_device'].'_buy_button']){$buy_button="<div class=button_div><a href='./index.php?jzdc=mall.goods&id=".$v2['id']."' target=_blank  class=add_cart user_color=button option_enable='".$v2['option_enable']."' s_id='".$sv['s_id']."' s_price='".$sv['s_price']."'>".self::$language['add_cart']."</a></div>";}
				...
				if($attribute['show_method']!='show_line'){
					...
					if($_COOKIE['jzdc_device']=='pc'){
						$temp.="<div class=goods id=g_".$v2['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v2['id']."' target=".$attribute['target']." class=goods_a><span class=goods_img_div><img wsrc='./program/mall/img_thumb/".$v2['icon']."' /></span>".$multi_angle_img."<span class=title>".$v2['title']."</span></a><span class=price_span>".$w_price."</span>".$sum_div.$buy_button."</div>";
					}else{
						$temp.="<div class=goods id=g_".$v2['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v2['id']."' target=".$attribute['target']." class=goods_a><span class=goods_img_div><img wsrc='./program/mall/img_thumb/".$v2['icon']."' /></span><span class=title>".$v2['title']."</span></a><span class=price_span>".$w_price."</span>".$buy_button.$sum_div."</div>";
					}
				}else{
					if($_COOKIE['jzdc_device']=='pc'){
						$temp.="<div class=line id=g_".$v2['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v2['id']."' target=_blank class=goods_img><img wsrc='./program/mall/img_thumb/".$v2['icon']."' /></a><div class=good_info><a href='./index.php?jzdc=mall.goods&id=".$v2['id']."'  target=".$attribute['target']." class=title>".$v2['title']."</a><span class=price_span>".$w_price."</span>".$sum_div.$buy_button."</div></div>";
					}else{
						$temp.="<div class=line id=g_".$v2['id']." bidding=".$bidding_show."><a href='./index.php?jzdc=mall.goods&id=".$v2['id']."' target=_blank class=goods_img><img wsrc='./program/mall/img_thumb/".$v2['icon']."' /></a><div class=good_info><a href='./index.php?jzdc=mall.goods&id=".$v2['id']."'  target=".$attribute['target']." class=title>".$v2['title']."</a><span class=price_span>".$w_price."</span>".$buy_button.$sum_div."</div></div>";
					}
	...
	$module['username']=@$_SESSION['jzdc']['username'];
