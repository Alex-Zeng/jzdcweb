		...
		function get_list($pdo,$language,$table_pre,$class){
			...
			foreach($r as $v){
				$v['name']=de_safe_str($v['name']);
					...
					$show_sub='';
					if($r2['c']>0){$show_sub="<a href='./index.php?jzdc=mall.type&parent=".$v['id']."' >".$language['sub']."</a> ";}				
			  ...
			  <td class=operation_td><a href='#' onclick='return update(".$v['id'].")'  class='submit'>".$language['submit']."</a><!-- <a href=./index.php?jzdc=mall.type_remark&id=".$v['id']." class=remark>".$language['ad']."</a> <a href=./index.php?jzdc=mall.type_price&id=".$v['id']." class=type_price>".$language['type_price']."</a>--> ".$show_sub."<!--<a href='./index.php?jzdc=mall.type_brand&type=".$v['id']."' >".$language['brand']."</a> --><a href='./index.php?jzdc=mall.type_attribute&type=".$v['id']."' >".$language['attribute']."</a> <a href='./index.php?jzdc=mall.type_option&type=".$v['id']."' >".$language['option']."</a> <a href='#' onclick='return del(".$v['id'].")'  class='del'>".$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
			</tr>
	";	
			}
			return $list;
		...
		echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?jzdc=mall.type">'.self::$language['pages']['mall.type']['name'].'</a>'.$this->get_type_user_position($pdo,intval(@$_GET['parent'])).'<span class=text></span></div>';
