...
if(@$_GET['near']==100){
	$module['list']=file_get_contents('./templates/'.$m_require_login.'/'.$class.'/'.self::$config['program']['template_'.$m_require_login].'/'.$_COOKIE['jzdc_device'].'/shop_map.php');
}

$t_path='./templates/'.$m_require_login.'/'.$class.'/'.self::$config['program']['template_'.$m_require_login].'/'.$_COOKIE['jzdc_device'].'/'.str_replace($class."::","",$method).'_'.self::$config['web']['map_api'].'.php';
require($t_path);
