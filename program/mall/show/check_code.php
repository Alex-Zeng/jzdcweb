	...
	foreach($r2 as $v2){
		...
		if($_COOKIE['jzdc_device']=='pc'){
        	...
        	<span class=icon>
                ...
                <div class=price>".self::$language['price'].":".self::$language['money_symbol'].str_replace('.00','',$v2['price'])." &nbsp; ".self::$language['quantity'].":".self::format_quantity($v2['quantity']).$v2['unit'].$refund.str_replace('d_id','d_id='.$v2['id'],$edit_quantity)." <a href=./index.php?jzdc=mall.goods_snapshot&id=".$v2['snapshot_id']." target=_balnk>".self::$language['snapshot']."</a></div>
            </span>
        </div>";
		}
