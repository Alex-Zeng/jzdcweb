...
if(intval(@$_GET['shop_id'])==0 && isset($_SESSION['jzdc']['username'])){
	$sql="select `id` from ".self::$table_pre."shop where `username`='".$_SESSION['jzdc']['username']."' and `state`=2 order by `id` desc limit 0,1";
...
if(!$is_shopowner){
	$sql="select `name`,`username`,`template`,`".$_COOKIE['jzdc_device']."_menu`,`phone_menu_show` from ".self::$table_pre."shop where `id`=".SHOP_ID;
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['username']==@$_SESSION['jzdc']['username']){$is_shopowner=true;}
	self::$config['shop_template']=$r['template'];	
	self::$config['shop_name']=$r['name'];	
	self::$config['shop_'.$_COOKIE['jzdc_device'].'_menu']=$r[$_COOKIE['jzdc_device'].'_menu'];	
}else{
	$sql="select `template`,`".$_COOKIE['jzdc_device']."_menu`,`phone_menu_show`,`name` from ".self::$table_pre."shop where `id`=".SHOP_ID;
	$r=$pdo->query($sql,2)->fetch(2);
	self::$config['shop_template']=$r['template'];	
	self::$config['shop_'.$_COOKIE['jzdc_device'].'_menu']=$r[$_COOKIE['jzdc_device'].'_menu'];	
	self::$config['shop_name']=$r['name'];	
...
if(self::$config['agency'] && $_GET['jzdc']=='mall.goods' && isset($_GET['store_id'])){
...
function get_shop_moudule($str,$pdo,$mall_layout_modules,$obj){
  ...
  return $modules;	
	...
	//if($phone_menu_show && $_COOKIE['jzdc_device']=='phone' && $v['url']!='mall.goods'){$v['bottom'].=',mall.menu';}
	//if($phone_menu_show && $_COOKIE['jzdc_device']=='phone'){$v['bottom'].=',mall.menu';}
	...
	$css_path="./templates/0/mall_shop/".self::$config['shop_template']."/".$_COOKIE['jzdc_device']."/shop_main.css";
