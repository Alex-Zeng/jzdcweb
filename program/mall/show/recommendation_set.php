...
$sql="select `id`,`recommendation_discount`,`recommendation_rebate` from ".self::$table_pre."shop where `username`='".$_SESSION['jzdc']['username']."' and `state`=2 order by `id` desc limit 0,1";
...
foreach($r2 as $v2){
	...
	$list.='<div class=goods id=goods_'.$v2['id'].'><a href="./index.php?jzdc=mall.goods&id='.$v2['id'].'" target=_blank class=goods_a><img src="./program/mall/img_thumb/'.$v2['icon'].'" /><br />'.$w_price.'<span class=goods_name>'.$v2['title'].'</span></a><textarea placeholder="'.self::$language['promotion_word'].'" >'.$v2['recommendation_slogan'].'</textarea><div class=state></div></div>';	
