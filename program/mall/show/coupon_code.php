...
foreach($r as $v){
	...
	if($v['sum_quantity']-$v['draws']>0){$push='<a href="./index.php?jzdc=mall.coupon_push&id='.$v['id'].'" class=push>'.self::$language['push'].'</a>';}else{$push='';}
	$list.="<tr id='tr_".$v['id']."'>
		<td><span class=name>".$v['name']."</span></td>
		<td><select class=open jzdc_value=".$v['open']." d_id=".$v['id']."><option value='0'>".self::$language['no']."</option><option value='1'>".self::$language['yes']."</option></select> <span class=state></span></td>
		...
		<td><a href='./index.php?jzdc=mall.coupon_log&id=".$v['id']."' class=draws>".$v['draws']."</a></td>
		<td><a href='./index.php?jzdc=mall.coupon_log&id=".$v['id']."&order=use_time|desc' class=used>".$v['used']."</a></td>
