...
foreach($r as $v){
	...
	$list.='<div class="p_group" id="p_'.$v['id'].'">
        	<a class=stock_big href="./index.php?jzdc=mall.goods&id='.$v['big'].'" target="_blank"><i>1'.self::$language['stock_big'].'<b href=./index.php?jzdc=mall.goods_admin&act=stock_big&ps_id='.$v['id'].'>'.self::$language['set'].'</b></i><img src=./program/mall/img_thumb/'.$big['icon'].' /><span>'.$big['title'].'</span></a>
            <div class=equal ></div><input type="text" class=quantity value="'.$v['quantity'].'" />
        	<a class=stock_small href="./index.php?jzdc=mall.goods&id='.$v['small'].'" target="_blank"><i>'.self::$language['stock_small'].'<b href=./index.php?jzdc=mall.goods_admin&act=stock_small&ps_id='.$v['id'].'>'.self::$language['set'].'</b></i><img src=./program/mall/img_thumb/'.$small['icon'].' /><span>'.$small['title'].'</span><b class=del>'.self::$language['del'].'</b></a>            
        </div>';	
}
$module['list']=$list;
if($_GET['current_page']==1){$module['list']='<div class="p_group">
        	<a class=stock_big href=#><i>1'.self::$language['stock_big'].'<b href=./index.php?jzdc=mall.goods_admin&act=stock_big&ps_id=0>'.self::$language['set'].'</b></i><img src=./program/mall/img/default.png /><span>'.self::$language['no_set'].'</span></a>
            <div class=equal ></div><input type="text" class=quantity />
        	<a class=stock_small href=#><i>'.self::$language['stock_small'].'<b href=./index.php?jzdc=mall.goods_admin&act=stock_small&ps_id=0>'.self::$language['set'].'</b></i><img src=./program/mall/img/default.png /><span>'.self::$language['no_set'].'</span></a>            
        </div>'.$module['list'];}
