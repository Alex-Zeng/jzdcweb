<?php
/**
 *	商城数据显示类 示例 ./index.php?jzdc=mall.goods_add (jzdc=类名.方法名)，大部分情况是通过 __call方法 加载执行 ./program/mall/show/ 目录下的对应名称的文件
 */
class mall{
	...
	function __construct($pdo){
		if(!self::$config){
			...
			self::$one_task=false;
			if($_COOKIE['jzdc_device']=='phone'){self::$one_task=true;}
			...
			if(isset($_SESSION['jzdc']['username'])){
				if(!isset($_SESSION['jzdc'][SHOP_ID.'_discount'])){
					...
					if($r['group_id']=='' || $r['group_id']<1){
						self::add_shop_buyer($pdo,$_SESSION['jzdc']['username'],SHOP_ID);//自动添加到店内会员
						$sql="select `group_id` from ".self::$table_pre."shop_buyer where `shop_id`=".SHOP_ID." and `username`='".$_SESSION['jzdc']['username']."'  limit 0,1";
						$r=$pdo->query($sql,2)->fetch(2);
					}
					*/
					if($r['group_id']=='' || $r['group_id']<1){
							$_SESSION['jzdc'][SHOP_ID.'_shop_group']='';
							$_SESSION['jzdc'][SHOP_ID.'_discount']=10;
							$_SESSION['jzdc'][SHOP_ID.'_group_id']=0;
					}else{
						$sql="select `name`,`discount`,`id` from ".self::$table_pre."shop_buyer_group where `id`=".$r['group_id']." and `shop_id`=".SHOP_ID." limit 0,1";
						$r=$pdo->query($sql,2)->fetch(2);
						$_SESSION['jzdc'][SHOP_ID.'_shop_group']=$r['name'];
						$_SESSION['jzdc'][SHOP_ID.'_discount']=$r['discount'];
						$_SESSION['jzdc'][SHOP_ID.'_group_id']=$r['id'];
					}
...
//======================================================================================================= 获取 平台商品分类 上级名称
	function get_type_names($pdo,$id,$target,$title=''){
		$names='<a href="./index.php?jzdc=mall.goods_list&type='.$id.'" class=parent target='.$target.'>'.$title.'</a>';			
		...
		foreach($r as $v){
			$v=de_safe_str($v);
			if($v['url']==''){$v['url']='./index.php?jzdc=mall.goods_list&type='.$v['id'];}
			...
			foreach($r2 as $v2){
				$v2=de_safe_str($v2);
				if($v2['url']==''){$v2['url']='./index.php?jzdc=mall.goods_list&type='.$v2['id'];}
				...
				foreach($r3 as $v3){
					$v3=de_safe_str($v3);
					if($v3['url']==''){$v3['url']='./index.php?jzdc=mall.goods_list&type='.$v3['id'];}
...
//======================================================================================================= 获取 店内商品分类 上级名称
	function get_shop_type_names($pdo,$id,$target,$title=''){
		$names='<a href="./index.php?jzdc=mall.shop_goods_list&shop_id='.SHOP_ID.'&type='.$id.'" class=parent target='.$target.'>'.$title.'</a>';			
		...
		foreach($r as $v){
			$v=de_safe_str($v);
			$v['url']='./index.php?jzdc=mall.shop_goods_list&shop_id='.SHOP_ID.'&type='.$v['id'];
			$names.='<a href="'.$v['url'].'" id="type_'.$v['id'].'" target='.$target.'><b class=n>'.$v['name'].'</b><p class=show_sub>&nbsp;</p></a>';
			...
			foreach($r2 as $v2){
				$v2=de_safe_str($v2);
				$v2['url']='./index.php?jzdc=mall.shop_goods_list&shop_id='.SHOP_ID.'&type='.$v2['id'];
				$names.='<a href="'.$v2['url'].'" id="type_'.$v2['id'].'" target='.$target.'><b class=n>'.$v2['name'].'</b><p class=show_sub>&nbsp;</p></a>';
				...
				foreach($r3 as $v3){
					$v3=de_safe_str($v3);
					$v3['url']='./index.php?jzdc=mall.shop_goods_list&shop_id='.SHOP_ID.'&type='.$v3['id'];
					$names.='<a href="'.$v3['url'].'" id="type_'.$v3['id'].'" target='.$target.'>'.$v3['name'].'</a>';
				}
				$names.='</div>';
			}
...
//======================================================================================================= 根据平台分类设定当前所在位置条（后台专用）
	function get_type_user_position($pdo,$id){
		...
		$sql="select `name`,`parent`,`id` from ".self::$table_pre."type where `id`=$id";
		$r=$pdo->query($sql,2)->fetch(2);
		$position='<a href="./index.php?jzdc=mall.type&parent='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>';
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
			$position='<a href="./index.php?jzdc=mall.type&parent='.$r['id'].'&order='.@$_GET['order'].'">'.$r['name'].'</a>'.$position;
		}
		if($r['parent']>0){
			$sql="select `name`,`parent`,`id` from ".self::$table_pre."type where `id`=".$r['parent'];
			$r=$pdo->query($sql,2)->fetch(2);
...
//======================================================================================================= 采购商支付信息
    function get_buyer_pay_info($pdo,$id){
        $sql="select pay_type, `number`, picture, DATE_FORMAT(accept_time,\"%Y-%m-%d\") as accept_time, DATE_FORMAT(pay_time,\"%Y-%m-%d\") as pay_time from jzdc_mall_order_pay where (pay_type=1 or pay_type=2) and `order_id`=".$id." limit 1";
        $r=$pdo->query($sql,2)->fetch(2);
        if (empty($r)){
            return "无";
        }
        if ($r['pay_type'] == 1) {
            //转账
            $desc = "转账, 流水号:".$r['number'].",支付日期:".$r['pay_time'];
        }else if ($r['pay_type'] == 2){
            $desc = "汇票, 票号:".$r['number'].",承兑时间:".$r['accept_time'];
        }

        if(!empty($r['picture'])){
            $desc.="  <a href='./program/mall/img/".$r['picture']."' target='_blank' style='color:#3399CC;'>【回执照片】</a>";
        }

       return $desc;
    }
//======================================================================================================= 采购商支付信息
    function get_supplier_pay_info($pdo,$id){
        $sql="select pay_type, `number`, picture, DATE_FORMAT(accept_time,\"%Y-%m-%d\") as accept_time, DATE_FORMAT(pay_time,\"%Y-%m-%d\") as pay_time from jzdc_mall_order_pay where (pay_type=3 or pay_type=4) and `order_id`=".$id." limit 1";
        $r=$pdo->query($sql,2)->fetch(2);
        if (empty($r)){
            return "无";
        }
...
//======================================================================================================= 返回商品供应商 options 
	function get_goods_supplier_option($pdo){
        ...
        //从供应商用户组里面选择
        $sql="select id, real_name as name from jzdc_index_user where `group`=5 and `state`=1 ";
        $r=$pdo->query($sql,2);
		$list='';
		foreach($r as $v){
			$list.='<option value="'.$v['id'].'">'.$v['name'].'</option>';
...
//======================================================================================================= 返回商品供应商名称 
	function get_supplier_name($pdo,$id){
        ...
        //从供应商用户组里面选择
        $sql="select real_name as name from jzdc_index_user where `id`=".$id;
		$r=$pdo->query($sql,2)->fetch(2);
		return $r['name'];
	}
...
//======================================================================================================= 返回店铺满元惠方式
	function get_fulfil_preferential_method_sub($language,$v){
		...
		if($v['use_method']==0){
			...
			$r=$language['reduction'].$v['less_money'].$language['yuan'];
		}else{
...
//======================================================================================================= 返回店铺满元惠信息
	function get_fulfil_preferential_method($pdo,$table_pre,$language,$shop_id){
		...
		foreach($r as $v){
			$temp='';
			$temp=self::$language['join_goods_front'][$v['join_goods']].self::$language['full'].$v['min_money'].$language['yuan'];	
...
//======================================================================================================= 更新逾期未付款订单状态为，超时取消
	function update_expire_order($pdo,$table_pre,$pay_time_limit){
		...
		$sql="select * from  ".$table_pre."order where `state`=0 and `add_time`<".$remain." and `check_code_state`!=1";
		$r=$pdo->query($sql,2);
		foreach($r as $v){
	...
	function update_pre_sale_order_13($pdo){
		...
		foreach($r as $v){
			...
			if($pre['last_pay_start_time']<=$time){
				$actual_money=$v['goods_money']-$pre['reduction']+$v['express_cost_buyer'];
				$sql="update ".self::$table_pre."order set `state`='13',`actual_money`=".$actual_money." where `id`=".$v['id'];
				$pdo->exec($sql);
			}
		}*/
		...
		if(intval($r['decrease_quantity'])==intval($order['state']) && $order['inventory_decrease']==0){
			...
			foreach($r as $v){
				...
				if($pdo->exec($sql)){
					...
					$sql="insert into ".$table_pre."goods_quantity_log (`goods_id`,`quantity`,`username`,`time`,`order_id`,`batch_id`,`shop_id`,`s_id`) values ('".$v['goods_id']."','-".$v['quantity']."','jzdc','".time()."','".$order['id']."','".$batch_id."','".$order['shop_id']."','".$v['s_id']."')";
					$pdo->exec($sql);
	...
	function update_cart($pdo,$table_pre){
		if(isset($_SESSION['jzdc']['username'])){
			if(@$_COOKIE['mall_cart']!=''){
				...
				foreach($cart as $k=>$v){
					$sql="select `id` from ".$table_pre."cart where `username`='".$_SESSION['jzdc']['username']."' and `key`='".safe_str($k)."' limit 0,1";
					$r=$pdo->query($sql,2)->fetch(2);
					if($r['id']!=''){
...
//====================================================================================================================================虚拟物品自动发货 start
	function virtual_auto_delivery($config,$language,$pdo,$table_pre,$order){
		...
		if($g_ids!=''){
			...
			if(isset($random_code)){
				$sql="update ".$table_pre."order set `check_code`='".$random_code."' where `id`=".$order['id'];
				$pdo->exec($sql);	
			}
			...
			$sql="update ".$table_pre."order set `send_time`='".time()."' where `id`=".$order['id'];
			$pdo->exec($sql);	
...
//======================================================================================================= 判断是否店内VIP会员(弃用)
	function is_shop_vip($pdo,$shop_id){
		...
		$sql="select `vip` from ".self::$table_pre."shop_buyer where `shop_id`=".$shop_id." and `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
		$r=$pdo->query($sql,2)->fetch(2);
...
//=======================================================================================================【检测是否总部】
	...
	function collect_interest($pdo){
			...
			case 'mall.goods_list':
				if(@$_GET['search']!=''){
					...
					if($r['parent']==0){
						...
						$pdo->exec($sql);
						$sql="select `id` from ".self::$table_pre."interest_group_user where `group_id`=".$gid." and `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
						$r=$pdo->query($sql,2)->fetch(2);
						if($r['id']==''){
							$sql="insert into ".self::$table_pre."interest_group_user (`group_id`,`username`,`frequency`,`last_time`) values ('".$gid."','".$_SESSION['jzdc']['username']."','1','".$time."')";
							$pdo->exec($sql);
						}else{
							$sql="update ".self::$table_pre."interest_group_user set `frequency`=`frequency`+1,`last_time`=".$time." where `group_id`=".$gid." and `username`='".$_SESSION['jzdc']['username']."' limit 1";
							$pdo->exec($sql);	
						}
		...
		foreach($word as $v){
			...
			$sql="select `id` from ".self::$table_pre."interest_word_user where `word_id`=".$id." and `username`='".$_SESSION['jzdc']['username']."' limit 0,1";
			$r=$pdo->query($sql,2)->fetch(2);
			if($r['id']==''){
				$sql="insert into ".self::$table_pre."interest_word_user (`word_id`,`username`,`frequency`,`last_time`) values ('".$id."','".$_SESSION['jzdc']['username']."','1','".$time."')";
				$pdo->exec($sql);
				
			}else{
				$sql="update ".self::$table_pre."interest_word_user set `frequency`=`frequency`+1,`last_time`=".$time." where `word_id`=".$id." and `username`='".$_SESSION['jzdc']['username']."' limit 1";
				$pdo->exec($sql);	
			}
