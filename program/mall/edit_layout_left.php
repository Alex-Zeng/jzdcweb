...
$edit_panel='<p id=edit_page_layout_div style="display:none;"><span id=save_composing_state></span><a class=add_module href="./index.php?jzdc=mall.edit_page_layout_add_module&area=head&url=">'.$edit_malllayout_language['add']. $edit_malllayout_language['module'].'</a><a href=# id=save_composing>'.$edit_malllayout_language['save'].$edit_malllayout_language['composing'].'</a><a href=# id=cancel_composing>'.$edit_malllayout_language['cancel'].$edit_malllayout_language['composing'].'</a><a href="./index.php?jzdc=mall.edit_page_layout&program=mall&url='.$_GET['jzdc'].'" id=expert_mode>'.$edit_malllayout_language['expert_mode'].'</a><a href=# id=switch_composing>'.$edit_malllayout_language['switch'].$edit_malllayout_language['composing'].'</a>
...
$(document).ready(function(){
	...
	$('#edit_page_layout_div .add_module').click(function(){
		...
		$(this).attr('href',$(this).attr('href')+get_param('jzdc')+'&params='+temp);
	});
	$('#edit_page_layout_div #save_composing').click(function(){
		...
		$("[shop_layout=head] [jzdc-module]").each(function(index, element) {
			if($(this).attr('save_name')){m_head+=$(this).attr('save_name')+',';}else{ m_head+=$(this).attr('jzdc-module')+',';}
        });
		$("[shop_layout=left] [jzdc-module]").each(function(index, element) {
			if($(this).attr('save_name')){m_left+=$(this).attr('save_name')+',';}else{ m_left+=$(this).attr('jzdc-module')+',';}
        });
		$("[shop_layout=right] [jzdc-module]").each(function(index, element) {
			if($(this).attr('save_name')){m_right+=$(this).attr('save_name')+',';}else{ m_right+=$(this).attr('jzdc-module')+',';}
        });
		$("[shop_layout=full] [jzdc-module]").each(function(index, element) {
			if($(this).attr('save_name')){m_full+=$(this).attr('save_name')+',';}else{ m_full+=$(this).attr('jzdc-module')+',';}
        });
		$("[shop_layout=bottom] [jzdc-module]").each(function(index, element) {
			if($(this).attr('save_name')){m_bottom+=$(this).attr('save_name')+',';}else{ m_bottom+=$(this).attr('jzdc-module')+',';}
        });
		...
		$.post("./receive.php?target=mall::edit_page_layout&act=save_composing&malllayout="+layout+"&jzdc="+get_param('jzdc'), {m_head:m_head,m_left:m_left,m_right:m_right,m_full:m_full,m_bottom:m_bottom},function(data){
			//alert(data);
			v=eval("("+data+")");
			...
			if(v.state=='success'){
				...
				if(get_param('jzdc')==''){url='index.php';}
	...
	$("#composing_selection a").click(function(){
		...
		$("#set_composing_state").load("receive.php?target=mall::edit_page_layout&act=switch_composing&layout=full&jzdc="+get_param('jzdc')+'&to='+css,function(){
	...
	$(".module_del").click(function(){
		...
		page=page.replace(/\./,'_');
		//jzdc_alert($(this).parent().parent().prev().attr('id')+','+page);
		module_name=$(this).parent().parent('[jzdc-module]').attr('id').replace(/_\d{1,}/,'');
		//jzdc_alert(module_name);
		if(module_name==page){
			jzdc_alert('<?php echo $edit_malllayout_language['this_is_the_main_module_of_the_page_can_not_be_remove'];?>');
			return false;	
		}
		
		
		$(this).parent().parent('[jzdc-module]').remove();
        //$(this).parent().remove();
		return false;	
	});
	
	$(".module_edit_content").each(function(index, element) {
    	module_name=$(this).parent().parent('[jzdc-module]').attr('id');   
	...
	$(".module_edit_template").each(function(index, element) {
		temp=$(this).parent().parent('[jzdc-module]').attr('id');
		if(temp){
	...
	$(".module_edit_quantity").each(function(index, element) {
    	module_name=$(this).parent().parent('[jzdc-module]').attr('id');   
		//jzdc_alert(module_name);
		$(this).attr('href','index.php?jzdc=mall.set_module_data_quantity&module='+module_name);
    });
...
.ui-sortable [jzdc-module] a,ul,li{cursor:move; }
.ui-sortable [jzdc-module] a:hover{cursor:move; }
.ui-sortable [jzdc-module]{cursor:move; }
</style>
<div id=mall_layout >
     	...
     	<div id='top_malllayout_inner' align="center">
            <div id='malllayout_top'  shop_layout="head">
				...
				if($v['args']!='' && !is_numeric($v['args'])){
					$attribute_set_url='./index.php?jzdc='.get_class($v['object']).'.'.$v['method'].'_set&args='.urlencode($v['args']).'&url='.$_GET['jzdc'].'&id='.@$_GET['id'];
