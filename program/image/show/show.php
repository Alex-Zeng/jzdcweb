...
if($id==0){echo (self::$language['need_params']);return false;}else{
	...
	$module['img_list']='';
	foreach($r as $vv){
		$list.="<a id=".$vv['id']." href='./index.php?jzdc=image.show&id=".$vv['id']."&type=$type&search=".urlencode($_GET['search'])."&order=".$_GET['order']."'><img osrc=./program/image/img_thumb/".$vv['src']."  title=\"".$vv['title']."\"  ></a>";	
		$module['img_list'].="<li><img osrc=./program/image/img/".$vv['src']." id=i_".$vv['id']." title=".$vv['title']." /></li>";	
	}
$module['go_url']="./index.php?jzdc=image.show&type=$type&search=".urlencode($_GET['search'])."&order=".$_GET['order']."&id=";
...
if(@$_COOKIE['touch']){
	$t_path='./templates/'.$m_require_login.'/'.$class.'/'.self::$config['program']['template_'.$m_require_login].'/'.$_COOKIE['jzdc_device'].'/'.str_replace($class."::","",$method).'_touch.php';
