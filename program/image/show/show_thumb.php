...
if($_GET['type']>0){
	...
	if($_GET['current_page']==1 || $_COOKIE['jzdc_device']=='pc'){
...
foreach($r as $v){
	...
	if($v['link']==''){$link='./index.php?jzdc=image.'.$detail_show_method.'&id='.$v['id'].'&type='.@$_GET['type'].'&order='.$_GET['order']."&search=".urlencode($_GET['search']);}else{$link=$v['link'];}
