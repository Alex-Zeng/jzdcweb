...
class page{
	...
	function __construct(){
		...
		if(isset($_GET['u']) && $_GET['u']!=''){//短网址分享跳转
			...
			$key=substr($_GET['u'],0,8);
			$sql="select `url`,`id` from ".$this->pdo->index_pre."surl where `key`='".$key."' limit 0,1";
			$r=$this->pdo->query($sql,2)->fetch(2);
			if($r['url']!=''){
				$sql="update ".$this->pdo->index_pre."surl set `click`=`click`+1 where `id`=".$r['id'];
				$this->pdo->exec($sql);
				header('location:./index.php?jzdc='.$r['url'].'&share='.substr($_GET['u'],8));exit;
			}
		}
		...
		if(isset($_SESSION['jzdc']['function']) && $edit_page_layout){
			if(in_array('index.edit_page_layout',$_SESSION['jzdc']['function'])){$edit_page_layout=true;}else{$edit_page_layout=false;}
		...
		if($config['web']['weixin_auto_login'] && $_COOKIE['jzdc_device']=='phone' && isWeiXin() && !isset($_SESSION['jzdc']['username']) && @$_GET['jzdc']!='index.oauth_bind_option' && !isset($_GET['oauth'])){
			header('location:./oauth/wx/?backurl=http://'.str_replace('&','|||',get_url()));	
		}
		
		$_POST['jzdc_user_color_set']=get_color_array($this->pdo);
		...
		if($v){
			...
			if(!isset($_SESSION['jzdc']['page'])){
				if($v['require_login']=='1'){
					$backurl=get_url();
					$backurl=str_replace('&','|||',$backurl);
					header("location:./index.php?jzdc=index.login&backurl=".$backurl);
				}else{
					send_user_set_cookie($this->pdo);	
				}
			}else{
				if(!in_array($url,$_SESSION['jzdc']['page']) && $v['require_login']=='1'){
					if(!$edit_page_layout){
						exit('<div align="center" style="padding-top:20px;">'.$language['page_noPower'].' <a href=./index.php?jzdc=index.user>'.$language['go_home'].'</a></div>');
					}
				}
			...
			if($program_config['program']['state']=='closed'){header("location:./index.php?jzdc=index.closed&program=$program");}
			...
			if($url=='index.user'){
				$sql="select `user_sum` from ".$this->pdo->index_pre."group where `id`=".$_SESSION['jzdc']['group_id'];
			...
			$css_path="./templates/".$v['require_login']."/$program/".$program_config['program']['template_'.$v['require_login']]."/".$_COOKIE['jzdc_device']."/main.css";
			if(!is_file($css_path)){
				$temp=require("./program/index/config.php");
				$css_path="./templates/0/index/".$temp['program']['template_0']."/".$_COOKIE['jzdc_device']."/main.css";
			}
			
			//===============================================================================================短网址生成输出 start
			if($v['require_login']=='0'){
				
				function surl_key_exist($pdo,$key){
					$sql="select `id` from ".$pdo->index_pre."surl where `key`='".$key."' limit 0,1";
					$r=$pdo->query($sql,2)->fetch(2);
					if($r['id']!=''){return true;}else{return false;}
				}
				$url=get_url();
				$url=explode('jzdc=',$url);
				if(isset($url[1])){
					$url=safe_str($url[1]);
				...
				$sql="select * from ".$this->pdo->index_pre."surl where `url`='".$url."' limit 0,1";
				$share=$this->pdo->query($sql,2)->fetch(2);	
				if($share['id']==''){
					$key=get_random(8);
					if(surl_key_exist($this->pdo,$key)){$key=get_random(8);}
					if(surl_key_exist($this->pdo,$key)){$key=get_random(8);}
					if(surl_key_exist($this->pdo,$key)){$key=get_random(8);}
					if(surl_key_exist($this->pdo,$key)){$key=get_random(8);}
					if(surl_key_exist($this->pdo,$key)){$key=get_random(8);}
					if(!surl_key_exist($this->pdo,$key)){
						$title=explode('-',$head['title']);
						$sql="insert into ".$this->pdo->index_pre."surl (`url`,`key`,`title`,`time`) values ('".$url."','".$key."','".$title[0]."','".time()."')";
				...
				if($share['id']==''){
					...
					$share_url='http://'.$config['web']['domain'].'/?u='.$share['key'];
				}
			...
			if($edit_page_layout){
				...
				$_GET['jzdc']=(@$_GET['jzdc']!='')?$_GET['jzdc']:'index.index';
				$edit_panel='<p id=edit_page_layout_div style="display:none;"><span id=save_composing_state></span><a class=add_module href="./index.php?jzdc=index.edit_page_layout_add_module&area=head&url=">'.$edit_layout_language['add']. $edit_layout_language['module'].'</a><a href=# id=save_composing>'.$edit_layout_language['save'].$edit_layout_language['composing'].'</a><a href=# id=cancel_composing>'.$edit_layout_language['cancel'].$edit_layout_language['composing'].'</a><a href="./index.php?jzdc=index.edit_page_layout&program='.$program.'&url='.$_GET['jzdc'].'" id=expert_mode>'.$edit_layout_language['expert_mode'].'</a><a href=# id=switch_composing>'.$edit_layout_language['switch'].$edit_layout_language['composing'].'</a>
	 ...
	 protected function get_moudule($str,$obj){
		...
		foreach($module as $v){
			...
			if(!isset($_SESSION['jzdc']['function'])){
				if(!in_array($temp_v,$config['unlogin_function_power'])){$v2[0]='index';$v2[1]='need_login';}
				
			}else{
				if(!in_array($temp_v,$config['unlogin_function_power']) && !in_array($temp_v,$_SESSION['jzdc']['function'])){
					//echo 'xx';
