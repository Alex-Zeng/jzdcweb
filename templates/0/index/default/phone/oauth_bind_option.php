<div id=<?php echo $module['module_name'];?>  class="portlet light" jzdc-module="<?php echo $module['module_name'];?>">
<script>
	 ...
	 $("#<?php echo $module['module_name'];?> .skip").click(function(){
		...
		$.get('<?php echo $module['action_url'];?>',function(data){
			...
			if(data=="<script>window.close();<\/script>"){window.close();}else{
				...
				$("#<?php echo $module['module_name'];?>_html .skip").css('opacity',1);
				window.location.href='./index.php?jzdc=index.user&time='+((new Date()).getTime());
			}
...
#<?php echo $module['module_name'];?>_html .loading{margin-top:10px;}
    ...
    <div id="<?php echo $module['module_name'];?>_html">
        ...
        <div class=body>
            ...
            <a href=./index.php?jzdc=index.reg_user&group_id=<?php echo $module['default_group_id']?>&oauth=1 class=new><?php echo self::$language['bind'];?><?php echo self::$language['new_account'];?></a>
            <a href=./index.php?jzdc=index.login&oauth=1 class=old><?php echo self::$language['bind'];?><?php echo self::$language['existing_account'];?></a>
