<div id=<?php echo $module['module_name'];?>  jzdc-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		if($(".phone_country").attr('class')){
			...
			$(".phone_country").val($('.phone_country').attr('jzdc_value'));
	...
	$("#<?php echo $module['module_name'];?>_html input").blur(function(data){
        ...
        inpu_value = $("#"+this.id).val();
		if(this.id=='username'){
            if(inpu_value=='' || inpu_value=='<?php echo self::$language['username_hint'];?>' || inpu_value.length<4 || inpu_value.length>20  ){
                $("#"+this.id+"_state").html('请输入4-20位数字、字母、汉字');
                username.focus();
                return false;
            }else{
                //alert("post");
                $("#"+this.id+"_state").html("<span class='fa fa-spinner fa-spin'></span>");
                $.post('<?php echo $module['action_url'];?>&act=check_user',{username:inpu_value,token:$("#<?php echo $module['module_name'];?> #token").val()}, function(data){
                    try{v=eval("("+data+")");}catch(exception){alert(data);}
                    if (v.state == 'fail' ){
                        $("#username_state").html("" + v.info);
                    }else{
                        $("#username_state").html("");
                    }
                    return false;
		...
		if(this.id=='password'){
            if(inpu_value.length<6 || inpu_value.length>20){
                $("#"+this.id+"_state").html('密码长度6~20位');
                $("#"+this.id).focus();
                return false;
            }
        }
        if(this.id=='confirm_password'){
            if(inpu_value.length<6 || inpu_value.length>20){
                ...
                $("#"+this.id).focus();
                return false;
            }else if(inpu_value!=$("#password").val()){
                $("#"+this.id+"_state").html('<?php echo self::$language['twice_password_not_same']?>');
                $("#"+this.id).focus();
                return false;
            }
        }
        if(this.id=='phone'){
            if(inpu_value.length!=11){
                $("#"+this.id+"_state").html('请输入有效的手机号码');
                $("#"+this.id).focus();
                return false;
            }else{
                $("#"+this.id+"_state").html("<span class='fa fa-spinner fa-spin'></span>");
                $.post('<?php echo $module['action_url'];?>&act=check_phone',{phone:inpu_value,token:$("#<?php echo $module['module_name'];?> #token").val()}, function(data){
                    try{v=eval("("+data+")");}catch(exception){alert(data);}
                    if (v.state == 'fail' ){
                        $("#phone_state").html("" + v.info);
                    }else{
                        $("#phone_state").html("");
                    }
	...
	$(".get_verification_code").click(function(){
		...
		if($("#<?php echo $module['module_name'];?>_html #email").attr('type')){
			...
			$.post('<?php echo $module['action_url'];?>&act=get_verification_code',{phone:$("#<?php echo $module['module_name'];?> #phone").val(),phone_country:86,email:$("#<?php echo $module['module_name'];?> #email").val(),token:$("#<?php echo $module['module_name'];?> #token").val(),}, function(data){
				//alert(data);
				try{v=eval("("+data+")");}catch(exception){alert(data);}				
				if(v.info=='weixin'){
    ...
    function exe_check(){
        ...
        if(username.prop('value')=='' || username.prop('value')=='<?php echo self::$language['username_hint'];?>' || username.prop('value').length<4 || username.prop('value').length>20  ){
            alert("请输入4-20位数字、字母、汉字");
            username.focus();
            return false;
        }
        if(password.prop('value')==''){password.focus();return false;}
        if(confirm_password.prop('value')==''){confirm_password.focus();return false;}
        if(password.prop('value').length<6 || password.prop('value').length>20){alert('密码长度6~20位');password.focus();return false;}
        ...
        top_ajax_form('jzdc_form','login_div','show_result');
        return false;
        }
    ...
    function show_result(){
        ...
        if(json.errType=='exe_success'){jzdc_alert(json.errInfo);window.location.href='index.php?jzdc=index.login';return false;}
       // if(json.errType=='exe_fail'){jzdc_alert(json.errInfo);}
        //jzdc_alert(json.errType);
		
		
		
		jzdc_alert(json.errInfo);
		$("#"+json.errType).focus();
...
.title_div a{ display: inline-block; vertical-align:top; width:35%; text-align:right; padding-right:1rem; font-size:0.8rem; overflow:hidden;color:<?php echo $_POST['jzdc_user_color_set']['nv_1']['background']?>  !important;}
.title_div a:hover{opacity:0.8;}
#<?php echo $module['module_name'];?>_html .line .agreement{ width:1rem !important;}
.agreement_div{color:<?php echo $_POST['jzdc_user_color_set']['nv_1']['background']?>;}
.agreement_div a{color:<?php echo $_POST['jzdc_user_color_set']['nv_1']['background']?>  !important;}
...
#<?php echo $module['module_name'];?>_html .phone_input_div #phone{ display:inline-block; vertical-align:top;  border:none; line-height:28px; height:28px;}
    ...
    <div id=<?php echo $module['module_name'];?>_html>
        ...
        <div class=form_div>
        	<div class=f_body>
        		...
        		<div class=title_div><span><?php echo self::$language['welcome_reg']?></span><a href="./index.php?jzdc=index.login"><?php echo self::$language['existing_account']?></a></div>

                <form id="jzdc_form" name="jzdc_form" method="GET" action="<?php echo $module['action_url'];?>" onSubmit="return exe_check();">
                  ...
                  <div class=line><span class="m_label"><?php echo self::$language['username'];?>：</span><span class=input_span><input class="input_text" type="text" name="username" id="username" placeholder="4-20位数字、字母、汉字"  onfocus="clear_defaut_value();" /></div> </span><span id=username_state></span>

                  <div class=line><span class="m_label"><?php echo self::$language['password'];?>：</span><span class=input_span><input class="input_text" type="password" name="password" id="password" placeholder="密码长度6~20位"/></span></div><span id=password_state></span>
                  
                  <div class=line><span class="m_label"><?php echo self::$language['confirm'];?><?php echo self::$language['password'];?>：</span><span class=input_span><input class="input_text" type="password" name="confirm_password" id="confirm_password" placeholder="请再次输入密码"/></span></div><span id=confirm_password_state></span>
                  ...
                  <div class="line agreement_div"><span class="m_label">&nbsp;</span><span class=input_span><input type="checkbox" class=agreement checked="" /><?php echo self::$language['agreement']?><a href="<?php echo $module['user_agreement_url'];?>" target="_blank" id=user_agreement><?php echo self::$language['user_agreement'];?></a></span></div>
