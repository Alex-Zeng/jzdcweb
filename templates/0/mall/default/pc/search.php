<div id=<?php echo $module['module_name'];?> jzdc-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		...
		if(get_param('search')!=''){
			$("#<?php echo $module['module_name'];?>_html .option .current_v").attr('v',$("#<?php echo $module['module_name'];?> .option .option_list span[v='"+get_param('jzdc')+"']").attr('v')).html($("#<?php echo $module['module_name'];?> .option .option_list span[v='"+get_param('jzdc')+"']").html());	
		}
		
		$("#<?php echo $module['module_name'];?> #jzdc").val($("#<?php echo $module['module_name'];?>_html .option .current_v").attr('v'));
		...
		$("#<?php echo $module['module_name'];?> .option .option_list span").click(function(){
			...
			$("#<?php echo $module['module_name'];?> #jzdc").val($(this).attr('v'));
			return false;	
		});
		...
		$("#<?php echo $module['module_name'];?> .searcch_button").click(function(){
			...
			if($("#<?php echo $module['module_name'];?>_html #search").val()=='' && $("#<?php echo $module['module_name'];?>_html #search").attr('placeholder_url')!=''){
				...
				window.location.href='./index.php?jzdc='+$("#<?php echo $module['module_name'];?>_html #jzdc").val()+'&search='+$("#<?php echo $module['module_name'];?>_html #search").val();
			}
			return false;
		});		
    });
...
#top_layout_out #top_layout_inner #layout_top {white-space: nowrap;}
	...
	#<?php echo $module['module_name'];?>_html .search_input_div{ border:2px <?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['background']?>  solid; line-height:38px; height:38px; width:100%; overflow:hidden;}
	...
	#<?php echo $module['module_name'];?>_html .search_input_div .searcch_button{padding: 0px;  display:inline-block; vertical-align:top; width:16%; height:36px; line-height:36px; border-radius:0px; border:0;  text-align:center; background:<?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['background']?>; color:<?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['text']?>;}
	...
	#<?php echo $module['module_name'];?>_html .search_hot_div a:hover{color:<?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['background']?>;}
	...
	#<?php echo $module['module_name'];?>_html .option .option_list span{ display:block;background:<?php echo $_POST['jzdc_user_color_set']['nv_2_hover']['background']?>; color:<?php echo $_POST['jzdc_user_color_set']['nv_2_hover']['text']?>;padding-left:0.5rem; padding-right:1rem;}
	#<?php echo $module['module_name'];?>_html .option .option_list span:hover{background:<?php echo $_POST['jzdc_user_color_set']['nv_3_hover']['background']?>; color:<?php echo $_POST['jzdc_user_color_set']['nv_3_hover']['text']?>;}


    </style>
    <div id="<?php echo $module['module_name'];?>_html">
      <form id="shop_goods_search" name="shop_goods_search" method="get" action="./index.php">
        <div class="search_input_div"><span class=start></span><span class=option><span class=current_v v='mall.goods_list'>商品</span><div class=option_list ><span v='mall.goods_list'>商品</span></div></span><input type="hidden" name="jzdc" id="jzdc" value="mall.goods_list" /><span class="search_middle"><input type="text" name="search" id="search" value="<?php echo @$_GET['search']?>" placeholder="<?php echo $module['search_placeholder'];?>" old_placeholder="<?php echo $module['search_placeholder'];?>" placeholder_url="<?php echo $module['search_placeholder_url'];?>" /></span><a href="#" class=searcch_button><?php echo self::$language['search']?></a></div>
