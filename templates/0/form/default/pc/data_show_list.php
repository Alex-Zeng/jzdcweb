<div id=<?php echo $module['module_name'];?>  class="portlet light" jzdc-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
        //$(document).attr('title','企业认证');
        var  ti = $(document).attr('title');
        var arry = ti.split("-");
        $(document).attr('title',arry[0]);
    ...
    function jzdc_table_filter(id){
		if($("#"+id).prop("value")!=-1){
			...
			if(key!="search"){url=replace_get(url,"search","");}else{
				url='./index.php?jzdc=form.data_show_list&search='+$("#search_filter").val()+'&order='+get_param('order')+'&table_id='+get_param('table_id');
			}
    ...
    <div class="filter"><?php echo $module['filter']?><input type="text" name="search_filter" id="search_filter" placeholder="<?php echo $module['search_placeholder']?>" value="<?php echo @$_GET['search']?>" />
       ...
       &nbsp;  <!--<a href="./index.php?jzdc=form.data_add&table_id=<?php echo $_GET['table_id'];?>" target="_blank" class="add"><?php echo self::$language['add']?><?php echo self::$language['data']?></a> -->
    </div>
