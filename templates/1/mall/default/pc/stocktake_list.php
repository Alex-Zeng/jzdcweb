<div id=<?php echo $module['module_name'];?>  class="portlet light" jzdc-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		...
		$(document).on('click',"#<?php echo $module['module_name'];?> [jzdc-table] .loss", function() {
  			id=$(this).attr('d_id');
			...
			if(confirm_return){
				...
				$.post('<?php echo $module['action_url'];?>&act=loss',{id:id,stocktake:$("#<?php echo $module['module_name'];?> #tr_"+id+" .stocktake").val()}, function(data){
					...
					if(v.state=='success'){
						$("#<?php echo $module['module_name'];?> #tr_"+id+" .order_state").html('<?php echo self::$language['success'];?>');
					}
				});
			}
			return false;
		...
		$(document).on('click',"#<?php echo $module['module_name'];?> [jzdc-table] .bulk_loss", function() {
  			id=$(this).attr('d_id');
    ...
    <div class="portlet-title">
        <div class="caption"><?php echo $module['jzdc_table_name']?> <?php echo $module['s_name'];?></div>
    </div>
                        
                        
                        
    <div class="m_row"><div class="half"><div class="dataTables_length"><select class="form-control" id="page_size" ><option value="10">10</option><option value="20">20</option><option value="50">50</option><option value="100">100</option></select> <?php echo self::$language['per_page']?></m_label></div></div><div class="half"><div class="dataTables_filter"><m_label><a href='./index.php?jzdc=mall.stocktake_add&stocktake_id=<?php echo @$_GET['stocktake_id']?>' target=_blank><?php echo self::$language['barcode']?><?php echo self::$language['stocktake']?></a> &nbsp; <?php echo self::$language['search']?>:<input type="search"  placeholder="<?php echo self::$language['goods_name']?>" class="form-control" ></m_label></div></div></div>
