<div id=<?php echo $module['module_name'];?>  class="portlet light" jzdc-module="<?php echo $module['module_name'];?>" align=left >
		...
		if('relevance_package'=='<?php echo @$_GET['act'];?>'){
			...
			if(''=='<?php echo  @$_GET['package_ids'];?>'){
				$(".package_div").each(function(index, element) {
					...
					$(this).children('div').children('.operation_div').html('<a href="index.php?jzdc=mall.relevance_package&goods_id=<?php echo @$_GET['goods_id']?>&relevance_new='+$(this).attr('id').replace(/package_div_/,'')+'" class=relevance><?php echo self::$language['relevance'];?></a>');
				});	
			}else{
	...
	#<?php echo $module['module_name'];?>_html .equal_symbol:before{font: normal normal normal 3rem/1 jzdc; content:"\f00c";}
