<div id=<?php echo $module['module_name'];?>  class="portlet light" jzdc-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		$(document).on('click','#<?php echo $module['module_name'];?> .set_groups',function(){
			...
			$("#jzdc_iframe").attr('src',$(this).attr('href')+'&ids='+$(this).prev().prev('.group_ids').val());
			return false;	
		});
    });
	...
	<div style="text-align:right;"><a class=submit href=./index.php?jzdc=mall.interest_word&act=synchronization><?php echo self::$language['synchronization']?><?php echo self::$language['type']?></a></div>
    ...
    <div class=table_scroll><table class="table table-striped table-bordered table-hover dataTable no-footer"  role="grid" style="width:100%" cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <tr id="<?php echo $module['module_name'];?>_new">
              ...
              <td><input type="hidden" name="sgroup_ids_0" id="group_ids_0" value="0" class='group_ids' /><span class=group_ids_name></span> <a href='./index.php?jzdc=mall.interest_word_grop&w_id=0' class=set_groups><?php echo self::$language['set']?></a></td>
