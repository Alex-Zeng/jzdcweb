...
<script>
		...
		try{
			...
			try{
				...
				if(confirm("<?php echo self::$language['not_serial_port']?>")){
					location="http://file.lansion.cn/other/jzdccom.exe";
				}
				return null;
			}
		}
	...
	$("#<?php echo $module['module_name'];?> .barcode").keyup(function(event){
		...
		$(window).keydown(function(event){
			...
			if(event.keyCode==32 && $(event.target).attr('class')!=='received_money' && $(event.target).attr('class')!=='address'){
				...
				$.get('<?php echo $module['action_url'];?>&act=submit',{barcode:$(this).val()}, function(data){
					...
					if(v.state=='success'){
						if(v.html){
							...
							if($("#<?php echo $module['module_name'];?> [jzdc-table] tbody tr").length>0){
								$(v.html).insertBefore("#<?php echo $module['module_name'];?> [jzdc-table] tbody tr:first");
							}else{
								$("#<?php echo $module['module_name'];?> [jzdc-table] tbody").html(v.html);
							}
						...
						if(v.act){
							...
							$("#fade_div").css('display','block');
							$("#set_jzdc_iframe_div").css('display','block');
							$("#jzdc_iframe").attr('src','./index.php?jzdc=mall.checkout_search&search='+$("#<?php echo $module['module_name'];?> .barcode").val());
							$("#<?php echo $module['module_name'];?> .barcode").val('');
						}
		...
		$("[jzdc-table] thead a").unbind("click");
		...
		$("#<?php echo $module['module_name'];?> .pay_method").change(function(){
			...
			if($(this).val()=='alipay' || $(this).val()=='weixin'){
				...
				$("#fade_div").css('display','block');
				$("#set_jzdc_iframe_div").css('display','block');
				$("#jzdc_iframe").attr('src','./index.php?jzdc=mall.checkout_search&act=scanpay&id='+$("#<?php echo $module['module_name'];?> .pay_method option[value='"+$(this).val()+"']").attr('account_id')+'&user='+$("#<?php echo $module['module_name'];?> .user").val()+'&money='+$("#<?php echo $module['module_name'];?> #amount_payable_line .number").html());
				return false;
			}
		...
		$("#<?php echo $module['module_name'];?> .temp_name").keyup(function(event){
            if(event.keyCode==13 && $(this).val()!=''){
				...
				$("#<?php echo $module['module_name'];?> [jzdc-table] tbody td").animate({opacity:0},"slow",function(){$("#<?php echo $module['module_name'];?> [jzdc-table] tbody td").css('display','none');});
		...
		$("#<?php echo $module['module_name'];?> .clear_all").click(function(){
			$("#<?php echo $module['module_name'];?> [jzdc-table] tbody tr").remove();
	...
	#<?php echo $module['module_name'];?> .input_div a{ display:inline-block; background-color:<?php echo $_POST['jzdc_user_color_set']['nv_1']['background']?>; border:1px #FFFFFF solid; height:1.78rem; overflow:hidden; padding-left:0.5rem; padding-right:0.5rem;}
	...
	#<?php echo $module['module_name'];?> .line .input_div a{ display:inline-block; background-color:<?php echo $_POST['jzdc_user_color_set']['nv_1']['background']?> border:1px #FFFFFF solid; height:3.48rem; overflow:hidden; padding-left:0.5rem; padding-right:0.5rem;}
	
	
	#<?php echo $module['module_name'];?> .checkout_header{  position:fixed; top:0px;left:0px; width:100%; height:6rem; background-color:<?php echo $_POST['jzdc_user_color_set']['nv_1']['background']?>; color:<?php echo $_POST['jzdc_user_color_set']['nv_1']['text']?>; color:#FFF; border-bottom:<?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['background']?> solid 0.3rem;}
	#<?php echo $module['module_name'];?> .checkout_header a{ color:#FFF;}
	#<?php echo $module['module_name'];?> .checkout_header .icon{ display:inline-block; vertical-align:top; width:8%; border-right:1px solid <?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['background']?>; overflow:hidden; text-align:center; height:100%;}
	...
	#<?php echo $module['module_name'];?> .checkout_header .other .shop_info{ padding-left:2rem; line-height:2.5rem; border-bottom:1px solid <?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['background']?>;}
	...
	#<?php echo $module['module_name'];?> .checkout_header .other .shop_info .cashier .unlogin{ display:inline-block; line-height:2rem; padding-left:0.5rem; padding-right:0.5rem; border-radius:5px; background-color:<?php echo $_POST['jzdc_user_color_set']['nv_1']['background']?>;}
	...
	#<?php echo $module['module_name'];?> .checkout_footer{ padding-left:3rem; position:fixed; bottom:0px;left:0px; width:100%; height:3rem; line-height:3rem; background-color:<?php echo $_POST['jzdc_user_color_set']['nv_1']['background']?>; color:#FFF;}
	#<?php echo $module['module_name'];?> .checkout_footer a{ display:inline-block; vertical-align:top; line-height:2rem; margin-top:0.5rem; margin-right:5px; padding-left:5px; padding-right:5px; background-color:<?php echo $_POST['jzdc_user_color_set']['nv_2']['background']?>; color:#FFF; border-radius:5px;}
	...
	#<?php echo $module['module_name'];?> .goods_info .title{ vertical-align:top;display:inline-block; vertical-align:top; min-width:250px; padding-left:10px; height:80px;line-height:25px; font-size:1rem; overflow:hidden; white-space:normal;  }	
	...
	#<?php echo $module['module_name'];?> [jzdc-table] .del{ background:none; vertical-align:top; height:30px; line-height:30px; }
	#<?php echo $module['module_name'];?> [jzdc-table] .del:hover{ color:#ff9600;}
	...
	#<?php echo $module['module_name'];?> [jzdc-table] thead tr td{ background:none; background-color:#f0f0f0; color:#333; height:40px; line-height:40px; font-size:1rem; text-align:left;}
	...
	#<?php echo $module['module_name'];?> [jzdc-table] tbody .del{ display:none;}
	#<?php echo $module['module_name'];?> [jzdc-table] tbody tr:hover .del{display:inline-block;}
...
#<?php echo $module['module_name'];?> .search_result_div .current{background:<?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['background']?>; color:#fff;}
        	...
        	<div class=icon><a href=./index.php  target="_blank"><img src='./program/mall/shop_icon/<?php echo $module['shop_id']?>.png' /></a></div><div class=other>
            	<div class=shop_info>
                	<div class=info_fulfil>
						<a class=shop_name href=./index.php?jzdc=mall.shop_index&shop_id=<?php echo $module['shop_id']?> target="_blank"><?php echo $module['shop_name'];?>-<?php echo self::$language['checkout_desk'];?></a>
					...
					<?php echo self::$language['cashier'];?>：<a href="./index.php?jzdc=index.user" target="_blank" class=cashier_username><?php echo $module['username']?></a> <a  class="unlogin" id="unlogin" href="./receive.php?target=index::user&act=unlogin"><?php echo self::$language['unlogin_short']?></a></div>
                </div>
                <div class=user_act>
    	...
    	<div class=checkout_middle>
            ...
            <div class=checkout_left>
               <div class=line><span class=m_label><?php echo self::$language['barcode']?>:</span><span class=value>
                   <div class=input_div>
                        <input type="text" class=barcode placeholder="<?php echo self::$language['sweep_barcode']?>/<?php echo self::$language['name']?>/<?php echo self::$language['first_pinyin']?>" /><a href="./index.php?jzdc=mall.checkout_search" style="color:#FFF;" class=add_goods><?php echo self::$language['add']?></a></div>
    ...
    <div class=print_div><iframe width="100%" id="print_tag" name="print_tag" height="600px;"  frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="./index.php?jzdc=mall.checkout_print" >
</iframe></div>
           ...
           <div class="modal-dialog">
              <div class="modal-content">
                 ...
                 <div class="modal-body">
                     ...
                     <div style="text-align:center;"><a href=http://www.lansion.cn/comset.php>串口设备高级设置调试</a></div>
                 </div>
