...
<div id=<?php echo $module['module_name'];?>  class="portlet light" jzdc-module="<?php echo $module['module_name'];?>" align=left >
    ...
    function jzdc_table_filter(id){
		if($("#"+id).prop("value")!=-1){
			...
			if(key!="search"){url=replace_get(url,"search","");}else{
				url='./index.php?jzdc=form.data_admin&search='+$("#search_filter").val()+'&order='+get_param('order')+'&table_id='+get_param('table_id');
			}
    ...
    <div class="filter"><?php echo self::$language['content_filter']?>:
       ...
       <br /><br /><a href="./index.php?jzdc=form.data_add&table_id=<?php echo $_GET['table_id'];?>" target="_blank" class="add"><?php echo self::$language['add']?><?php echo self::$language['data']?></a>
