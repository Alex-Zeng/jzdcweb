<div id=<?php echo $module['module_name'];?>  class="" jzdc-module="<?php echo $module['module_name'];?>" align=left >
<script src="./plugin/datePicker/index.php"></script>
            ...
            $("#"+this.id+"_state").load('<?php echo $module['action_url'];?>&update='+this.id,json,function(){
                ...
                if($(this).html().length>10){
                    try{v=eval("("+$(this).html()+")");}catch(exception){alert($(this).html());}
					 $(this).html(v.info);
					if(v.state=='success' && get_param('field')!=''){alert('<?php echo self::$language['success']?>');window.location.href='./index.php?jzdc=index.user';return false;}
  ...
  //      });
        $("input[type='password']").blur(function(){
            //jzdc_alert(this.id);
            if(this.id=='confirm_new_password'){
                inpu_value = $("#"+this.id).val();
                if(inpu_value.length>=6 && inpu_value!=$("#new_password").val()){
                    $("#"+this.id+"_state").html('<span class=fail><?php echo self::$language['twice_password_not_same']?></span>');
                    $("#"+this.id).focus();
                    return false;
                }
            }
    ...
    function add_transaction_password(){
        ...
        $("#update_transaction_password_state").load('<?php echo $module['action_url'];?>&update=add_transaction_password',json,function(){
            if($(this).html().length>10){
                ...
                $(this).html(v.info);
				if(v.state=='success'){alert('<?php echo self::$language['success']?>');window.location.href='./index.php?jzdc=index.user';}
                if(v.state=='success'){$("#update_transaction_password .submit_button").css("display","none");}
            }
        });
...
<!--
    ...
    <tr id="tr_transaction_password"><td colspan="2">
        ...
        <tr><td class="m_label"><?php echo self::$language['old_transaction_password']?></td><td align="left"><input type="password" id="old_transaction_password" name="old_transaction_password" value="" /><span id="old_transaction_password_state"></span>  <a href="./index.php?jzdc=index.resetPassword&field=transaction_password" id="find_transaction_password"><?php echo self::$language['get_password']?></a></td></tr>
        <?php }?>
...
-->
    <tr id="tr_password"><td colspan="2">
        ...
        <tr><td class="m_label"><?php echo self::$language['old_password']?></td><td align="left"><input type="password" id="old_password" name="old_password" value="" placeholder="请输入旧密码"/><span id="old_password_state"></span></td></tr>
        <tr><td class="m_label"><?php echo self::$language['new_password']?></td><td align="left"><input type="password" id="new_password" name="new_password" value="" placeholder="密码长度6~20位"/><span id="new_password_state"></span></td></tr>
        <tr><td class="m_label"><?php echo self::$language['confirm']?><?php echo self::$language['new_password'];?></td><td align="left"><input type="password" id="confirm_new_password" name="confirm_new_password" value="" placeholder="请再次输入新密码"/><span id="confirm_new_password_state"></span></td></tr>
