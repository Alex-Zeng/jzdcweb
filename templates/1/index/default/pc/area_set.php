<div id=<?php echo $module['module_name'];?>  class="portlet light" jzdc-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		...
		$("#<?php echo $module['module_name'];?> input[jzdc_type='map']").each(function(index, element) {
		...
		$("#<?php echo $module['module_name'];?> input[jzdc_type='map']").focus(function(){
			...
			$("#jzdc_iframe").attr('src','http://<?php echo $module['map_api'];?>.lansion.cn/get_point.php?id='+$(this).attr('id')+'&point='+$(this).val()+'&zoom=5');
			return false;	
		});
    ...
    function jzdc_table_filter(id){
		if($("#"+id).prop("value")!=-1){
			...
			if($("#"+id).prop("value")==''){
				if(key=='country'){window.location.href='./index.php?jzdc=index.area_set';}
