<div id=<?php echo $module['module_name'];?>  class="portlet light" jzdc-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		...
		$("#<?php echo $module['module_name'];?> .inquiry").click(function(){
			...
			window.location.href='./index.php?jzdc=index.admin_users&start_time='+$("#<?php echo $module['module_name'];?> #start_time").val()+'&end_time='+$("#<?php echo $module['module_name'];?> #end_time").val();
			return false;	
		});
		...
		$("#<?php echo $module['module_name'];?> .exe_import").click(function(){
			...
			$.post("<?php echo $module['action_url'];?>&act=import",{field_set:$("#<?php echo $module['module_name'];?> .field_set").val(),import_file:$("#<?php echo $module['module_name'];?> #import_file").val()},function(data){
				...
				if(v.state=='fail'){
					...
					$("#<?php echo $module['module_name'];?> .exe_import").next('span').html(v.info+' <a href=./index.php?jzdc=index.admin_users><?php echo self::$language['refresh']?></a>');
				}
			});	
		...
		$("[jzdc-table] tbody tr").unbind('mouseover');
		$("[jzdc-table] tbody tr").unbind('mouseout');
		...
		$('#bulk_action_state_selected_a').click(function(){
			...
			ids=get_ids();
			if(ids==''){jzdc_alert("<?php echo self::$language['select_null']?>");}
    ...
    .login_num{background:<?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['background']?>; color:<?php echo $_POST['jzdc_user_color_set']['nv_1_hover']['text']?> !important; }
    </style>
    <div id="<?php echo $module['module_name'];?>_html" jzdc-table='1' class='portlet light'>
    ...
    <div class="portlet-title">
        ...
        <div class="actions">
        	...
        	&nbsp; <a href=./index.php?jzdc=index.user_map class=user_map><?php echo self::$language['pages']['index.user_map']['name']?></a>
-->
            <a  href=./index.php?jzdc=index.increase_user class="add increase_user"><?php echo self::$language['increase']?><?php echo self::$language['member']?></a>
    ...
    <div class="m_row"><div class="half"><div class="dataTables_length"><select class="form-control" id="page_size" ><option value="10">10</option><option value="20">20</option><option value="50">50</option><option value="100">100</option></select> <?php echo self::$language['per_page']?></m_label></div></div>
		<!--
		<div class="half">
  ...
  <input type="text" name="search_filter" id="search_filter" placeholder="<?php echo $module['placeholder'];?>" value="<?php echo @$_GET['search']?>" />
   ...
   -->
    ...
    <?php echo  $module['list']?>
		...
		<!--  <div class='jzdc_table_bulk_action_div'><?php echo $module['bulk_action'];?></div> -->
