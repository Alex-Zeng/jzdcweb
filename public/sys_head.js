...
function close_talk_iframe(close_window){
	...
	$("#set_jzdc_iframe_div").removeClass('talk_iframe');
	$("#jzdc_iframe").attr('scrolling','yes');
	if(close_window==1){$("#jzdc_iframe").attr('src','');}
}
...
function jzdc_iframe_is_open(){
	temp=$("#jzdc_iframe").attr('src').split('index.php?jzdc=im.talk');
	if($("#set_jzdc_iframe_div").css('display')=='block' && temp[1]){return true;}else{return false;}
}
...
$(document).ready(function(){
	...
	$("#im_talk_show").click(function(){
		if(!getCookie('jzdc_id')){window.location.href='index.php?jzdc=index.login';}
		if(getCookie('jzdc_device')=='phone'){window.location.href='./index.php?jzdc=im.talk&iframe=1'; return false;}
		set_iframe_position($(window).width()*0.6,'430');
		//jzdc_alert(replace_file);
		$("#fade_div").css('display','block');
		$("#set_jzdc_iframe_div").css('display','block');
		$("#set_jzdc_iframe_div").addClass('talk_iframe');
		temp=$("#jzdc_iframe").attr('src').split('index.php?jzdc=im.talk');
		if(!temp[1]){$("#jzdc_iframe").attr('src','./index.php?jzdc=im.talk&iframe=1');}
	...
	$("[talk]").click(function(){
		if(!getCookie('jzdc_id')){window.location.href='index.php?jzdc=index.login';}
		if(getCookie('jzdc_device')=='phone'){window.location.href='./index.php?jzdc=im.talk&iframe=1&with='+$(this).attr('talk'); return false;}
		set_iframe_position($(window).width()*0.6,'430');
		//jzdc_alert(replace_file);
		$("#fade_div").css('display','block');
		$("#set_jzdc_iframe_div").css('display','block');
		$("#set_jzdc_iframe_div").addClass('talk_iframe');
		$("#jzdc_iframe").attr('src','./index.php?jzdc=im.talk&iframe=1&with='+$(this).attr('talk'));
		$("#jzdc_iframe").attr('scrolling','no');
		$("#im_talk_show").removeClass('fadeIn animated infinite');	
		return false;	
	});
	...
	if(getCookie('jzdc_device')=='phone'){$("input").focus(function(){$(window).scrollTop($(this).offset().top-40);});}
	$(".share_text_input").val($(".share_text").html());
	if(getCookie('jzdc_id')){$(".share_text_input").val($(".share_text").html()+getCookie('jzdc_id'));}
	if(getCookie('im_new')==1 && getCookie('jzdc_id')){flash_im_talk_show();}
	...
	$('.jzdc_gif').each(function(index, element) {
		show_jzdc_gif($(this).attr('id'));
		window.setInterval("show_jzdc_gif('"+$(this).attr('id')+"')",$(this).attr('speed'));
		str='';
	...
	$(".jzdc_gif a").click(function(){
		$(this).parent().parent().attr('play','0');
		$(this).parent().parent().children(".jzdc_gif_show").attr('src',$(this).parent().parent().children('.data_list').children('img').eq($(this).index()).attr('src'));
	...
	if(get_param('search')!=''){
		...
		$("[jzdc_layout=full] tbody td").each(function(index, element) {if($(this).html().indexOf( "<" )==-1){$(this).html($(this).html().replace(reg,"<font color=red>$1</font>"));}});
		$("[jzdc_layout=full] a").each(function(index, element) {if($(this).html().indexOf( "<")==-1){$(this).html($(this).html().replace(reg,"<font color=red>$1</font>"));}});
		$("[jzdc_layout=full] span").each(function(index, element) {if($(this).html().indexOf( "<")==-1){$(this).html($(this).html().replace(reg,"<font color=red>$1</font>"));}});
		$("[jzdc_layout=full] div").each(function(index, element) {if($(this).html().indexOf( "<")==-1){$(this).html($(this).html().replace(reg,"<font color=red>$1</font>"));}});
	...
	if(getCookie('jzdc_device')=='phone'){$(".table_scroll").preventScroll();}
	...
	$("#set_jzdc_iframe_div").preventScroll();
	...
	if(getCookie('jzdc_device')=='phone'){
		...
		$(".fixed_right_div a").hover(function(){
				if($(this).parent().attr('class')=='fixed_right_div'){
					$(this).html($(this).attr('title'));
					$(this).css('background-size','0px,0px');
				}			
			},function(){
				if($(this).parent().attr('class')=='fixed_right_div'){
					$(this).html('<i class="'+$(this).attr('i_class')+'"></i>');
	...
	//jzdc_alert($(window).width()+','+$(window).height());
	//jzdc_alert(navigator.language+','+navigator.browerLanguage);
	
	if(get_param('show_single_module')!=''){show_single_module(get_param('show_single_module'));}
	
	show_tutorial_button();
	//jzdc_alert($("textarea[id='content']").length);
	set_ie_placeholder();
	//
	...
	//jzdc_alert(getCookie('jzdc_device'));
	if(touchAble){
	...
	checkCookie();
	if(getCookie('user_set_page_auto_size')=='1' && getCookie('jzdc_device')=='pc' && $(window).width()<1319){
		jzdc_zoom();
		//window.onresize = function(){jzdc_zoom();}
	}
	
	
	$("#jzdc_form").keydown(function(event){
...
function show_jzdc_gif(id){
	if($("#"+id).attr('play')==0){return false;}
	if($("#"+id+" .data_list .current").next().attr('src')){
		$("#"+id+" .jzdc_gif_show").attr('src',$("#"+id+" .data_list .current").next().attr('src'));
		...
		$("#"+id+" .jzdc_gif_show").attr('src',$("#"+id+" .data_list img").eq(0).attr('src'));
		$("#"+id+" .data_list img").removeClass('current');
...
function set_iframe_position(width,height){
	$("#set_jzdc_iframe_div").css('left',($(window).width()-width) /2 ).css('top',($(window).height()-height)/2);
	$("#jzdc_iframe").css('width',width).css('height',height);
}
...
function show_tutorial_button(){
	if(top.location==location){
		if(getCookie('tutorial')==1 && getCookie('user_set_tutorial_button')==1){
			...
			$("#tutorial_button_a").attr('href','http://www.jzdc.com/index.php?jzdc=talk.content&key=page_tutorial|'+get_param('jzdc'));
		}	
	}
}

function mouse_over_sound(){
	if(getCookie('user_set_mouse_over_sound')==1 && getCookie('jzdc_device')=='pc'){$("#notice_audio").attr('src','./sound/mouse_over.ogg').removeAttr('loop');}
}
function loading_sound(){
	if(getCookie('user_set_operation_sound')==1 && getCookie('jzdc_device')=='pc'){$("#notice_audio").attr('src','./sound/loading.ogg').attr('loop','loop');}
}
function success_sound(){
	if(getCookie('user_set_operation_sound')==1 && getCookie('jzdc_device')=='pc'){$("#notice_audio").attr('src','./sound/success.ogg').removeAttr('loop');}
}
function fail_sound(){
	if(getCookie('user_set_operation_sound')==1 && getCookie('jzdc_device')=='pc'){$("#notice_audio").attr('src','./sound/fail.ogg').removeAttr('loop');}
}
function warning_sound(){
	if(getCookie('user_set_operation_sound')==1 && getCookie('jzdc_device')=='pc'){$("#notice_audio").attr('src','./sound/warning.ogg').removeAttr('loop');}
}
...
function checkCookie(){  
	if(!window.navigator.cookieEnabled){jzdc_alert("In order to properly access the website, please set your browser to receive COOKIE");}
} 
...
function jzdc_zoom(){
	...
	if(top.location==location){
		...
		//jzdc_alert(document.body.style.zoom);
...
function exePrint(){
	...
	if(exe_print=='true'){
		print_id=get_param("jzdc").replace(".","_");
		try{
			...
			if(event.keyCode==13){
				...
				$('input[type="text"]').each(function(index, element) {
                    if($(this).attr('id')==e_id){
						...
						//jzdc_alert($(inputs[index+1]).attr('id'));
...
function get_device(){
	//jzdc_alert(window.screen.availWidth+'>'+window.screen.availHeight);
	jzdc_device_temp=getCookie('jzdc_device');

    //控制只有管理员才能显示PC界面
    var arr = new Array("index.user","index.personal_center","index.edit_user","mall.seller",
        "mall.master","index.sum_set","mall.m_order_admin","diymodule.admin","diymodule.edit",
        "index.edit_page_layout","index.edit_menu","mall.goods_edit","mall.goods_add",
        "mall.goods_add_option_type","mall.shop_set","mall.order_admin","index.edit_function",
        "index.view_function","mall.apply_shop","mall.goods_admin","index.color_edit","index.color",
        "index.increase_user","mall.shop_edit","mall.shop_admin","index.img_set",
        "mall.type_option","index.edit_user_group","index.admin_edit_user","slider.img",
        "index.template_admin","slider.group","index.admin_navigation","image.admin","image.index",
        "menu.index","menu.admin","mall.m_goods_admin","index.view_page","index.edit_page",
        "index.view_menu","index.admin_group","index.config","index.admin_users","mall.type",
        "index.public_content_sttings","mall.type_attribute","mall.config","form.data_show_list","form.data_show_detail",
	    "form.table_admin","form.table_edit","form.field_add","form.field_edit","form.table_css","mall.unit");
    var pageDef = QueryString("jzdc");
    if( (window.screen.availWidth>window.screen.availHeight  && (pageDef=="index.login" || (contains(arr, pageDef) && (getCookie('jzdc_group_id')=='1'|| getCookie('jzdc_group_id')=='2'|| getCookie('jzdc_group_id')=='3')))) ){
        setCookie('jzdc_device','pc',30);

    }else{
        setCookie('jzdc_device','phone',30);
    }

    //alert('set='+getCookie('jzdc_device_set'));
	// if(!getCookie('jzdc_device_set')){
	// 	if( (window.screen.availWidth>window.screen.availHeight) ){
	// 		setCookie('jzdc_device','pc',30);
	// 	}else{
	// 		setCookie('jzdc_device','phone',30);
	// 	}
	// 	//alert(getCookie('jzdc_device')+'='+jzdc_device_temp);
	// }
	//alert(getCookie('jzdc_device'));
	if(getCookie('jzdc_device')!=jzdc_device_temp){
		//jzdc_alert(getCookie('jzdc_device'));
		url= window.location.href;
		if(url.indexOf('?')!=-1){
...
for(var name in obj) {
    jzdc_alert( "name:" + name + " value:" + obj[name] );
}
 */
...
function  touch_drag_start(id) {
	...
	obj.css('z-index',get_max_z_index());
		...
		//jzdc_alert('Y='+iY+',X='+iX);
		if($(e.target).attr('class')=='jzdc_drag_div'){event.preventDefault();}
		return false;
}
...
function  touch_drag_end() {
	...
	//event.preventDefault();
	if($(e.target).attr('class')=='jzdc_drag_div'){e.cancelBubble = true;event.preventDefault();}
}
...
function drag_move() {
	var e =window.event || arguments.callee.caller.arguments[0];
	//obj=$(id);
	//jzdc_alert(border_left+','+border_top+','+border_right+','+border_bottom);
	if (dragging){
		t_x=e.clientX - iX;
		t_y=e.clientY - iY;
		obj.css({"left":t_x + "px", "top":t_y + "px"});
...
function drag_stop(parent){dragging=false;}
 ...
 //==================================================================================================================================jzdc_table start
    $(document).ready(function(){
		//jzdc_alert($(".no_related_content_span").html());
		if($(".no_related_content_span").length!=0){
			$("[jzdc-table] .batch_operation_div").css('display','none');
			$("[jzdc-table] .bulk_action_div").css('display','none');
		}
        $("[jzdc-table] .filter select").change(function(){
            jzdc_table_filter($(this).attr('id'));
        });
         $("[jzdc-table] .filter #search_filter").keyup(function(event){
            keycode=event.which;
            if(keycode==13){jzdc_table_filter('search_filter');}
        });
        ...
        $("[jzdc-table] [group-checkable]").change(function(){
			if($(this).prop("checked")){
				$("[jzdc-table] .id").prop("checked",true);
			}else{
				$("[jzdc-table] .id").prop("checked",false);
			}
		});
		 
        $("[jzdc-table] thead a,.sort a").click(function(){
            url=window.location.href;
		...
		if(get_param('search')){$("[jzdc-table] [type=search]").val(decodeURI(get_param('search')));}
		$("[jzdc-table] [type=search]").keyup(function(event){
			keycode=event.which;
			if(keycode==13){
				url=window.location.href;
		...
		if(get_param('page_size')){$("[jzdc-table] #page_size").val(get_param('page_size'));}
        $("[jzdc-table] #page_size").change(function(){
			url=window.location.href;
        ...
        $("[jzdc-table] tbody tr td").click(function(event){
	...
	function e_search(){
		jzdc_table_filter('search_filter');
	}
    ...
    function select_all(){
        ...
        $("[jzdc-table] tbody .id").prop('checked',true);
        $("[jzdc-table] tbody tr").addClass('checked');
        return false;	
    }
    function reverse_select(){
        $("[jzdc-table] tbody .id").each(function(){
